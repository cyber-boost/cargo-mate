...
impl InstallerGenTool {
    ...
    fn parse_cargo_toml(&self, manifest_path: &str) -> Result<InstallerConfig> {
        ...
        let package = cargo_toml.get("package")
            ...
            .ok_or_else(|| ToolError::InvalidArguments("No [package] section in Cargo.toml".to_string()))?;
    ...
    fn generate_windows_installer(&self, config: &InstallerConfig, output_dir: &str) -> Result<GeneratedInstaller> {
        let installer_name = format!("{}-{}-windows-setup.exe", config.name, config.version);
        ...
        // Create PowerShell installer as lightweight option
        ...
        let files_created = vec![
            nsis_path.to_string_lossy().to_string(),
            inno_path.to_string_lossy().to_string(),
            ps_path.to_string_lossy().to_string(),
        ];

        let mut metadata = HashMap::new();
        metadata.insert("installer_type".to_string(), "Windows (NSIS/Inno Setup/PowerShell)".to_string());
        ...
        Ok(GeneratedInstaller {
            ...
            files_created,
            installer_size: 0, // Would calculate actual size
            installer_path: installer_path.to_string_lossy().to_string(),
            metadata,
        })
    }

    fn generate_macos_installer(&self, config: &InstallerConfig, output_dir: &str) -> Result<GeneratedInstaller> {
        ...
        let files_created = vec![
            ...
            postinstall_path.to_string_lossy().to_string(),
            dmg_path.to_string_lossy().to_string(),
        ];

        let mut metadata = HashMap::new();
        metadata.insert("installer_type".to_string(), "macOS (.pkg/.dmg)".to_string());
        metadata.insert("code_signing".to_string(), "Required for distribution".to_string());
    ...
    fn generate_linux_installer(&self, config: &InstallerConfig, output_dir: &str) -> Result<GeneratedInstaller> {
        ...
        // Create AppImage creation script
        ...
        let files_created = vec![
            deb_dir.join("control").to_string_lossy().to_string(),
            deb_dir.join("postinst").to_string_lossy().to_string(),
            rpm_path.to_string_lossy().to_string(),
            appimage_path.to_string_lossy().to_string(),
        ];

        let mut metadata = HashMap::new();
        metadata.insert("installer_type".to_string(), "Linux (DEB/RPM/AppImage)".to_string());
...
"#,
    ...
    fn generate_powershell_installer(&self, config: &InstallerConfig) -> Result<String> {
        Ok(format!(r#"# PowerShell Installer for {name}
# Generated by CargoMate InstallerGen
...
"#,
    ...
    fn display_report(&self, installers: &[GeneratedInstaller], output_format: OutputFormat, verbose: bool) {
        match output_format {
            OutputFormat::Human => {
                println!("\n📦 {} - Platform-Specific Installers Generated", "CargoMate InstallerGen".bold().blue());
                println!("{}", "═".repeat(70).blue());

                for installer in installers {
                    println!("\n🏗️  {} Installer:", installer.platform.to_uppercase().green());
                ...
                for installer in installers {
                    match installer.platform.as_str() {
                        ...
                        "macos" => {
                            ...
                            println!("  • PKG: productbuild --distribution Distribution --package-path .");
                        }
                        "linux" => {
            ...
            OutputFormat::Table => {
                println!("{:<12} {:<15} {:<20} {:<10}",
                        "Platform", "Type", "Output File", "Files");
                println!("{}", "─".repeat(70));

                for installer in installers {
                    println!("{:<12} {:<15} {:<20} {:<10}",
                            installer.platform,
                            installer.metadata.get("installer_type").unwrap_or(&"Unknown".to_string()),
                            std::path::Path::new(&installer.installer_path).file_name().unwrap_or_default().to_string_lossy(),
                            installer.files_created.len().to_string());
                }
            }
        }
    }
...
impl Tool for InstallerGenTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Generate platform-specific installers for your Rust application. \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        let platforms: Vec<String> = matches.get_one::<String>("platforms")
            .unwrap()
            .split(',')
        ...
        println!("📦 {} - Generating Platform-Specific Installers", "CargoMate InstallerGen".bold().blue());

        // Parse configuration
        let mut config = if Path::new(config_file).exists() {
            self.parse_cargo_toml(config_file)?
        } else {
            return Err(ToolError::InvalidArguments(format!("Config file {} not found", config_file)));
        };
        ...
        for platform in &platforms {
            ...
            match platform.as_str() {
                "windows" => {
                    match self.generate_windows_installer(&config, &platform_output.to_string_lossy()) {
                        Ok(installer) => {
                ...
                "macos" => {
                    match self.generate_macos_installer(&config, &platform_output.to_string_lossy()) {
                        Ok(installer) => {
                            generated_installers.push(installer);
                            if verbose {
                ...
                "linux" => {
                    match self.generate_linux_installer(&config, &platform_output.to_string_lossy()) {
                        Ok(installer) => {
                            generated_installers.push(installer);
                            if verbose {
        ...
        if generated_installers.is_empty() {
            return Err(ToolError::ExecutionFailed("No installers were successfully generated".to_string()));
        }

        // Handle signing and notarization
        if sign {
            println!("\n🔐 Signing installers...");
            // Implementation would depend on available signing tools
            println!("⚠️  Installer signing requires platform-specific certificates");
        }
