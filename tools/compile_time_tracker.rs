...
/// Timing information for a specific crate
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Compilation bottleneck identified
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CompileTimeTrackerTool {
    ...
    fn run_timed_compilation(&self, manifest_path: &str, args: &[&str]) -> Result<CompilationProfile> {
        ...
        let output = cmd.output()
            .map_err(|e| ToolError::ExecutionFailed(format!("Cargo build failed: {}", e)))?;
        ...
        let cpu_utilization = if duration.as_secs() > 0 {
            (crate_timings.values().map(|t| t.duration).sum::<f64>() / duration.as_secs_f64()) * 100.0
        } else {
            0.0
        };
    ...
    fn parse_cargo_json_output(&self, output: &[u8]) -> Result<HashMap<String, CrateTiming>> {
            ...
            if let Ok(value) = serde_json::from_str::<serde_json::Value>(line) {
                if value["reason"] == "compiler-artifact" {
                    if let Some(package_id) = value["package_id"].as_str() {
                        let crate_name = package_id.split(' ').next().unwrap_or(package_id).to_string();

                        // Extract timing information (simplified - real implementation would need more sophisticated parsing)
                        let duration = 1.0; // Placeholder - real timing would come from cargo's timing output
                        let dependencies = Vec::new(); // Would parse from JSON
                        let source_files = 1; // Would count actual source files
                        let lines_of_code = 100; // Would count actual lines

                        timings.insert(package_id.to_string(), CrateTiming {
                            crate_name,
                            duration,
                            dependencies,
                            source_files,
        ...
        // If no JSON timing data, create mock data for demonstration
        if timings.is_empty() {
            let mock_crates = vec![
                ("serde_derive", 12.8, vec!["serde", "quote", "syn"]),
                ("regex-syntax", 8.4, vec!["regex"]),
                ("tokio", 7.1, vec!["bytes", "pin-project-lite"]),
                ("futures", 4.2, vec!["futures-core"]),
                ("clap", 3.9, vec!["clap_derive"]),
            ];

            for (name, duration, deps) in mock_crates {
                timings.insert(name.to_string(), CrateTiming {
    ...
    fn estimate_memory_usage(&self, timings: &HashMap<String, CrateTiming>) -> u64 {
        // Simple estimation based on crate count and complexity
        let base_memory = 50_000_000u64; // 50MB base
        let per_crate_memory = 10_000_000u64; // 10MB per crate
        let complexity_memory = timings.values()
            .map(|t| t.lines_of_code as u64 * 1000) // 1KB per line of code
            .sum::<u64>();

        base_memory + (timings.len() as u64 * per_crate_memory) + complexity_memory
    }

    fn analyze_crate_timings(&self, profile: &CompilationProfile) -> Result<Vec<CrateTiming>> {
        let mut timings: Vec<_> = profile.crate_timings.values().cloned().collect();
        timings.sort_by(|a, b| b.duration.partial_cmp(&a.duration).unwrap_or(std::cmp::Ordering::Equal));
        Ok(timings)
    }

    fn identify_bottlenecks(&self, timings: &[CrateTiming], total_duration: Duration) -> Vec<Bottleneck> {
        ...
        for timing in timings {
            ...
            if percentage >= 5.0 { // Only consider significant bottlenecks
                ...
                bottlenecks.push(Bottleneck {
                    ...
                    issue,
                    impact: if percentage >= 15.0 { "High" } else if percentage >= 10.0 { "Medium" } else { "Low" }.to_string(),
                    suggestion,
                });
            }
        }
    ...
    fn analyze_crate_bottleneck(&self, timing: &CrateTiming) -> (String, String) {
        if timing.crate_name.contains("derive") {
            (
                ...
                "Consider reducing derive macro usage or splitting into smaller crates".to_string(),
            )
        } else if timing.crate_name.contains("syntax") || timing.crate_name.contains("parser") {
            (
                "Complex const functions or syntax parsing".to_string(),
                "Review const function complexity or use lazy_static for expensive computations".to_string(),
            )
    ...
    fn analyze_parallelization(&self, timings: &[CrateTiming], current_jobs: usize) -> Result<ParallelizationAnalysis> {
        // Analyze which crates could be compiled in parallel
        let blocking_crates = self.find_blocking_crates(timings);

        // Estimate optimal job count
        ...
        let recommendation = if current_jobs < optimal_jobs {
            format!("Increase --jobs to {} for {:.1}x potential speedup", optimal_jobs, speedup_potential)
        } else if !blocking_crates.is_empty() {
            format!("Address blocking crates: {} to improve parallelization", blocking_crates.join(", "))
        } else {
    ...
    fn find_blocking_crates(&self, timings: &[CrateTiming]) -> Vec<String> {
        ...
        for timing in timings {
            if timing.duration > 5.0 { // Crates taking >5 seconds block parallelization
                if timing.crate_name.contains("derive") ||
    ...
    fn generate_optimization_suggestions(&self, bottlenecks: &[Bottleneck], profile: &CompilationProfile) -> Vec<OptimizationSuggestion> {
        let mut suggestions = Vec::new();

        // General suggestions based on bottlenecks
        for bottleneck in bottlenecks {
            match bottleneck.impact.as_str() {
                "High" => {
                    suggestions.push(OptimizationSuggestion {
                        category: "Crate Optimization".to_string(),
                        description: format!("Optimize {} ({}% of build time)", bottleneck.crate_name, bottleneck.percentage_of_total.round()),
                        impact: "High".to_string(),
                        implementation: bottleneck.suggestion.clone(),
                        estimated_savings: format!("{:.1}s", bottleneck.duration * 0.3),
                    });
                }
                "Medium" => {
                    suggestions.push(OptimizationSuggestion {
                        category: "Build Pipeline".to_string(),
                        description: format!("Address {} bottleneck", bottleneck.crate_name),
                        impact: "Medium".to_string(),
                        implementation: "Enable pipelined compilation".to_string(),
                        estimated_savings: format!("{:.1}s", bottleneck.duration * 0.2),
                    });
                }
                _ => {}
            }
        ...
        suggestions.push(OptimizationSuggestion {
            ...
            impact: "Medium".to_string(),
            implementation: "Install and configure sccache as Rust compiler wrapper".to_string(),
            estimated_savings: format!("{:.1}s", profile.total_duration.as_secs_f64() * 0.4),
        });
        ...
        suggestions.push(OptimizationSuggestion {
            ...
            implementation: "Use cargo build with --release only when needed".to_string(),
            estimated_savings: format!("{:.1}s", profile.total_duration.as_secs_f64() * 0.2),
        });
        ...
        suggestions.push(OptimizationSuggestion {
            ...
            impact: "Low".to_string(),
            implementation: "Add lto = true to Cargo.toml [profile.release]".to_string(),
    ...
    fn track_incremental_benefits(&self, clean_time: f64, incremental_time: f64) -> Result<IncrementalAnalysis> {
        let changed_files = 3; // Mock data
        let recompiled_units = 12; // Mock data
...
impl Tool for CompileTimeTrackerTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let threshold: f64 = matches.get_one::<String>("threshold")
            ...
            .parse()
            .unwrap_or(10.0);
        let jobs: usize = matches.get_one::<String>("jobs")
            .unwrap()
            .parse()
            .unwrap_or(4);
        ...
        if dry_run {
            println!("🔍 Would analyze compilation performance for: {}", manifest_path);
            return Ok(());
        }

        if !Path::new(manifest_path).exists() {
            return Err(ToolError::InvalidArguments(format!("Cargo.toml not found: {}", manifest_path)));
        }

        println!("⏱️  {} - {}", "Compilation Time Analysis".bold(), self.description().cyan());
        println!("📁 Project: {}", manifest_path.bold());

        // Run clean build if requested
        let clean_profile = if clean_build {
            println!("\n🏗️  Running clean build...");
            match self.run_timed_compilation(manifest_path, &["--release"]) {
                Ok(profile) => {
                    println!("✅ Clean build completed in {}", self.format_duration(profile.total_duration.as_secs_f64()).green());
                    Some(profile)
                }
                Err(e) => {
                    if verbose {
        ...
        let incremental_profile = if incremental {
            ...
            match self.run_timed_compilation(manifest_path, &[]) {
                Ok(profile) => {
                    println!("✅ Incremental build completed in {}", self.format_duration(profile.total_duration.as_secs_f64()).green());
                    Some(profile)
                }
                Err(e) => {
                    if verbose {
        ...
        // Use the first available profile for analysis
        let profile = clean_profile.as_ref()
            .or(incremental_profile.as_ref())
            .ok_or_else(|| ToolError::ExecutionFailed("No build profile available".to_string()))?;

        match output_format {
            OutputFormat::Human => {
                println!("\n📊 Build Performance:");
                println!("• Total build time: {}", self.format_duration(profile.total_duration.as_secs_f64()).bold());

                if let (Some(clean), Some(incr)) = (&clean_profile, &incremental_profile) {
                    ...
                    if clean_time > 0.0 {
                        ...
                        println!("• Clean build: {}", self.format_duration(clean_time));
                        println!("• Incremental build: {}", self.format_duration(incr_time));
                        println!("• Speedup: {:.1}x", speedup);
                    }
                }

                println!("• Peak memory usage: {}", self.format_memory(profile.peak_memory_usage));
                println!("• CPU utilization: {:.1}%", profile.cpu_utilization);

                // Analyze crate timings
                if verbose_timing || bottlenecks {
                    match self.analyze_crate_timings(profile) {
                        Ok(timings) => {
                            ...
                            if bottlenecks {
                                let bottlenecks = self.identify_bottlenecks(&timings, profile.total_duration);
                ...
                if parallel {
                    match self.analyze_crate_timings(profile) {
                        Ok(timings) => {
                            match self.analyze_parallelization(&timings, jobs) {
                                Ok(analysis) => {
                                    ...
                                    println!("• Speedup potential: {:.1}x", analysis.speedup_potential);
                                    if !analysis.blocking_crates.is_empty() {
                                        println!("• Blocking crates: {}", analysis.blocking_crates.join(", ").yellow());
                                    }
                                    println!("• Recommendation: {}", analysis.recommendation.cyan());
                                }
                                Err(e) => {
                                    if verbose {
                        ...
                        Err(e) => {
                            if verbose {
                                println!("⚠️  Could not analyze parallelization: {}", e);
                            }
                        }
                    }
                }
                ...
                if let (Some(clean), Some(incr)) = (&clean_profile, &incremental_profile) {
                    match self.track_incremental_benefits(
                        ...
                        Ok(analysis) => {
                            ...
                            println!("• Recompiled units: {}", analysis.recompiled_units);
                ...
                if optimize {
                    match self.analyze_crate_timings(profile) {
                        Ok(timings) => {
                            let bottlenecks = self.identify_bottlenecks(&timings, profile.total_duration);
                            let suggestions = self.generate_optimization_suggestions(&bottlenecks, profile);
                            ...
                            if !suggestions.is_empty() {
                                ...
                                for (i, suggestion) in suggestions.iter().enumerate() {
                                    ...
                                    println!("   Implementation: {}", suggestion.implementation.cyan());
                                    println!("   Estimated savings: {}", suggestion.estimated_savings.green());
            ...
            OutputFormat::Json => {
                let mut json_output = serde_json::json!({
                    ...
                    "total_build_time_seconds": profile.total_duration.as_secs_f64(),
                    "peak_memory_bytes": profile.peak_memory_usage,
                    "cpu_utilization_percent": profile.cpu_utilization,
                });

                if let Ok(timings) = self.analyze_crate_timings(profile) {
                    json_output["crate_timings"] = serde_json::to_value(&timings).unwrap();
                }
            ...
            OutputFormat::Table => {
                ...
                println!("│ Build Time: {:<39} │", self.format_duration(profile.total_duration.as_secs_f64()));
                println!("│ Memory Usage: {:<37} │", self.format_memory(profile.peak_memory_usage));
                println!("│ CPU Usage: {:<40} │", format!("{:.1}%", profile.cpu_utilization));
                if bottlenecks {
                    match self.analyze_crate_timings(profile) {
                        Ok(timings) => {
                            let bottleneck_count = self.identify_bottlenecks(&timings, profile.total_duration).len();
