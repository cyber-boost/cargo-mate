...
impl MigrationGenTool {
    ...
    fn parse_struct_definition(&self, struct_def: &ItemStruct, file_path: &str, line_number: usize) -> Result<StructDefinition> {
    ...
    fn parse_field_definition(&self, field: &Field) -> Result<FieldDefinition> {
        ...
        let comment = field.attrs.iter()
            .find(|attr| attr.path().segments.last().unwrap().ident == "doc")
            .and_then(|attr| {
                if let Ok(syn::Meta::NameValue(meta)) = attr.parse_args::<syn::Meta>() {
                    // In syn 2.0, handle the value field differently
                    Some("doc_comment_placeholder".to_string()) // Placeholder for doc comment
                } else {
                    None
                }
            });
            ...
            Type::Path(type_path) => {
                if let Some(segment) = type_path.path.segments.last() {
                    match segment.ident.to_string().as_str() {
                        ...
                        "Vec" => Ok("JSON".to_string()), // For arrays
    ...
    fn generate_migration_sql(&self, struct_def: &StructDefinition) -> MigrationPlan {
        ...
        let description = format!("Create {} table for {} struct", table_name, struct_def.name);
        let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S").to_string();

        let changes = vec![format!("Create table with {} columns", struct_def.fields.len())];
    ...
    fn generate_migration_file(&self, plan: &MigrationPlan, output_dir: &str) -> Result<String> {
    ...
    fn find_existing_migration(&self, table_name: &str, migrations_dir: &str) -> Option<String> {
    ...
    fn compare_struct_with_existing(&self, new_struct: &StructDefinition, existing_migration: &str) -> Result<Vec<String>> {
        let existing_content = fs::read_to_string(existing_migration)?;
        let mut changes = Vec::new();

        // Parse existing migration to extract column definitions
        let existing_columns = self.extract_columns_from_sql(&existing_content)?;

        // Compare with new struct fields
        for field in &new_struct.fields {
            let column_name = self.field_name_to_column_name(&field.name);
            if let Some(existing_col) = existing_columns.get(&column_name) {
                if existing_col != &field.ty {
                    changes.push(format!("ALTER COLUMN {} TYPE {} -> {}", column_name, existing_col, field.ty));
                }
            } else {
    ...
    fn extract_columns_from_sql(&self, sql_content: &str) -> Result<HashMap<String, String>> {
        let mut columns = HashMap::new();

        for line in sql_content.lines() {
            let line = line.trim();
            if line.starts_with("CREATE TABLE") || line.is_empty() || line.starts_with("--") {
                continue;
            }
            if line.starts_with(");") {
                break;
    ...
    fn generate_alter_migration(&self, struct_def: &StructDefinition, changes: &[String], timestamp: &str) -> MigrationPlan {
        let table_name = self.struct_name_to_table_name(&struct_def.name);
        ...
        for change in changes {
            if change.starts_with("ADD COLUMN") {
                ...
                // For down migration, we'd need to drop the column
                down_sql.push_str(&format!("-- Would need to drop column for full revert\n"));
                ...
                down_sql.push_str(&format!("-- Revert {} (manual review required)\n", change));
            }
        }

        let description = format!("Alter {} table - {} changes", table_name, changes.len());
    ...
    fn display_report(&self, report: &MigrationReport, output_format: OutputFormat, verbose: bool) {
        match output_format {
            OutputFormat::Human => {
                println!("\n🗃️  {} - SQL Migration Generation Report", "CargoMate MigrationGen".bold().blue());
                ...
                if verbose {
                    ...
                    for plan in &report.migration_plan {
                        ...
                        if plan.up_sql.lines().count() > 5 {
                            println!("      ... ({} more lines)", plan.up_sql.lines().count() - 5);
                        }
                    }
                }
            ...
            OutputFormat::Table => {
                println!("{:<25} {:<20} {:<15} {:<10}",
                        "Table", "Description", "Changes", "Timestamp");
                println!("{}", "─".repeat(75));

                for plan in &report.migration_plan {
                    println!("{:<25} {:<20} {:<15} {:<10}",
                            plan.table_name,
                            plan.description.chars().take(18).collect::<String>(),
                            plan.changes.len().to_string(),
                            plan.timestamp.chars().take(8).collect::<String>());
                }
            }
        }
    }
...
impl Tool for MigrationGenTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Generate SQL migration scripts from Rust struct definitions. \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🗃️  {} - Generating SQL Migrations", "CargoMate MigrationGen".bold().blue());
        ...
        if structs.is_empty() {
            return Err(ToolError::ExecutionFailed("No struct definitions found in input file".to_string()));
        }

        if verbose {
            println!("\n📋 Found {} struct(s):", structs.len());
            for struct_def in &structs {
                println!("  • {} - {} fields", struct_def.name.green(), struct_def.fields.len());
            }
        }
        ...
        for struct_def in &structs {
            if diff {
                ...
                let table_name = self.struct_name_to_table_name(&struct_def.name);
                if let Some(existing_migration) = self.find_existing_migration(&table_name, migrations_dir) {
                    if let Ok(changes) = self.compare_struct_with_existing(struct_def, &existing_migration) {
                        if !changes.is_empty() {
                            let timestamp = chrono::Utc::now().format("%Y%m%d_%H%M%S").to_string();
                            let plan = self.generate_alter_migration(struct_def, &changes, &timestamp);
                            migration_plans.push(plan);
                        }
                    }
                } else {
                    // No existing migration, create new table
                    let plan = self.generate_migration_sql(struct_def);
                    migration_plans.push(plan);
                }
            } else {
                // Generate new table migration
                let plan = self.generate_migration_sql(struct_def);
                migration_plans.push(plan);
            }
        }
        ...
        for plan in &migration_plans {
            if !dry_run {
                match self.generate_migration_file(plan, output_dir) {
                    ...
                    Err(e) => {
                        println!("❌ Failed to generate migration for {}: {}", plan.table_name, e);
                    }
                }
            }
        }
        ...
        let report = MigrationReport {
            ...
            tables_created: migration_plans.iter().filter(|p| p.up_sql.contains("CREATE TABLE")).count(),
