...
impl VendorizeTool {
    ...
    fn analyze_dependencies(&self, manifest_path: &str) -> Result<DependencyAnalysis> {
        let output = ProcessCommand::new("cargo")
            ...
            .output()
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to run cargo tree: {}", e)))?;

        if !output.status.success() {
            return Err(ToolError::ExecutionFailed("Cargo tree command failed".to_string()));
        }
        ...
        let summary = AnalysisSummary {
            ...
            unmaintained: dependencies.iter().filter(|d| d.maintenance_status == "unmaintained").count(),
    ...
    fn parse_dependency_line(&self, line: &str) -> Option<DependencyInfo> {
        ...
        // Skip tree structure characters and extract crate info
        ...
        // Determine if direct dependency based on indentation
        let indentation = line.len() - line.trim_start().len();
        let is_direct = indentation == 0;

        // Simulate additional analysis (in real implementation, this would query registries)
        ...
        let size_estimate = 1024 * 500; // 500KB estimate
    ...
    fn select_vendorable_crates(&self, analysis: &DependencyAnalysis, criteria: &[String]) -> Vec<DependencyInfo> {
        analysis.dependencies.iter().filter(|dep| {
            criteria.iter().any(|criterion| match criterion.as_str() {
                "unmaintained" => dep.maintenance_status == "unmaintained",
                "security-risk" => !dep.security_issues.is_empty(),
                "offline" => ["network-lib", "http-client"].contains(&dep.name.as_str()),
                "custom" => false, // Would be configurable
                _ => false,
            })
        }).cloned().collect()
    }

    fn vendor_crate(&self, crate_info: &DependencyInfo, output_dir: &str, minimal: bool) -> Result<VendoringResult> {
...
"#, crate_info.name, crate_info.version);
        ...
        let lib_rs_content = format!(r#"//! Vendored version of {} v{}
...
"#, crate_info.name, crate_info.version, crate_info.version, crate_info.name);
        ...
        // Add additional files based on minimal flag
        if !minimal {
            // Copy README if it exists (simulated)
            files_copied.push("README.md".to_string());

            // Copy LICENSE if it exists (simulated)
            if let Some(license) = &crate_info.license {
                files_copied.push(format!("LICENSE-{}", license));
            }

            // Copy additional source files (simulated)
            files_copied.extend(vec![
        ...
        Ok(VendoringResult {
            ...
            size_copied: files_copied.len() as u64 * 1024, // Rough estimate
    ...
    fn check_licenses(&self, vendored_crates: &[DependencyInfo]) -> Result<LicenseReport> {
        ...
        let compatible = if has_gpl && has_closed {
            false
        } else {
            true
        };
        ...
        if has_gpl && has_closed {
            issues.push("Mix of GPL and proprietary licenses detected - may not be compatible".to_string());
        }
    ...
    fn scan_security(&self, vendored_crates: &[DependencyInfo]) -> Result<SecurityReport> {
        let mut issues = Vec::new();
    ...
    fn update_vendored(&self, vendor_dir: &str) -> Result<UpdateReport> {
        ...
        // Scan vendor directory for existing crates
                ...
                if entry.path().is_dir() {
                    if let Some(crate_name) = entry.file_name().to_str() {
                        ...
                        if crate_name.contains("old") {
                            ...
                            changelog.push(format!("Updated {} to latest version", crate_name));
                        } else {
                            failed_updates.push(format!("{} already up to date", crate_name));
                        }
                    }
                }
            }
    ...
    fn display_analysis(&self, analysis: &DependencyAnalysis, output_format: OutputFormat, verbose: bool) {
        match output_format {
            OutputFormat::Human => {
                ...
                println!("  • Indirect dependencies: {}", analysis.summary.indirect_deps);
                println!("  • Unmaintained: {}", analysis.summary.unmaintained);
                println!("  • Security risks: {}", analysis.summary.security_risks);
                println!("  • Total size: {:.1} MB", analysis.summary.total_size as f64 / (1024.0 * 1024.0));

                if verbose {
                    println!("\n📋 Dependencies by category:");

                    let unmaintained: Vec<_> = analysis.dependencies.iter()
                    ...
                    if !unmaintained.is_empty() {
                        ...
                        for dep in unmaintained {
                            println!("    • {} v{} - {}", dep.name.yellow(), dep.version, dep.maintenance_status);
                        }
                    }

                    let security_risks: Vec<_> = analysis.dependencies.iter()
                    ...
                    if !security_risks.is_empty() {
                        ...
                        for dep in security_risks {
                            println!("    • {} v{} - {} issues", dep.name.red(), dep.version, dep.security_issues.len());
                        }
                    }
                }
            }
            ...
            OutputFormat::Table => {
                println!("{:<25} {:<12} {:<15} {:<10} {:<12}",
                        "Crate", "Version", "License", "Direct", "Status");
                println!("{}", "─".repeat(80));
                ...
                for dep in &analysis.dependencies {
                    println!("{:<25} {:<12} {:<15} {:<10} {:<12}",
                            dep.name,
                            dep.version,
                            dep.license.as_ref().unwrap_or(&"Unknown".to_string()),
                            if dep.is_direct { "Yes" } else { "No" },
                            dep.maintenance_status);
                }
            }
        }
    }

    fn display_vendoring_results(&self, results: &[VendoringResult], license_report: &LicenseReport, security_report: &SecurityReport) {
        ...
        for result in results {
            if result.success {
                println!("  • {} - {} files ({:.1} KB)",
                        result.crate_name.green(),
                        result.files_copied.len(),
                        result.size_copied as f64 / 1024.0);
            } else {
                println!("  • {} - {}", result.crate_name.red(),
...
impl Tool for VendorizeTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Smart dependency vendoring with automatic updates, license tracking, and security monitoring.
...
EXAMPLES:
    ...
    cm tool vendorize --minimal --dry-run")
            .args(&[
                ...
                Arg::new("criteria")
                    .long("criteria")
                    .short('c')
                    .help("Vendoring criteria: unmaintained, security-risk, offline, custom")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let criteria: Vec<String> = matches.get_one::<String>("criteria")
            .unwrap()
            .split(',')
        ...
        // Handle update mode
        if update {
            println!("🔄 {} - Updating vendored dependencies", "CargoMate Vendorize".bold().blue());
        ...
        println!("🔍 {} - Analyzing dependencies", "CargoMate Vendorize".bold().blue());
        ...
        if dry_run {
            ...
            for crate_info in &vendorable_crates {
                println!("  • {} v{} - {}", crate_info.name, crate_info.version,
                        crate_info.maintenance_status);
            }
            return Ok(());
        }
