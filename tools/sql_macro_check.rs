...
impl SqlMacroCheckTool {
                ...
                if let Ok(regex) = Regex::new(pattern) {
                    if let Some(captures) = regex.captures(line) {
                        if let Some(sql_match) = captures.get(1) {
                            ...
                            let analysis = self.analyze_sql_query(
                                file_path.to_string(),
                                macro_name,
                                sql_query,
                                line_num + 1,
    ...
    fn analyze_sql_query(&self, file_path: String, macro_name: String, sql_query: String, line_number: usize) -> SqlMacroAnalysis {
    ...
    fn extract_parameters(&self, sql_query: &str) -> Vec<String> {
        ...
        let param_patterns = vec![
            r"\$\d+",      // PostgreSQL style: $1, $2, etc.
            r"\?",         // ODBC style: ?
            r":\w+",       // Named parameters: :param
            r"#\{\w+\}",   // Ruby-style: #{param}
            r"%s|%d|%f",   // sprintf style
        ];
    ...
    fn has_sql_injection_risks(&self, sql_query: &str) -> bool {
        ...
        sql_query.to_lowercase().contains("concat") ||
        sql_query.contains("' + ") || sql_query.contains(" + '")
    }

    fn has_unparameterized_queries(&self, sql_query: &str, parameters: &[String]) -> bool {
        // Check if there are string literals that should be parameters
        let string_literals = Regex::new(r"'[^']*'").unwrap();
        let string_count = string_literals.captures_iter(sql_query).count();

        // If there are many string literals and few parameters, it might be unparameterized
        string_count > parameters.len() + 2
    }

    fn uses_deprecated_features(&self, sql_query: &str) -> bool {
        let deprecated_features = vec![
            "mysql_",
            "old_",
            "deprecated_",
            "type=",
        ...
        deprecated_features.iter().any(|feature| sql_query.to_lowercase().contains(feature))
    }
    ...
    fn has_missing_indexes(&self, sql_query: &str) -> bool {
        // Simple heuristic: queries with WHERE clauses on non-indexed patterns
        ...
        if let Some(captures) = where_clause.captures(sql_query) {
            if let Some(where_part) = captures.get(1) {
                ...
                // Check for patterns that might benefit from indexes
    ...
    fn has_cartesian_product(&self, sql_query: &str) -> bool {
        // Check for JOINs without ON clauses or multiple tables without JOINs
        let join_without_on = Regex::new(r"JOIN\s+\w+\s*(?:WHERE|ORDER|GROUP|HAVING|$)").unwrap();
    ...
    fn check_sql_syntax(&self, sql_query: &str) -> Vec<SyntaxError> {
        ...
        if paren_count != 0 {
            errors.push(SyntaxError {
                ...
                description: format!("Unmatched parentheses: {} open, {} close", sql_query.chars().filter(|&c| c == '(').count(), sql_query.chars().filter(|&c| c == ')').count()),
                sql_snippet: sql_query.to_string(),
            });
        }

        // Check for missing semicolons
        if !sql_query.trim().ends_with(';') && !sql_query.to_lowercase().contains("select") {
            errors.push(SyntaxError {
                file_path: "unknown".to_string(),
                line_number: 0,
    ...
    fn generate_suggestions(&self, analyses: &[SqlMacroAnalysis]) -> Vec<String> {
        ...
        if has_security_issues {
            suggestions.push("Use parameterized queries to prevent SQL injection".to_string());
            suggestions.push("Avoid string concatenation in SQL queries".to_string());
            suggestions.push("Use prepared statements with proper parameter binding".to_string());
        }

        if has_performance_issues {
            suggestions.push("Specify columns explicitly instead of using SELECT *".to_string());
            suggestions.push("Add appropriate indexes for frequently queried columns".to_string());
            suggestions.push("Avoid Cartesian products by using proper JOIN conditions".to_string());
            suggestions.push("Consider query optimization and EXPLAIN plans".to_string());
        }

        // General suggestions
        suggestions.push("Use database migrations for schema changes".to_string());
        suggestions.push("Implement proper error handling for database operations".to_string());
        suggestions.push("Add database connection pooling for better performance".to_string());
        suggestions.push("Use transactions for multi-statement operations".to_string());

        suggestions
    }

    fn display_report(&self, report: &SqlAnalysisReport, output_format: OutputFormat, verbose: bool) {
        match output_format {
            OutputFormat::Human => {
                println!("\n🔍 {} - SQL Macro Analysis Report", "CargoMate SqlMacroCheck".bold().blue());
                ...
                println!("  • Performance Issues: {}", report.performance_issues.len());
                ...
                if !report.macros_analyzed.is_empty() && verbose {
                    ...
                    for analysis in &report.macros_analyzed {
                        let security_icon = if analysis.security_score >= 80 { "🛡️" } else { "⚠️" };
                        let performance_icon = if analysis.performance_score >= 80 { "🚀" } else { "🐌" };

                        println!("  {} {} - {} (Security: {}, Performance: {})",
                                analysis.macro_name,
                                analysis.file_path.split('/').last().unwrap_or(&analysis.file_path),
                                format!("{}:{}", analysis.line_number, analysis.sql_query.chars().take(50).collect::<String>()),
                ...
                if !report.security_issues.is_empty() {
                    ...
                    for issue in &report.security_issues {
                        ...
                        println!("  {} {}:{} - {}", severity_icon, issue.file_path.split('/').last().unwrap_or(&issue.file_path), issue.line_number, issue.description);
                        if verbose {
                ...
                if !report.performance_issues.is_empty() {
                    ...
                    for issue in &report.performance_issues {
                        println!("  🐌 {}:{} - {}", issue.file_path.split('/').last().unwrap_or(&issue.file_path), issue.line_number, issue.description);
                        if verbose {
                            println!("    SQL: {}", issue.sql_snippet.dimmed());
                            println!("    💡 {}", issue.suggestion);
                        }
                ...
                if !report.syntax_errors.is_empty() {
                    ...
                    for error in &report.syntax_errors {
                        println!("  🔴 {}:{} - {}", error.file_path.split('/').last().unwrap_or(&error.file_path), error.line_number, error.description);
                        if verbose {
                ...
                println!("\n✅ Analysis complete!");
                let total_issues = report.security_issues.len() + report.performance_issues.len() + report.syntax_errors.len();
                if total_issues == 0 {
            ...
            OutputFormat::Table => {
                println!("{:<30} {:<15} {:<15} {:<15} {:<10}",
                        "File", "Queries", "Security", "Performance", "Syntax");
                println!("{}", "─".repeat(90));

                // Group by file
                let mut file_stats = HashMap::new();
                for analysis in &report.macros_analyzed {
                    let entry = file_stats.entry(&analysis.file_path).or_insert((0, 0, 0, 0));
                    entry.0 += 1; // queries
                }

                for issue in &report.security_issues {
                    if let Some(entry) = file_stats.get_mut(&issue.file_path) {
                        entry.1 += 1; // security issues
                    }
                }

                for issue in &report.performance_issues {
                    if let Some(entry) = file_stats.get_mut(&issue.file_path) {
                        entry.2 += 1; // performance issues
                    }
                }

                for error in &report.syntax_errors {
                    if let Some(entry) = file_stats.get_mut(&error.file_path) {
                        entry.3 += 1; // syntax errors
                    }
                }
...
impl Tool for SqlMacroCheckTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Analyze SQL queries in Rust code for security vulnerabilities, \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🔍 {} - Analyzing SQL Macros", "CargoMate SqlMacroCheck".bold().blue());
            ...
            match self.analyze_sql_macros(file_path) {
                Ok(analyses) => {
                    for analysis in analyses {
                        ...
                        // Convert analysis issues to specific issue types
                        for issue in &analysis.issues {
                            if issue.contains("injection") || issue.contains("parameterized") || issue.contains("deprecated") {
                                security_issues.push(SecurityIssue {
                                    ...
                                    severity: if issue.contains("injection") { "high".to_string() } else { "medium".to_string() },
        ...
        let final_analyses = if security_only || performance_only || syntax_only {
            all_analyses.into_iter().filter(|analysis| {
                if security_only {
                    analysis.security_score < 100
                } else if performance_only {
                    analysis.performance_score < 100
                } else {
                    false // syntax_only - we'll handle this separately
                }
            }).collect()
        } else {
            all_analyses
        ...
        let final_performance_issues = if performance_only || !security_only && !syntax_only {
            performance_issues
        } else {
            Vec::new()
        };
...
impl SqlMacroCheckTool {
    fn generate_security_suggestion(&self, issue: &str) -> String {
        if issue.contains("injection") {
            "Use parameterized queries with bound parameters instead of string concatenation".to_string()
        } else if issue.contains("parameterized") {
            "Replace string literals with parameter placeholders (e.g., $1, ?, :param)".to_string()
        } else if issue.contains("deprecated") {
            "Update to use current SQL features and avoid deprecated functions".to_string()
        } else {
    ...
    fn generate_performance_suggestion(&self, issue: &str) -> String {
        if issue.contains("SELECT *") {
            "Specify column names explicitly to reduce data transfer and improve query performance".to_string()
