...
impl ProtoBindTool {
    ...
    fn parse_proto_message(&self, first_line: &str, lines: &mut std::iter::Peekable<std::str::Lines>) -> Option<ProtoMessage> {
        let name = first_line
    ...
    fn parse_proto_field(&self, line: &str) -> Option<ProtoField> {
        ...
        // Parse field number from something like "name = 1;"
        let number_part = if repeated { parts[3] } else { parts[2] };
        let number_str = number_part.strip_prefix("=").unwrap_or("").trim_end_matches(";");
    ...
    fn parse_proto_service(&self, first_line: &str, lines: &mut std::iter::Peekable<std::str::Lines>) -> Option<ProtoService> {
        let name = first_line
    ...
    fn parse_proto_method(&self, line: &str) -> Option<ProtoMethod> {
        ...
        let output_type = output_part.trim_start_matches('(').trim_end_matches(");").trim().to_string();
    ...
    fn parse_openapi_spec(&self, file_path: &str) -> Result<OpenAPISchema> {
        ...
        let value: Value = if ext == "yaml" || ext == "yml" {
            serde_yaml::from_str(&content)
                .map_err(|e| ToolError::ExecutionFailed(format!("Failed to parse YAML: {}", e)))?
        } else {
                ...
                if let Some(schemas_map) = schemas_obj.as_object() {
                    for (name, schema) in schemas_map {
                        if let Some(schema_obj) = schema.as_object() {
                            ...
                            schemas.push(OpenAPISchemaItem {
                                name: name.clone(),
                                properties,
                            });
                        }
                    ...
                    if let Some(path_obj) = path_item.as_object() {
                        for (method, operation) in path_obj {
                            if let Some(op_obj) = operation.as_object() {
                                ...
                                let request_body = self.extract_openapi_request_body(op_obj);
                                let response_body = self.extract_openapi_response_body(op_obj);
    ...
    fn extract_openapi_request_body(&self, operation: &Map<String, Value>) -> Option<String> {
        operation.get("requestBody")
            ...
            .map(|ref_str| ref_str.strip_prefix("#/components/schemas/").unwrap_or(ref_str).to_string())
    }

    fn extract_openapi_response_body(&self, operation: &Map<String, Value>) -> Option<String> {
    ...
    fn parse_graphql_schema(&self, file_path: &str) -> Result<GraphQLSchema> {
        ...
        while let Some(line) = lines.next() {
            ...
            if line.starts_with("type ") && !line.contains("Query") && !line.contains("Mutation") {
    ...
    fn parse_graphql_type(&self, first_line: &str, lines: &mut std::iter::Peekable<std::str::Lines>) -> Option<GraphQLType> {
        let name = first_line
            .strip_prefix("type ")?
            .trim_end_matches(" {")
            .trim()
    ...
    fn parse_graphql_fields(&self, lines: &mut std::iter::Peekable<std::str::Lines>) -> Vec<GraphQLField> {
        let mut fields = Vec::new();
    ...
    fn generate_rust_bindings(&self, schema: &SchemaType, format: &str) -> Result<String> {
        match schema {
            SchemaType::Proto(proto) => self.generate_proto_rust_bindings(proto, format),
            SchemaType::OpenAPI(openapi) => self.generate_openapi_rust_bindings(openapi, format),
            SchemaType::GraphQL(graphql) => self.generate_graphql_rust_bindings(graphql, format),
        }
    }

    fn generate_proto_rust_bindings(&self, schema: &ProtoSchema, format: &str) -> Result<String> {
        let mut code = format!("// Generated Rust bindings from Protocol Buffer schema\n\n");
        ...
        for service in &schema.services {
            ...
            for method in &service.methods {
                ...
                code.push_str(&format!("    ) -> Result<{}, tonic::Status>;\n", method.output_type));
            }
    ...
    fn generate_openapi_rust_bindings(&self, schema: &OpenAPISchema, format: &str) -> Result<String> {
        ...
        for path in &schema.paths {
            ...
            let response_type = path.response_body.as_ref().map(|s| s.as_str()).unwrap_or("String");
            code.push_str(&format!(") -> Result<{}, reqwest::Error> {{\n", response_type));

            code.push_str(&format!("        let url = format!(\"{{}}{{}}\", self.base_url, \"{}\");\n", path.path));

            if let Some(req_body) = &path.request_body {
                code.push_str(&format!("        let response = self.client.{}(url)\n", http_method));
            ...
            if let Some(resp_body) = &path.response_body {
                code.push_str(&format!("        let result = response.json::<{}>().await?;\n", resp_body));
    ...
    fn generate_graphql_rust_bindings(&self, schema: &GraphQLSchema, format: &str) -> Result<String> {
        ...
        for query in &schema.queries {
            ...
            if format == "serde" {
                ...
                code.push_str(&format!("    query_path = \"{}.graphql\",\n", query.name.to_lowercase()));
    ...
    fn generate_grpc_client(&self, schema: &ProtoSchema) -> Result<String> {
        ...
        for service in &schema.services {
            ...
            code.push_str(&format!("impl {} {{\n", client_name));
            code.push_str("    pub async fn connect<D>(dst: D) -> Result<Self, tonic::transport::Error>\n");
            code.push_str("    where\n");
            code.push_str("        D: std::convert::TryInto<tonic::transport::Endpoint>,\n");
            code.push_str("        D::Error: Into<Box<dyn std::error::Error + Send + Sync>>,\n");
            code.push_str("    {\n");
            code.push_str("        let conn = tonic::transport::Endpoint::new(dst)?.connect().await?;\n");
            ...
            for method in &service.methods {
                ...
                code.push_str(&format!("        request: {},\n", method.input_type));
                code.push_str(&format!("    ) -> Result<{}, Status> {{\n", method.output_type));
    ...
    fn proto_type_to_rust(&self, proto_type: &str, repeated: bool) -> String {
        let base_type = match proto_type {
            ...
            "double" => "f64",
            "bytes" => "Vec<u8>",
            _ => proto_type, // Custom message type
        };
    ...
    fn get_default_value(&self, proto_type: &str, repeated: bool) -> String {
        if repeated {
            ...
            match proto_type {
                ...
                "int32" | "int64" | "uint32" | "uint64" | "sint32" | "sint64" | "fixed32" | "fixed64" | "sfixed32" | "sfixed64" => "0".to_string(),
                "bool" => "false".to_string(),
                "float" | "double" => "0.0".to_string(),
                "bytes" => "Vec::new()".to_string(),
                _ => format!("{}::new()", proto_type), // Custom message type
            }
        }
    }
}
...
impl Tool for ProtoBindTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Auto-generate Rust bindings from various schema formats including Protocol Buffers (.proto), OpenAPI/Swagger specs (JSON/YAML), and GraphQL schemas (.graphql). Supports Serde derives and gRPC client generation.")
            .args(&[
                Arg::new("input")
                    .long("input")
                    .short('i')
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🔧 {} - {}", "CargoMate ProtoBind".bold().blue(), self.description().cyan());
        ...
        let schema = match format.as_str() {
            "proto" => {
                ...
                if verbose {
                    println!("   📄 Found {} messages and {} services", proto_schema.messages.len(), proto_schema.services.len());
                }
            ...
            "openapi" => {
                ...
                if verbose {
                    println!("   📄 Found {} schemas and {} paths", openapi_schema.components.schemas.len(), openapi_schema.paths.len());
                }
            ...
            "graphql" => {
                ...
                if verbose {
                    println!("   📄 Found {} types, {} queries, and {} mutations",
                        graphql_schema.types.len(),
                        graphql_schema.queries.len(),
                        graphql_schema.mutations.len());
                }
                SchemaType::GraphQL(graphql_schema)
            }
            _ => return Err(ToolError::InvalidArguments(format!("Unsupported format: {}", format))),
        };
        ...
        // Generate gRPC client if requested and format is proto
        ...
        match output_format {
            ...
            OutputFormat::Json => {
                let result = serde_json::json!({
                    ...
                    "input": input,
                    "output": output,
                    "grpc_generated": grpc && !grpc_code.is_empty(),
                    "serde_enabled": serde,
                    "code_preview": rust_code.lines().take(10).collect::<Vec<_>>().join("\n")
                });
                println!("{}", serde_json::to_string_pretty(&result).unwrap());
            }
            OutputFormat::Table => {
                println!("{:<15} {:<10} {:<8} {:<8}", "Format", "Input", "gRPC", "Serde");
                println!("{}", "─".repeat(50));
                println!("{:<15} {:<10} {:<8} {:<8}",
                    format,
