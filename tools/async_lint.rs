...
/// Blocking operation found in async context
...
impl AsyncLintTool {
    ...
    fn analyze_async_patterns(&self, file_path: &str) -> Result<Vec<AsyncIssue>> {
        ...
        if file_path.ends_with(".rs") {
            match parse_file(&content) {
                ...
                Err(e) => {
                    return Err(ToolError::ExecutionFailed(format!("Failed to parse Rust file: {}", e)));
                }
            }
        } else {
            return Err(ToolError::InvalidArguments("Only Rust (.rs) files are supported".to_string()));
        }
    ...
    fn analyze_await_patterns(&self, functions: &[syn::ItemFn]) -> Vec<AwaitIssue> {
        ...
        for function in functions {
            ...
            issues.extend(visitor.issues.into_iter().map(|mut issue| {
    ...
    fn suggest_async_improvements(&self, all_issues: &[AsyncIssue]) -> Vec<AsyncSuggestion> {
        ...
        if total_blocking > 0 {
            suggestions.push(AsyncSuggestion {
                category: "Blocking Operations".to_string(),
                description: format!("Found {} blocking operations in async contexts", total_blocking),
                impact: "High".to_string(),
                suggestion: "Replace std::fs with tokio::fs, std::thread::sleep with tokio::time::sleep".to_string(),
            });
        }
        ...
        if unnecessary_awaits > 0 {
            suggestions.push(AsyncSuggestion {
                ...
                description: format!("Found {} unnecessary await expressions", unnecessary_awaits),
                impact: "Low".to_string(),
                suggestion: "Remove unnecessary async/await for immediate values".to_string(),
            });
        }
        ...
        if nested_async > 0 {
            suggestions.push(AsyncSuggestion {
                ...
                suggestion: "Flatten nested async blocks for better readability".to_string(),
            });
        }
        ...
        if select_issues > 0 {
            suggestions.push(AsyncSuggestion {
                ...
                description: format!("Found {} potential deadlock patterns", select_issues),
                impact: "High".to_string(),
                suggestion: "Use try_select! or restructure concurrent operations".to_string(),
            });
        }
        ...
        suggestions.push(AsyncSuggestion {
            ...
            impact: "Medium".to_string(),
            suggestion: "Use JoinSet for concurrent operations, implement proper error handling".to_string(),
        });
        ...
        suggestions.push(AsyncSuggestion {
            ...
            impact: "Low".to_string(),
            suggestion: "Use async fn consistently, avoid mixing sync and async code".to_string(),
        });

        suggestions
    }

    fn analyze_concurrency_patterns(&self, file_path: &str) -> Result<ConcurrencyAnalysis> {
        let content = fs::read_to_string(file_path)?;

        let async_fn_count = content.matches("async fn").count();
        let await_count = content.matches("await").count();
        let select_count = content.matches("select!").count() + content.matches("join!").count();
        ...
        // Simple heuristics for race conditions
        ...
        Ok(ConcurrencyAnalysis {
            ...
            blocking_operations: content.matches("std::fs::").count() + content.matches("std::thread::").count(),
...
impl<'ast> Visit<'ast> for AsyncVisitor {
    fn visit_item_fn(&mut self, node: &'ast syn::ItemFn) {
        ...
        if is_async {
            ...
            self.issues.extend(
                await_visitor.issues.into_iter().map(|mut issue| {
            ...
            self.issues.extend(
                deadlock_visitor.risks.into_iter().map(|mut risk| {
        ...
        if let Some(func_name) = self.extract_function_name(node) {
            ...
            for (blocking, async_version) in &blocking_patterns {
                if func_name.contains(blocking) {
                    let line = 0; // Line info not available in syn 2.0
                    let col = 0;  // Column info not available in syn 2.0

                    self.operations.push(BlockingOperation {
                        function: String::new(), // Will be filled by parent visitor
                        line,
                        column: col,
...
impl<'ast> Visit<'ast> for AwaitPatternVisitor {
    fn visit_expr_await(&mut self, node: &'ast syn::ExprAwait) {
        // Check for unnecessary await on immediate values
        ...
        if let Some(func_name) = self.extract_function_name(node) {
            if func_name == "async" {
                ...
                self.issues.push(AwaitIssue {
                    ...
                    code_snippet: "async { async { work() }.await }.await".to_string(),
        ...
        if let Some(func_name) = self.extract_function_name(node) {
            if func_name.contains("select!") {
                ...
                self.risks.push(DeadlockRisk {
                    ...
                    description: "Multiple futures competing for same resource".to_string(),
                    suggestion: "Use try_select! or restructure to avoid resource contention".to_string(),
                });
            }
        }
...
impl Tool for AsyncLintTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let ignored_rules: Vec<String> = ignore_rules.split(',')
        ...
        if Path::new(input).is_file() {
            ...
            return Err(ToolError::InvalidArguments(format!("Path not found: {}", input)));
        }

        match output_format {
            OutputFormat::Human => {
                println!("⚡ {} - {}", "Async Pattern Analysis".bold(), self.description().cyan());
                ...
                for issue_set in &all_issues {
                    if !issue_set.issues.is_empty() {
                        ...
                        for issue in &issue_set.issues {
                            match issue {
                                AsyncIssueType::BlockingOperation(op) => {
                                    if detect_blocking && !ignored_rules.contains(&"blocking".to_string()) {
                                        blocking_count += 1;
                                        println!("  🚫 Line {}: {} in async function",
                                            op.line.to_string().red(),
                                            op.operation.yellow()
                                        );
                                ...
                                AsyncIssueType::AwaitIssue(issue) => {
                                    if analyze_await && !ignored_rules.contains(&"await".to_string()) {
                                        await_count += 1;
                                        println!("  🔄 Line {}: {}", issue.line.to_string().yellow(), issue.issue);
                                        println!("     Code: {}", issue.code_snippet.red());
                                        println!("     💡 {}", issue.suggestion.cyan());
                                    }
                                }
                                AsyncIssueType::DeadlockRisk(risk) => {
                                    if detect_deadlock && !ignored_rules.contains(&"deadlock".to_string()) {
                                        deadlock_count += 1;
                                        println!("  🔒 Line {}: {}", risk.line.to_string().red(), risk.risk_type);
                        ...
                        if blocking_count + await_count + deadlock_count > 0 {
                            println!("  📊 Issues in this file: {} blocking, {} await, {} deadlock",
                    ...
                    for issue_set in &all_issues {
                        match self.analyze_concurrency_patterns(&issue_set.file) {
                            Ok(analysis) => {
                                ...
                                println!("    Async functions: {}", analysis.total_async_functions);
                                println!("    Average await depth: {:.1}", analysis.average_await_depth);
                                println!("    Concurrent operations: {}", analysis.concurrent_operations);
                                ...
                                if analysis.blocking_operations > 0 {
                                    println!("    Blocking operations: {}", analysis.blocking_operations.to_string().red());
                                }
                            }
                            Err(e) => {
                                if verbose {
                ...
                println!("\n📈 Summary: {} files analyzed, {} issues found",
                    all_issues.len(), total_issues);
            }
            OutputFormat::Json => {
                let mut json_output = serde_json::json!({
                    "files_analyzed": all_issues.len(),
                    "issues": all_issues,
                });
            ...
            OutputFormat::Table => {
                println!("┌─ Async Pattern Analysis ─────────────────┐");
                println!("│ Files analyzed: {:<25} │", all_issues.len());
                let total_issues: usize = all_issues.iter().map(|i| i.issues.len()).sum();
