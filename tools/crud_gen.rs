...
impl CrudGenTool {
    ...
    fn parse_field(&self, field: &Field) -> Option<FieldInfo> {
        ...
        // Check if field is marked as primary key (by name convention or attributes)
    ...
    fn generate_crud_operations(&self, struct_info: &StructInfo, backend: &str) -> Result<String> {
        match backend {
            ...
            _ => Err(ToolError::ExecutionFailed(format!("Unsupported backend: {}", backend))),
        }
    }
    ...
    fn generate_sql_crud(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        let primary_key = struct_info.fields.iter().find(|f| f.is_primary_key).map(|f| f.name.as_str()).unwrap_or("id");
        ...
        for (i, field) in struct_info.fields.iter().enumerate() {
            ...
            code.push_str(&format!("    {} {}{}{}{}\n", snake_case(&field.name), sql_type, nullable, pk, comma));
        }
        code.push_str(");\n\n");

        // CRUD operations
        code.push_str("// Insert operation\n");
        let columns: Vec<String> = struct_info.fields.iter().map(|f| snake_case(&f.name)).collect();
        let placeholders: Vec<String> = (1..=struct_info.fields.len()).map(|i| format!("${}", i)).collect();
        code.push_str(&format!("INSERT INTO {} ({}) VALUES ({});\n\n",
            table_name,
            columns.join(", "),
            placeholders.join(", ")));
        ...
        code.push_str(&format!("SELECT {} FROM {} WHERE {} = $1;\n\n",
            columns.join(", "),
            table_name,
        ...
        let updates: Vec<String> = struct_info.fields.iter()
            .filter(|f| !f.is_primary_key)
            .enumerate()
            .map(|(i, f)| format!("{} = ${}", snake_case(&f.name), i + 2))
            .collect();
        code.push_str(&format!("UPDATE {} SET {} WHERE {} = $1;\n\n",
            table_name,
            updates.join(", "),
            snake_case(primary_key)));
        ...
        code.push_str(&format!("DELETE FROM {} WHERE {} = $1;\n", table_name, snake_case(primary_key)));
    ...
    fn generate_diesel_crud(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        code.push_str(&format!("table! {{\n    {} ({}) {{\n", table_name, snake_case(&struct_info.fields[0].name)));
        for field in &struct_info.fields[1..] {
            code.push_str(&format!("        {} -> {},\n", snake_case(&field.name), self.map_rust_type_to_diesel(&field.ty)));
        }
        code.push_str("    }\n}\n\n");
        ...
        code.push_str(&format!("pub fn create_{}(conn: &PgConnection, new_{}: &New{}) -> QueryResult<{}> {{\n",
            snake_case(struct_name), snake_case(struct_name), struct_name, struct_name));
        ...
        code.push_str(&format!("pub fn get_{}(conn: &PgConnection, id: i32) -> QueryResult<{}> {{\n",
            snake_case(struct_name), struct_name));
        code.push_str(&format!("    {}::table.find(id).first(conn)\n", table_name));
        code.push_str("}\n\n");

        code.push_str(&format!("pub fn update_{}(conn: &PgConnection, id: i32, {}_update: &{}) -> QueryResult<{}> {{\n",
            snake_case(struct_name), snake_case(struct_name), struct_name, struct_name));
        ...
        code.push_str(&format!("pub fn delete_{}(conn: &PgConnection, id: i32) -> QueryResult<usize> {{\n",
            snake_case(struct_name)));
    ...
    fn generate_seaorm_crud(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        code.push_str(&format!("#[derive(Copy, Clone, Default, Debug, DeriveRelation, EnumIter)]\n"));
        ...
        code.push_str(&format!("    {}::insert({}.into_active_model())\n", entity_name, snake_case(struct_name)));
        ...
        code.push_str(&format!("    {}: {}\n", snake_case(struct_name), model_name));
        code.push_str(&format!(") -> Result<{}, DbErr> {{\n", model_name));
        code.push_str(&format!("    {}::update({}.into_active_model())\n", entity_name, snake_case(struct_name)));
        code.push_str("        .exec(db)\n");
        code.push_str("        .await\n");
        code.push_str("}\n\n");
        ...
        code.push_str(&format!("    {}::delete_by_id(id).exec(db).await?;\n", entity_name));
        code.push_str("    Ok(())\n");
        code.push_str("}\n");
    ...
    fn generate_api_endpoints(&self, struct_info: &StructInfo, framework: &str) -> Result<String> {
        match framework {
            ...
            _ => Err(ToolError::ExecutionFailed(format!("Unsupported framework: {}", framework))),
        }
    }
    ...
    fn generate_axum_api(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        for field in &struct_info.fields {
            if !field.is_primary_key {
                let ty = if field.is_optional { field.ty.clone() } else { field.ty.clone() };
                code.push_str(&format!("    pub {}: {},\n", field.name, ty));
            }
        }
        code.push_str("}\n\n");
        ...
        code.push_str(&format!("    Json(payload): Json<Create{}Request>,\n", struct_name));
        ...
        code.push_str(&format!("pub async fn get_{}(\n", route_name));
        code.push_str("    State(state): State<Arc<AppState>>,\n");
        code.push_str(&format!("    Path({}): Path<{}>,\n", primary_key, self.get_primary_key_type(struct_info)));
        code.push_str(&format!(") -> Result<Json<{}>, StatusCode> {{\n", struct_name));
        code.push_str("    // Implementation here\n");
        ...
        code.push_str("    State(state): State<Arc<AppState>>,\n");
        code.push_str(&format!("    Path({}): Path<{}>,\n", primary_key, self.get_primary_key_type(struct_info)));
        code.push_str(&format!("    Json(payload): Json<Update{}Request>,\n", struct_name));
        code.push_str(&format!(") -> Result<Json<{}>, StatusCode> {{\n", struct_name));
        code.push_str("    // Implementation here\n");
        ...
        code.push_str("    State(state): State<Arc<AppState>>,\n");
        code.push_str(&format!(") -> Result<Json<Vec<{}>>, StatusCode> {{\n", struct_name));
        code.push_str("    // Implementation here\n");
        ...
        code.push_str(&format!("pub fn {}_routes() -> Router<Arc<AppState>> {{\n", route_name));
        code.push_str("    Router::new()\n");
        code.push_str(&format!("        .route(\"/\", post(create_{}))\n", route_name));
        code.push_str(&format!("        .route(\"/\", get(list_{}))\n", route_name));
        code.push_str(&format!("        .route(\"/:{}\", get(get_{}))\n", primary_key, route_name));
        code.push_str(&format!("        .route(\"/:{}\", put(update_{}))\n", primary_key, route_name));
        code.push_str(&format!("        .route(\"/:{}\", delete(delete_{}))\n", primary_key, route_name));
        code.push_str("}\n");
    ...
    fn generate_actix_api(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        code.push_str(&format!("    payload: web::Json<Create{}Request>,\n", struct_name));
        code.push_str(") -> Result<HttpResponse> {\n");
        code.push_str("    // Implementation here\n");
        code.push_str("    // Call your CRUD create function\n");
        code.push_str("    Ok(HttpResponse::Created().json(serde_json::json!({\"status\": \"created\"})))\n");
        code.push_str("}\n\n");
        ...
        code.push_str("    // Implementation here\n");
        code.push_str("    // Call your CRUD get function\n");
        code.push_str("    Ok(HttpResponse::Ok().json(serde_json::json!({\"status\": \"ok\"})))\n");
        code.push_str("}\n\n");
    ...
    fn generate_validation_code(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        for field in &struct_info.fields {
            if !field.is_primary_key {
                ...
                let ty = if field.is_optional { format!("Option<{}>", field.ty) } else { field.ty.clone() };
                code.push_str(&format!("    pub {}: {},\n", field.name, ty));
            }
        }
        code.push_str("}\n\n");
    ...
    fn generate_validation_attrs(&self, field: &FieldInfo) -> String {
        ...
        // Add basic validations based on field type
...
impl Tool for CrudGenTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Generate complete CRUD (Create, Read, Update, Delete) operations from Rust struct definitions. Supports SQL, Diesel, SeaORM backends and Axum, Actix, Rocket web frameworks.")
            .args(&[
                Arg::new("input")
                    .long("input")
                    .short('i')
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🔧 {} - {}", "CargoMate CrudGen".bold().blue(), self.description().cyan());
        ...
        for struct_info in &structs {
            ...
            let crud_file = format!("{}/{}_crud_{}.rs", output, snake_case(&struct_info.name), backend);

            // Generate API endpoints
            let api_code = self.generate_api_endpoints(struct_info, framework)?;
            let api_file = format!("{}/{}_api_{}.rs", output, snake_case(&struct_info.name), framework);
            ...
            let validation_file = format!("{}/{}_validation.rs", output, snake_case(&struct_info.name));
            ...
            match output_format {
                OutputFormat::Human => {
                    ...
                    if dry_run {
                        ...
                        println!("   {}", "─".repeat(50));
                        println!("{}", crud_code.lines().take(10).collect::<Vec<_>>().join("\n"));
                        println!("   ...");
                    } else {
                        // Write files
                        fs::write(&crud_file, crud_code)
                            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to write {}: {}", crud_file, e)))?;

                        fs::write(&api_file, api_code)
                            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to write {}: {}", api_file, e)))?;

                        if let Some(validation_code) = validation_code {
                            fs::write(&validation_file, validation_code)
                                .map_err(|e| ToolError::ExecutionFailed(format!("Failed to write {}: {}", validation_file, e)))?;
                        }
                ...
                OutputFormat::Table => {
                    ...
                    println!("{}", "─".repeat(50));
                    println!("{:<20} {:<15} {:<15}", struct_info.name, backend, framework);
                }
            }
        }
