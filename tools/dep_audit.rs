...
impl DepAuditTool {
    ...
    fn parse_dependency_line(&self, line: &str) -> Option<DependencyInfo> {
        // Parse lines like: "├── serde v1.0.193 (MIT OR Apache-2.0)"
        let line = line.trim();

        // Skip tree structure characters
        ...
        // Look for license info in parentheses
        ...
        // Determine if direct dependency (based on indentation level)
        ...
        Some(DependencyInfo {
            ...
            version,
            license,
            security_issues: Vec::new(), // Would need external service
            maintenance_status: "unknown".to_string(),
            is_direct,
        })
    }

    fn check_licenses(&self, deps: &mut [DependencyInfo], allowed_licenses: &[String]) -> usize {
        let mut issues = 0;

        for dep in deps.iter_mut() {
            if let Some(ref license) = dep.license {
                let license_ok = allowed_licenses.iter().any(|allowed| {
    ...
    fn check_security(&self, deps: &mut [DependencyInfo]) -> usize {
        // This would typically query a security database
        // For now, we'll simulate some basic checks
        let mut issues = 0;

        for dep in deps.iter_mut() {
            // Simulate checking for known vulnerable versions
            if dep.name == "some-old-crate" && dep.version.starts_with("0.") {
                dep.security_issues.push("Known security vulnerability in old versions".to_string());
                issues += 1;
            }

            // Check for yanked versions (simplified)
    ...
    fn check_maintenance(&self, deps: &mut [DependencyInfo]) -> usize {
        ...
        for dep in deps.iter_mut() {
            // This would typically check crates.io API for maintenance status
            // For now, simulate based on version patterns
    ...
    fn display_results(&self, result: &AuditResult, format: OutputFormat, verbose: bool) {
        match format {
            OutputFormat::Json => {
                println!("{}", serde_json::to_string_pretty(result).unwrap());
            }
            OutputFormat::Table => {
                println!("{:<30} {:<12} {:<20} {:<10} {:<8}",
                        "Dependency", "Version", "License", "Security", "Maint");
                println!("{}", "─".repeat(85));
            ...
            OutputFormat::Human => {
                ...
                if verbose {
                    ...
                    for dep in &result.dependencies {
                        if !dep.security_issues.is_empty() || dep.license.is_none() || dep.maintenance_status != "active" {
                            ...
                            if dep.maintenance_status != "active" {
                                println!("   🔧 Status: {}", dep.maintenance_status.yellow());
                            }
...
impl Tool for DepAuditTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Audit Rust dependencies for security vulnerabilities, license compatibility, and maintenance status")
            .args(&[
                Arg::new("strict")
                    .long("strict")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let ignore_list = matches.get_one::<String>("ignore")
            .map(|s| s.split(',').map(|s| s.trim().to_string()).collect::<HashSet<_>>())
            .unwrap_or_default();
        let allowed_licenses: Vec<String> = matches.get_one::<String>("licenses")
            .unwrap()
            .split(',')
            .map(|s| s.trim().to_string())
            .collect();
        let check_security = matches.get_flag("check-security") || !matches.contains_id("check-security");
        let check_maintenance = matches.get_flag("check-maintenance") || !matches.contains_id("check-maintenance");
        ...
        if !Path::new("Cargo.toml").exists() {
            return Err(ToolError::ExecutionFailed("Not in a Rust project (Cargo.toml not found)".to_string()));
        }

        // Parse dependencies
        let mut dependencies = self.parse_cargo_tree()?;

        // Filter ignored crates
        ...
        if strict && (security_issues > 0 || license_issues > 0 || maintenance_issues > 0) {
            println!();
            println!("{}", "❌ Audit failed due to issues found in strict mode".red().bold());
            std::process::exit(1);
        }
        ...
        if security_issues > 0 {
            println!();
            println!("{}", "⚠️  Security issues found - review carefully".yellow().bold());
        }
