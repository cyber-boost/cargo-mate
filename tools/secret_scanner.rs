...
impl SecretScannerTool {
    ...
    fn get_secret_patterns(&self) -> Vec<SecretPattern> {
        vec![
            ...
            SecretPattern {
                ...
                example: "ghp_1234567890abcdef1234567890abcdef1234".to_string(),
            },
            SecretPattern {
                ...
                example: "gho_1234567890abcdef1234567890abcdef1234".to_string(),
            },
            SecretPattern {
                name: "GitLab Token".to_string(),
                pattern: r"glpat-[0-9a-zA-Z\-_]{20}".to_string(),
                confidence: "high".to_string(),
                description: "GitLab Personal Access Token".to_string(),
                example: "glpat-1234567890abcdefghij".to_string(),
            },
            SecretPattern {
                name: "Slack Token".to_string(),
                pattern: r"xox[baprs]-[0-9a-zA-Z]{10,48}".to_string(),
                confidence: "high".to_string(),
                description: "Slack API Token".to_string(),
                example: "xoxb-1234567890-1234567890-abcdefghijklmnopqrstuvwx".to_string(),
            },
            SecretPattern {
                name: "Discord Token".to_string(),
                pattern: r"[MN][A-Za-z\d]{23}\.[\w-]{6}\.[\w-]{27}".to_string(),
                confidence: "high".to_string(),
                description: "Discord Bot Token".to_string(),
                example: "MTAwMDAwMDAwMDAwMDAwMDAw.GQ0wHm.example".to_string(),
            },
            SecretPattern {
                name: "Stripe Key".to_string(),
                pattern: r"[rs]k_live_[0-9a-zA-Z]{24}".to_string(),
                confidence: "high".to_string(),
                description: "Stripe API Key".to_string(),
                example: "sk_live_1234567890abcdefghijklmnopqrstuvwxyz".to_string(),
            },
            SecretPattern {
                name: "Database URL".to_string(),
                pattern: r"(?i)postgres://[^:\s]+:[^@\s]+@".to_string(),
                confidence: "high".to_string(),
            ...
            SecretPattern {
                ...
                pattern: r#"(?i)(api[_-]?key|apikey|secret|token|auth[_-]?token)\s*[:=]\s*['"]?.*['"]?"#.to_string(),
                confidence: "medium".to_string(),
                description: "Generic API key or token".to_string(),
                example: "api_key=1234567890abcdef1234567890abcdef".to_string(),
            },
            SecretPattern {
                ...
                example: "-----BEGIN PRIVATE KEY-----".to_string(),
            },
            SecretPattern {
                name: "SSH Private Key".to_string(),
                pattern: r"-----BEGIN\s+OPENSSH\s+PRIVATE\s+KEY-----".to_string(),
                confidence: "high".to_string(),
                description: "SSH Private Key".to_string(),
                example: "-----BEGIN OPENSSH PRIVATE KEY-----".to_string(),
            },
            SecretPattern {
                ...
                example: "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIiwibmFtZSI6IkpvaG4gRG9lIiwiaWF0IjoxNTE2MjM5MDIyfQ.SflKxwRJSMeKKF2QT4fwpMeJf36POk6yJV_adQssw5c".to_string(),
            },
            SecretPattern {
                name: "Password in Config".to_string(),
                pattern: r#"(?i)(password|passwd|pwd)\s*[:=]\s*['"]?.*['"]?"#.to_string(),
                confidence: "low".to_string(),
                description: "Potential password in configuration".to_string(),
                example: "password=secret123".to_string(),
            },
            SecretPattern {
                name: "Bearer Token".to_string(),
                pattern: r#"(?i)bearer\s+[^'"\s]+"#.to_string(),
                confidence: "medium".to_string(),
                description: "Bearer token in Authorization header".to_string(),
                example: "Authorization: Bearer abcdef1234567890".to_string(),
            },
        ]
    }
    ...
    fn scan_file_for_secrets(&self, file_path: &str) -> Result<Vec<SecretFinding>> {
        ...
        // Skip certain file types that commonly contain false positives
            ...
            for pattern in &patterns {
                if let Ok(regex) = Regex::new(&pattern.pattern) {
                    if let Some(captures) = regex.captures(line) {
                        if let Some(secret_match) = captures.get(1).or_else(|| captures.get(0)) {
                            ...
                            let recommendation = self.generate_recommendation(pattern, file_path);
                            ...
                            findings.push(SecretFinding {
                                ...
                                masked_secret,
                                severity: self.calculate_severity(&pattern.confidence, &secret_value),
                                recommendation,
                            });
                        }
                    }
    ...
    fn is_false_positive(&self, secret: &str, pattern: &SecretPattern, line: &str) -> bool {
        ...
        if lower_line.contains("//") && lower_line.find("//").unwrap() < lower_line.find(&lower_secret).unwrap_or(0) {
            return true;
        }
    ...
    fn mask_secret(&self, secret: &str) -> String {
        ...
        format!("{}{}{}",
                &secret[0..visible_chars],
                "*".repeat(masked_chars),
                &secret[secret.len() - visible_chars..])
    }

    fn extract_context(&self, content: &str, line_number: usize, context_lines: usize) -> String {
    ...
    fn generate_recommendation(&self, pattern: &SecretPattern, file_path: &str) -> String {
        let base_recommendation = match pattern.name.as_str() {
            "AWS Access Key" | "AWS Secret Key" => "Move to AWS IAM roles or use environment variables with proper access control",
            "GitHub Token" | "GitHub OAuth" => "Use GitHub Actions secrets or environment variables",
            ...
            "Database URL" => "Use environment variables or connection configuration files",
            "Generic API Key" => "Use environment variables or secure configuration management",
            ...
            "Password in Config" => "Use secure password hashing and environment variables",
        ...
        format!("{} (found in {})", base_recommendation, file_path.split('/').last().unwrap_or(file_path))
    }
    ...
    fn find_files_recursive(&self, dir: &str, files: &mut Vec<String>) -> Result<()> {
        ...
        for entry in fs::read_dir(path)? {
            ...
            if path.is_dir() {
                ...
                if !matches!(dir_name.as_ref(),
                    "target" | ".git" | "node_modules" | ".cargo" | ".vscode" |
                    ".idea" | ".DS_Store" | "dist" | "build" | "out") {
                    self.find_files_recursive(&path.to_string_lossy(), files)?;
                }
            } else if let Some(ext) = path.extension() {
                // Scan relevant file types
                match ext.to_string_lossy().as_ref() {
                    "rs" | "toml" | "json" | "yaml" | "yml" | "env" | "config" |
                    "ini" | "cfg" | "properties" | "sh" | "bash" | "py" | "js" |
                    "ts" | "php" | "rb" | "go" | "java" | "kt" | "scala" => {
                ...
                // Scan files without extensions that might contain secrets
                if matches!(file_name_str.as_ref(),
    ...
    fn calculate_statistics(&self, findings: &[SecretFinding]) -> ScanStatistics {
        ...
        ScanStatistics {
            ...
            medium_confidence,
            low_confidence,
            false_positives: 0, // We'll calculate this separately
    ...
    fn generate_recommendations(&self, findings: &[SecretFinding]) -> Vec<String> {
        ...
        if !findings.is_empty() {
            ...
            recommendations.push("Store secrets in secure configuration management systems".to_string());
            recommendations.push("Use .env files for local development (add to .gitignore)".to_string());
            recommendations.push("Implement pre-commit hooks to prevent secret commits".to_string());
        ...
        if has_high_severity {
            recommendations.insert(0, "🚨 CRITICAL: High-severity secrets found - rotate immediately!".to_string());
        }

        recommendations
    }

    fn display_report(&self, report: &SecretScanReport, output_format: OutputFormat, verbose: bool) {
        match output_format {
            OutputFormat::Human => {
                println!("\n🔍 {} - Secret Scanner Report", "CargoMate SecretScanner".bold().blue());
                println!("{}", "═".repeat(60).blue());

                println!("\n📊 Summary:");
                println!("  • Files Scanned: {}", report.files_scanned);
                println!("  • Secrets Found: {}", report.secrets_found);
                println!("  • Files with Secrets: {}", report.statistics.files_with_secrets);
                println!("  • High Confidence: {}", report.statistics.high_confidence);
                println!("  • Medium Confidence: {}", report.statistics.medium_confidence);
                println!("  • Low Confidence: {}", report.statistics.low_confidence);
                println!("  • Most Common Type: {}", report.statistics.most_common_type);
                ...
                if !report.findings.is_empty() {
                    ...
                    for finding in &report.findings {
                        ...
                        println!("  {} {} {} - {} (Line {})",
                                severity_icon,
                                confidence_icon,
                                finding.secret_type.red(),
                                finding.file_path.split('/').last().unwrap_or(&finding.file_path),
                                finding.line_number);

                        if verbose {
                            println!("    🔒 Secret: {}", finding.masked_secret.dimmed());
                            println!("    📝 Context:");
                ...
                if report.secrets_found == 0 {
                    ...
                    println!("   Found {} potential secret(s) to review", report.secrets_found);
                }
            }
            OutputFormat::Json => {
            ...
            OutputFormat::Table => {
                println!("{:<35} {:<12} {:<15} {:<10} {:<20}",
                        "File", "Line", "Type", "Severity", "Masked Secret");
                println!("{}", "─".repeat(100));

                for finding in &report.findings {
                    let file_name = finding.file_path.split('/').last().unwrap_or(&finding.file_path);
                    println!("{:<35} {:<12} {:<15} {:<10} {:<20}",
                            file_name.chars().take(34).collect::<String>(),
                            finding.line_number.to_string(),
                            finding.secret_type.chars().take(14).collect::<String>(),
                            finding.severity,
                            finding.masked_secret.chars().take(19).collect::<String>());
                }
            }
        }
    }
...
impl Tool for SecretScannerTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Scan your codebase for hardcoded secrets, API keys, tokens, and other \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🔍 {} - Scanning for Secrets", "CargoMate SecretScanner".bold().blue());

        let scan_directory = if workspace {
            ".".to_string()
        } else {
            directory.clone()
        };
                ...
                Ok(findings) => {
                    for finding in findings {
                        ...
                        let include_finding = match min_confidence.as_str() {
                            ...
                            "medium" => finding.confidence == "high" || finding.confidence == "medium",
                            "low" => true,
                            _ => true,
                        };
        ...
        // Filter out findings based on format
        let final_findings = all_findings.into_iter().filter(|finding| {
            match format.as_str() {
                "none" => false,
                "masked" => true,
                "full" => true,
        ...
        let report = SecretScanReport {
            ...
            false_positives: Vec::new(), // We filter these out during scanning
            statistics,
            recommendations,
        ...
        if ci_mode && !report.findings.is_empty() {
            ...
            if critical_count > 0 {
                println!("::error title=Critical Secrets Found::Found {} critical secrets that must be addressed", critical_count);
            }
