...
/// Data structure analysis for cache efficiency
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl CacheAnalyzerTool {
    ...
    fn profile_cache_usage(&self, binary_path: &str, functions: &[String]) -> Result<Vec<CacheProfile>> {
        let mut profiles = Vec::new();

        // Check if perf is available
        if !self.check_tool_availability("perf") {
            return Err(ToolError::ExecutionFailed(
                "perf tool not available. Please install linux-tools-common or equivalent.".to_string()
            ));
        }

        for function in functions {
            // Use perf to collect cache statistics
            let perf_output = ProcessCommand::new("perf")
                .args(&[
                    "stat",
                    "-e", "cache-misses,cache-references,L1-dcache-load-misses,L1-dcache-loads",
                    "-p", &format!("$(pidof {})", binary_path.split('/').last().unwrap_or(binary_path)),
                    "sleep", "1"
                ])
                .output();
            ...
            match perf_output {
                ...
                _ => {
                    // Fallback: create a mock profile for demonstration
                    profiles.push(CacheProfile {
                        function: function.clone(),
                        cache_miss_rate: 5.0, // Assume 5% miss rate
                        l1_misses: 1000,
                        l2_misses: 500,
                        l3_misses: 100,
    ...
    fn parse_perf_output(&self, output: &str, function: &str) -> Result<CacheProfile> {
        ...
        Ok(CacheProfile {
            ...
            l3_misses: 0, // Not directly available from basic perf
            total_accesses: l1_loads,
            cycles: 0, // Would need additional perf events
        })
    }

    fn extract_perf_count(&self, line: &str) -> Option<u64> {
        // Extract number from perf output line like "1,234 cache-misses"
        let parts: Vec<&str> = line.split_whitespace().collect();
        if let Some(first_part) = parts.first() {
            let cleaned: String = first_part.chars()
    ...
    fn analyze_data_structures(&self, file_path: &str) -> Result<Vec<DataStructureAnalysis>> {
        ...
        // Simple regex-based struct analysis (in a real implementation, you'd use syn)
        let struct_regex = regex::Regex::new(r"#\[derive\([^)]*\)\]\s*pub struct\s+(\w+)\s*\{([^}]*)\}").unwrap();
        ...
        for captures in struct_regex.captures_iter(&content) {
            if let (Some(name), Some(fields_str)) = (captures.get(1), captures.get(2)) {
                ...
                // Estimate struct size and cache efficiency
                ...
                for (field_name, field_type) in &fields {
                    ...
                    // Simple heuristic: fields with "count", "index", "len" are likely hot
                ...
                let cache_lines_spanned = (total_size + 63) / 64; // Assume 64-byte cache lines
                let alignment = if total_size >= 32 { 32 } else if total_size >= 16 { 16 } else { 8 };
                let padding_waste = (alignment - (total_size % alignment)) % alignment;

                analyses.push(DataStructureAnalysis {
                    name: struct_name,
                    size: total_size,
                    alignment,
                    cache_lines_spanned,
        ...
        for line in fields_str.lines() {
            if let Some(captures) = field_regex.captures(line.trim()) {
                if let (Some(name), Some(ty)) = (captures.get(1), captures.get(2)) {
                    fields.push((name.as_str().to_string(), ty.as_str().trim().to_string()));
                }
            }
        }
    ...
    fn estimate_field_size(&self, field_type: &str) -> usize {
        match field_type.trim() {
            ...
            "u64" | "i64" | "f64" => 8,
            "usize" | "isize" => 8,
            "String" | "&str" => 24, // Pointer + length + capacity
            "&[u8]" | "Vec<u8>" => 24, // Pointer + length + capacity
    ...
    fn detect_false_sharing(&self, analyses: &[DataStructureAnalysis]) -> Vec<FalseSharingIssue> {
        let mut issues = Vec::new();

        for analysis in analyses {
            if analysis.cache_lines_spanned > 1 {
                // Check for potentially false-sharing fields
                for hot_field in &analysis.hot_fields {
                    for cold_field in &analysis.cold_fields {
                        issues.push(FalseSharingIssue {
                            structure: analysis.name.clone(),
                            field1: hot_field.clone(),
                            field2: cold_field.clone(),
                            access_pattern: "Hot field may share cache line with cold field".to_string(),
                            severity: if analysis.cache_lines_spanned > 2 { "High" } else { "Medium" }.to_string(),
                        });
                    }
                }
            }
    ...
    fn suggest_optimizations(&self, profile: &CacheProfile) -> Vec<CacheOptimization> {
        ...
        if profile.cache_miss_rate > 10.0 {
            suggestions.push(CacheOptimization {
                category: "Cache Misses".to_string(),
                description: format!("High cache miss rate ({:.1}%)", profile.cache_miss_rate),
                impact: "High".to_string(),
                suggestion: "Consider data structure reorganization or prefetching".to_string(),
            });
        }
        ...
        if profile.l1_misses > 1000 {
            suggestions.push(CacheOptimization {
                ...
                impact: "High".to_string(),
                suggestion: "Review data access patterns and consider loop unrolling".to_string(),
            });
        }
        ...
        suggestions.push(CacheOptimization {
            ...
            suggestion: "Group frequently accessed fields together (Struct of Arrays)".to_string(),
        });
        ...
        suggestions.push(CacheOptimization {
            ...
            impact: "Medium".to_string(),
            suggestion: "Consider __builtin_prefetch() for predictable access patterns".to_string(),
        });

        suggestions
    }

    fn measure_prefetch_efficiency(&self, _code_patterns: &[CodePattern]) -> Result<PrefetchAnalysis> {
        // In a real implementation, this would analyze assembly output
...
impl Tool for CacheAnalyzerTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        match output_format {
            OutputFormat::Human => {
                println!("🔍 {} - {}", "CPU Cache Analysis".bold(), self.description().cyan());
                    ...
                    match self.profile_cache_usage(target, &functions) {
                        Ok(profiles) => {
                            for profile in profiles {
                                ...
                                println!("📈 Cache Miss Rate: {}", self.colorize_percentage(profile.cache_miss_rate, threshold));
                                println!("🔢 L1 Cache Misses: {}", profile.l1_misses.to_string().yellow());
                                println!("🔢 L2 Cache Misses: {}", profile.l2_misses.to_string().yellow());

                                if profile.total_accesses > 0 {
                                    println!("📊 Total Memory Accesses: {}", profile.total_accesses);
                                }
                ...
                if analyze_data_structures {
                    if target.ends_with(".rs") {
                            ...
                            Ok(analyses) => {
                                if !analyses.is_empty() {
                                    ...
                                    for analysis in analyses {
                                        ...
                                        println!("    Cache lines spanned: {}", analysis.cache_lines_spanned);
                                        println!("    Alignment: {} bytes", analysis.alignment);

                                        if analysis.padding_waste > 0 {
                                            println!("    Padding waste: {} bytes", analysis.padding_waste.to_string().yellow());
                                        }

                                        if !analysis.hot_fields.is_empty() {
                                            println!("    Hot fields: {}", analysis.hot_fields.join(", ").green());
                                        }
                                        if !analysis.cold_fields.is_empty() {
                                            println!("    Cold fields: {}", analysis.cold_fields.join(", ").cyan());
                                        }
                                    }
                                } else {
                        ...
                        println!("⚠️  Data structure analysis requires a Rust source file (.rs)");
                    }
                }
                    ...
                    if let Ok(analyses) = self.analyze_data_structures(target) {
                        ...
                        if !issues.is_empty() {
                            ...
                            for issue in issues {
                                ...
                                println!("  • [{}] {}: {} and {} may share cache lines",
                                    severity_color,
                                    issue.structure.bold(),
                                    issue.field1.cyan(),
                                    issue.field2.cyan()
                                );
                            }
                ...
                if analyze_prefetch {
                    match self.measure_prefetch_efficiency(&[]) {
                        Ok(analysis) => {
                            println!("\n⚡ Prefetch Analysis:");
                            println!("  Efficiency: {}", self.colorize_percentage(analysis.efficiency, 70.0));
                            println!("  Prefetch instructions: {}", analysis.prefetch_instructions);
                            println!("  Cache line utilization: {}", self.format_percentage(analysis.cache_line_utilization));
                            println!("  Sequential access ratio: {}", self.format_percentage(analysis.sequential_access_ratio));
                        }
                        Err(e) => {
                            if verbose {
            ...
            OutputFormat::Json => {
                ...
                if let Ok(profiles) = self.profile_cache_usage(target, &functions) {
                    json_output["cache_profiles"] = serde_json::to_value(&profiles).unwrap();
                }

                if analyze_data_structures && target.ends_with(".rs") {
                    if let Ok(analyses) = self.analyze_data_structures(target) {
                        json_output["data_structures"] = serde_json::to_value(&analyses).unwrap();
                    }
                }

                if analyze_prefetch {
                    if let Ok(analysis) = self.measure_prefetch_efficiency(&[]) {
                        json_output["prefetch_analysis"] = serde_json::to_value(&analysis).unwrap();
                    }
                }
