...
use std::fs;
use std::path::Path;
use syn::{parse_file, File, Item, ItemFn, Fields, Field, Type, PathSegment, Ident, visit::Visit, spanned::Spanned};
use quote::quote;
...
struct FunctionInfo {
    ...
    span_placeholder: String, // proc_macro2::Span not serializable in syn 2.0
}
...
impl ExampleGenTool {
    ...
    fn generate_function_examples(&self, func_info: &FunctionInfo, example_types: &[String]) -> Result<Vec<Example>> {
        ...
        for example_type in example_types {
            match example_type.as_str() {
                ...
                "integration" => {
                    if let Some(example) = self.create_integration_test_example(func_info)? {
                        examples.push(example);
                    }
                }
                "doc" => {
                ...
                "error-handling" => {
                    let error_examples = self.generate_error_handling_examples(func_info)?;
    ...
    fn create_unit_test_example(&self, func_info: &FunctionInfo) -> Result<Option<Example>> {
        ...
        let param_values: Vec<String> = func_info.params.iter()
            .map(|param| self.generate_parameter_value(&param.type_info))
            .collect();

        // Generate function call
        let param_list = func_info.params.iter()
            .zip(&param_values)
        ...
        Ok(Some(Example {
            ...
            code,
            description: Some(format!("Basic unit test showing how to call {} with typical parameters", func_info.name)),
            example_type: ExampleType::UnitTest,
        }))
    }

    fn create_integration_test_example(&self, func_info: &FunctionInfo) -> Result<Option<Example>> {
        let mut code = format!("#[cfg(test)]\nmod integration_tests {{\n    use super::*;\n\n");
        code.push_str(&format!("    #[test]\n    fn test_{}_integration() {{\n", func_info.name));

        // Generate more realistic test setup
        code.push_str("        // Setup test environment\n");

        // Generate parameter values with setup
        let param_values: Vec<String> = func_info.params.iter()
            .enumerate()
            .map(|(i, param)| self.generate_integration_parameter_value(&param.type_info, i))
            .collect();

        // Generate function call with setup
        let param_list = func_info.params.iter()
            .zip(&param_values)
            .map(|(param, value)| format!("{}: {}", param.name, value))
            .collect::<Vec<_>>()
        ...
        // Generate integration-style assertions
        ...
        if let Some(return_type) = &func_info.return_type {
            if return_type.contains("Result") {
                ...
                code.push_str("                assert!(true); // Replace with actual verification\n");
                code.push_str("            }\n");
                code.push_str("            Err(e) => panic!(\"Integration test failed: {}\", e),\n");
                code.push_str("        }\n");
            } else {
        ...
        Ok(Some(Example {
            ...
            code,
            description: Some(format!("Integration test showing {} usage in a broader context", func_info.name)),
            example_type: ExampleType::IntegrationTest,
        }))
    }

    fn create_doc_test_example(&self, func_info: &FunctionInfo) -> Result<Option<Example>> {
        ...
        // Generate parameter values for doc example
        let param_values: Vec<String> = func_info.params.iter()
        ...
        if let Some(return_type) = &func_info.return_type {
            if return_type.contains("Result") {
                code.push_str("/// match result {\n");
                code.push_str("///     Ok(value) => println!(\"Success: {{:?}}\", value),\n");
        ...
        Ok(Some(Example {
            ...
            code,
            description: Some(format!("Documentation example showing typical usage of {}", func_info.name)),
            example_type: ExampleType::DocTest,
        }))
    }

    fn generate_error_handling_examples(&self, func_info: &FunctionInfo) -> Result<Vec<Example>> {
        let mut examples = Vec::new();

        // Only generate error handling examples if function can fail
        if let Some(return_type) = &func_info.return_type {
            if return_type.contains("Result") || return_type.contains("Option") {
                // Error handling example
                let mut code = format!("// Error handling example for {}\n", func_info.name);
                ...
                if return_type.contains("Result") {
                    ...
                    code.push_str("        eprintln!(\"Operation failed: {{}}\", error);\n");
                    ...
                    code.push_str(&format!("if let Some(result) = {}({}) {{\n", func_info.name, param_list));
                    code.push_str("    println!(\"Success: {{:?}}\", result);\n");
                    code.push_str("    // Process successful result\n");
                    code.push_str("} else {\n");
                ...
                examples.push(Example {
                    title: format!("⚠️ Error Handling Example for {}", func_info.name),
                    code,
                    description: Some(format!("Comprehensive error handling patterns for {}", func_info.name)),
    ...
    fn generate_integration_parameter_value(&self, param_type: &str, index: usize) -> String {
        match param_type {
            "i32" | "i64" | "u32" | "u64" | "isize" | "usize" => format!("test_value_{}", index),
...
impl Tool for ExampleGenTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Automatically generate runnable examples from function signatures, helping developers understand how to use APIs. Creates unit tests, integration tests, documentation examples, and error handling patterns.")
            .args(&[
                Arg::new("input")
                    .long("input")
                    .short('i')
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let example_types: Vec<String> = matches.get_one::<String>("type")
            .unwrap()
            .split(',')
        ...
        println!("📚 {} - {}", "CargoMate ExampleGen".bold().blue(), self.description().cyan());
        ...
        if Path::new(input).is_dir() {
            ...
            for entry in fs::read_dir(input)? {
                ...
                if path.is_file() && path.extension().unwrap_or_default() == "rs" {
                    let functions = self.parse_function_signatures(&path.to_string_lossy())?;
                    ...
                    for func_info in functions {
                        ...
                        let examples = self.generate_function_examples(&func_info, &example_types)?;
        ...
        if all_examples.is_empty() {
            println!("{}", "No examples generated. Check input file or function filter.".yellow());
            return Ok(());
        }

        if verbose {
            println!("   📊 Generated {} examples from {} functions", all_examples.len(), all_examples.len() / example_types.len());
        }
        ...
        match output_format {
            OutputFormat::Human => {
                ...
                if dry_run {
                    ...
                    fs::write(&output_file, &output_content)
                        .map_err(|e| ToolError::ExecutionFailed(format!("Failed to write {}: {}", output_file, e)))?;
            ...
            OutputFormat::Json => {
                let result = serde_json::json!({
                    "input": input,
                    "output": output_file,
                    "examples_generated": all_examples.len(),
                    "format": format,
                    "example_types": example_types,
                    "content_preview": output_content.lines().take(5).collect::<Vec<_>>().join("\n")
                });
            ...
            OutputFormat::Table => {
                ...
                for (title, count) in example_counts {
                    println!("{:<30} {:<15} {:<20}", title, count, example_types.join(","));
                }
            }
        }
