...
/// Information about a macro call found in the code
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct MacroCall {
    pub name: String,
    pub args: String,
    pub span_placeholder: String, // proc_macro2::Span not serializable in syn 2.0
    pub file: String,
    pub line: usize,
    pub column: usize,
}

/// Step in the macro expansion process
#[derive(Debug, Clone)]
...
/// Validation result for expanded code
#[derive(Debug, Clone)]
...
impl MacroExpandTool {
    ...
    fn expand_macro_step_by_step(&self, macro_call: &MacroCall) -> Result<Vec<ExpansionStep>> {
        let mut steps = Vec::new();

        // For declarative macros, we can show the pattern matching process
        if let Ok(macro_def) = self.find_macro_definition(&macro_call.name, &macro_call.file) {
            ...
            // For unknown macros, show a generic expansion
            steps.push(ExpansionStep {
                before: macro_call.args.clone(),
    ...
    fn find_macro_definition(&self, macro_name: &str, file_path: &str) -> Result<String> {
        // Look for macro_rules! definition in the same file
        let content = fs::read_to_string(file_path)?;

        let macro_pattern = format!("macro_rules!\\s*{}\\s*\\{{", regex::escape(macro_name));
        ...
        if let Some(mat) = re.find(&content) {
            ...
            Err(ToolError::ExecutionFailed(format!("Macro definition for '{}' not found", macro_name)))
        }
    }
    ...
    fn apply_hygiene(&self, code: &str) -> Result<String> {
        // Simple hygiene application - replace common patterns
        let mut result = code
            .replace("Vec::", "::alloc::vec::Vec::")
            .replace("vec!", "::alloc::vec!");

        if result.contains("::alloc::vec::Vec::new()") {
            result = result.replace("::alloc::vec::Vec::new()", "::alloc::vec::Vec::new");
        }
    ...
    fn highlight_differences(&self, original: &str, expanded: &str) -> Result<String> {
        ...
        for (i, (orig, exp)) in original_lines.iter().zip(expanded_lines.iter()).enumerate() {
            if orig != exp {
                result.push_str(&format!("{} {} {}\n",
                    format!("{}:", i + 1).yellow(),
                    "-".red(),
    ...
    fn extract_macro_dependencies(&self, macro_call: &MacroCall) -> Vec<MacroDependency> {
    ...
    fn validate_macro_expansion(&self, expanded_code: &str) -> Result<Vec<ValidationIssue>> {
    ...
    fn classify_macro(&self, macro_name: &str) -> MacroType {
        match macro_name {
            "println" | "print" | "format" | "vec" | "assert" | "panic" => MacroType::BuiltIn,
...
impl<'ast> Visit<'ast> for MacroCallVisitor {
    fn visit_macro(&mut self, node: &'ast Macro) {
        if let Some(last_segment) = node.path.segments.last() {
            ...
            self.macro_calls.push(MacroCall {
                ...
                line: 0, // Line info not available in syn 2.0
                column: 0, // Column info not available in syn 2.0
            });
        }
    ...
    // visit_item_macro2 is deprecated in newer syn versions
    // The functionality is covered by visit_item_macro
}
...
impl Tool for MacroExpandTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        match output_format {
            OutputFormat::Human => {
                println!("🔍 {} - {}", "Macro Expansion Analysis".bold(), self.description().cyan());

                // Find and analyze macros
                match self.parse_macro_calls(input) {
                    Ok(macro_calls) => {
                        println!("\n📁 File: {}", input.bold());
                        println!("🔍 Macros Found: {}", macro_calls.len().to_string().cyan());
                        ...
                        if filtered_calls.is_empty() {
                            if let Some(name) = specific_macro {
                                println!("❌ Macro '{}' not found in the file.", name.red());
                            }
                            return Ok(());
                        }
                        ...
                        for call in &filtered_calls {
                            ...
                            macro_types.entry(format!("{:?}", macro_type))
                        ...
                        for (i, call) in filtered_calls.iter().enumerate() {
                            ...
                            if !deps.is_empty() {
                                println!("\n   📚 Dependencies:");
                                for dep in &deps {
                                    println!("     • {} ({})", dep.name.cyan(), dep.dependency_type);
                                }
                            }
                                            ...
                                            Ok(issues) => {
                                                if issues.is_empty() {
                                                    ...
                                                    for issue in issues {
                                                        ...
                                                        println!("\n   🚨 Validation Issue [{}]: {}", severity_color, issue.issue);
                            ...
                            if let Ok(steps) = self.expand_macro_step_by_step(call) {
                                if let Some(final_step) = steps.last() {
                                    ...
                                    let ratio = if original_size > 0 { expanded_size as f64 / original_size as f64 } else { 1.0 };
                            ...
                            // Separator between macros
                            if i < filtered_calls.len() - 1 {
                                println!("{}", "─".repeat(50).blue());
                            }
                        }

                        // Generate expanded code if requested
                        if let Ok(expanded) = self.generate_expanded_code(input) {
                            if diff {
                                println!("\n📋 Original vs Expanded Comparison:");
                                match self.highlight_differences(&fs::read_to_string(input).unwrap_or_default(), &expanded) {
                                    Ok(diff_output) => {
                                        for line in diff_output.lines().take(20) { // Limit output
                                            println!("   {}", line);
                                        }
                                        if diff_output.lines().count() > 20 {
                                            println!("   ... (truncated - {} more lines)", diff_output.lines().count() - 20);
                                        }
                                    }
                                    Err(e) => {
                                        if verbose {
                    ...
                    Err(e) => {
                        return Err(ToolError::ExecutionFailed(format!("Failed to analyze macros: {}", e)));
                    }
                }
            }
            OutputFormat::Json => {
