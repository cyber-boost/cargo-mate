...
use syn::{parse_file, File, Item, ItemTrait, ItemImpl, TraitItem, visit::Visit, spanned::Spanned};
use quote::quote;
...
impl TraitExplorerTool {
    ...
    fn discover_trait_definitions(&self, source_path: &str) -> Result<Vec<TraitDefinition>> {
    ...
    fn analyze_directory_for_traits(&self, dir_path: &str, traits: &mut Vec<TraitDefinition>) -> Result<()> {
    ...
    fn analyze_file_for_traits(&self, file_path: &Path, traits: &mut Vec<TraitDefinition>) -> Result<()> {
        ...
        impl TraitVisitor<'_> {
            ...
            fn extract_supertraits(supertraits: &syn::punctuated::Punctuated<syn::TypeParamBound, syn::token::Plus>) -> Vec<String> {
                ...
                for bound in supertraits {
                    if let syn::TypeParamBound::Trait(trait_bound) = bound {
                        let trait_name = trait_bound.path.segments.iter()
            ...
            fn format_method_signature(sig: &syn::Signature) -> String {
                        ...
                        match arg {
                            ...
                            syn::FnArg::Typed(pat_type) => {
                                if let syn::Pat::Ident(pat_ident) = &*pat_type.pat {
                                    Some(format!("{}: {}", pat_ident.ident, Self::type_to_string(&*pat_type.ty)))
                                } else {
                                    None
                                }
                            }
                ...
                let return_type = match &sig.output {
                    ...
                    syn::ReturnType::Type(_, ty) => format!(" -> {}", Self::type_to_string(ty)),
                };
    ...
    fn discover_trait_implementations(&self, source_path: &str) -> Result<Vec<TraitImplementation>> {
    ...
    fn analyze_directory_for_implementations(&self, dir_path: &str, implementations: &mut Vec<TraitImplementation>) -> Result<()> {
        let entries = fs::read_dir(dir_path)
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to read directory {}: {}", dir_path, e)))?;
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                self.analyze_directory_for_implementations(&path.to_string_lossy(), implementations)?;
            } else if let Some(ext) = path.extension() {
                if ext == "rs" {
    ...
    fn analyze_file_for_implementations(&self, file_path: &Path, implementations: &mut Vec<TraitImplementation>) -> Result<()> {
        let content = fs::read_to_string(file_path)?;
        let ast = parse_file(&content)?;
            ...
            fn visit_item_impl(&mut self, node: &ItemImpl) {
                if let Some((_, trait_path, _)) = &node.trait_ {
                    ...
                    let methods = node.items.iter()
                        .filter_map(|item| {
                            match item {
                                syn::ImplItem::Fn(method) => Some(ImplMethod {
                                    name: method.sig.ident.to_string(),
                                    is_override: method.attrs.iter().any(|attr| {
                                        attr.path().segments.iter().any(|seg| seg.ident == "override")
                                    }),
                                }),
                                _ => None,
                            }
    ...
    fn analyze_trait_usage(&self, implementations: &[TraitImplementation]) -> Result<HashMap<String, Vec<String>>> {
        let mut usage_patterns = HashMap::new();

        for impl_info in implementations {
            usage_patterns.entry(impl_info.trait_name.clone())
    ...
    fn find_missing_implementations(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation]) -> Vec<String> {
        let mut missing = Vec::new();

        let implemented_traits: HashMap<String, Vec<String>> = implementations.iter()
            .fold(HashMap::new(), |mut acc, impl_info| {
        ...
        for trait_def in traits {
            if let Some(implementors) = implemented_traits.get(&trait_def.name) {
                if implementors.is_empty() {
                    missing.push(format!("Trait '{}' has no implementations", trait_def.name));
                }
            } else {
                missing.push(format!("Trait '{}' has no implementations", trait_def.name));
            }
    ...
    fn generate_trait_documentation(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation]) -> Result<String> {
        let mut docs = String::from("# Trait Implementation Analysis\n\n");

        docs.push_str(&format!("Generated on: {}\n\n", chrono::Utc::now().format("%Y-%m-%d %H:%M:%S")));

        // Overview statistics
        docs.push_str("## 📊 Overview\n\n");
        docs.push_str(&format!("- **Total Traits**: {}\n", traits.len()));
        docs.push_str(&format!("- **Total Implementations**: {}\n", implementations.len()));

        let trait_usage: HashMap<String, usize> = implementations.iter()
            .fold(HashMap::new(), |mut acc, impl_info| {
                *acc.entry(impl_info.trait_name.clone()).or_insert(0) += 1;
                acc
            });

        let most_used_trait = trait_usage.iter()
        ...
        for trait_def in traits {
            ...
            for method in &trait_def.methods {
                ...
                let default = if method.has_default_implementation { " (has default)" } else { "" };
                docs.push_str(&format!("- `{}` - {}{}\n", method.signature, required, default));
            }
            docs.push_str("\n");

            // Find implementations
            let trait_implementations: Vec<&TraitImplementation> = implementations.iter()
            ...
            if !trait_implementations.is_empty() {
                ...
                for impl_info in &trait_implementations {
                    docs.push_str(&format!("- `{}` in `{}`\n", impl_info.target_type, impl_info.file_path));
                }
            } else {
    ...
    fn generate_trait_visualization(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation], format: &str) -> Result<String> {
        match format {
            "mermaid" => self.generate_mermaid_graph(traits, implementations),
            "dot" => self.generate_dot_graph(traits, implementations),
            _ => Err(ToolError::InvalidArguments(format!("Unsupported visualization format: {}", format))),
        }
    }

    fn generate_mermaid_graph(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation]) -> Result<String> {
        let mut mermaid = String::from("```mermaid\ngraph TD\n");

        // Add trait nodes
        for trait_def in traits {
            mermaid.push_str(&format!("    T{}[\"Trait: {}\"]\n", trait_def.name.replace("::", "_"), trait_def.name));
        }
        ...
        for impl_info in implementations {
            ...
            mermaid.push_str(&format!("    T{} --> I{}[\"Impl: {}\"]\n", trait_node, type_node, impl_info.target_type));
        }
        ...
        for trait_def in traits {
            for supertrait in &trait_def.supertraits {
                ...
                mermaid.push_str(&format!("    T{} --> T{}[\"Supertrait: {}\"]\n", trait_node, super_node, supertrait));
            }
        }
    ...
    fn generate_dot_graph(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation]) -> Result<String> {
        ...
        // Add trait nodes
        for trait_def in traits {
            dot.push_str(&format!("    \"{}\" [label=\"Trait: {}\", fillcolor=lightblue, style=filled];\n",
                trait_def.name, trait_def.name));
        }

        // Add implementation nodes and edges
        for impl_info in implementations {
            dot.push_str(&format!("    \"{}\" [label=\"Impl: {}\"];\n", impl_info.target_type, impl_info.target_type));
            dot.push_str(&format!("    \"{}\" -> \"{}\" [label=\"implements\"];\n", impl_info.target_type, impl_info.trait_name));
        }

        // Add supertrait relationships
        for trait_def in traits {
            for supertrait in &trait_def.supertraits {
                dot.push_str(&format!("    \"{}\" -> \"{}\" [label=\"extends\", style=dashed];\n", trait_def.name, supertrait));
            }
        }
    ...
    fn suggest_trait_implementations(&self, traits: &[TraitDefinition], implementations: &[TraitImplementation]) -> Vec<TraitSuggestion> {
        let mut suggestions = Vec::new();

        // Common types that might benefit from trait implementations
        let common_types = vec!["String", "i32", "i64", "bool", "Vec<T>", "HashMap<K, V>", "Option<T>", "Result<T, E>"];

        for trait_def in traits {
            // Skip traits that already have many implementations
            let implementation_count = implementations.iter()
                .filter(|impl_info| impl_info.trait_name == trait_def.name)
                .count();

            if implementation_count > 5 {
                continue; // Trait is well-implemented
            }

            for common_type in &common_types {
                let already_implemented = implementations.iter()
                    .any(|impl_info| {
                ...
                if !already_implemented {
                    let confidence = if common_type.contains("String") && trait_def.name.contains("Display") {
                        0.9
                    } else if common_type.contains("Vec") && trait_def.name.contains("IntoIterator") {
                        0.8
                    } else {
                        0.5
                    };

                    suggestions.push(TraitSuggestion {
                        trait_name: trait_def.name.clone(),
                        target_type: common_type.to_string(),
                        reason: format!("{} would benefit from {} implementation", common_type, trait_def.name),
                        confidence,
                    });
                }
            }
    ...
    fn generate_implementations_report(&self, _traits: &[TraitDefinition], _implementations: &[TraitImplementation]) -> Result<String> {
        Err(ToolError::ExecutionFailed("Implementation report feature not yet implemented".to_string()))
    }

    fn generate_missing_report(&self, _missing: &[String]) -> Result<String> {
        Err(ToolError::ExecutionFailed("Missing implementations report feature not yet implemented".to_string()))
    }

    fn generate_suggestions_report(&self, _suggestions: &[TraitSuggestion]) -> Result<String> {
        Err(ToolError::ExecutionFailed("Suggestions report feature not yet implemented".to_string()))
    }
}
...
impl Tool for TraitExplorerTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Discover all trait implementations in your codebase, analyze usage patterns, find missing implementations, and generate comprehensive trait documentation.")
            .args(&[
                Arg::new("input")
                    .long("input")
                    .short('i')
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("🔍 {} - {}", "CargoMate TraitExplorer".bold().blue(), self.description().cyan());

        if !Path::new(input).exists() {
            return Err(ToolError::InvalidArguments(format!("Input path not found: {}", input)));
        }
        ...
        // Discover trait implementations
        ...
        // Filter by specific trait if requested
        let (filtered_definitions, filtered_implementations) = if let Some(trait_name) = specific_trait {
            let defs: Vec<TraitDefinition> = trait_definitions.into_iter()
                .filter(|t| t.name == *trait_name)
                .collect();
            let impls: Vec<TraitImplementation> = trait_implementations.into_iter()
        ...
        let missing_implementations = self.find_missing_implementations(&filtered_definitions, &filtered_implementations);
        let suggestions = if suggest {
            self.suggest_trait_implementations(&filtered_definitions, &filtered_implementations)
        } else {
            Vec::new()
        };

        // Generate output based on requested analysis
        let mut output_content = String::new();

        if implementations_only {
            output_content = self.generate_implementations_report(&filtered_definitions, &filtered_implementations)?;
        } else if missing_only {
            output_content = self.generate_missing_report(&missing_implementations)?;
        } else if visualize {
            output_content = self.generate_trait_visualization(&filtered_definitions, &filtered_implementations, format)?;
        } else if suggest {
            output_content = self.generate_suggestions_report(&suggestions)?;
        } else {
            // Full analysis
            output_content = self.generate_trait_documentation(&filtered_definitions, &filtered_implementations)?;
        }
        ...
        match output_format {
            OutputFormat::Human => {
                ...
                if implementations_only {
                    println!("  📋 Showing implementations for {} traits", filtered_definitions.len());
                } else if missing_only {
                    println!("  ⚠️  Found {} potential missing implementations", missing_implementations.len());
                } else if visualize {
                    println!("  📊 Generated {} visualization", format);
                } else if suggest {
                    println!("  💡 Generated {} implementation suggestions", suggestions.len());
                }
            ...
            OutputFormat::Json => {
                let result = serde_json::json!({
                    ...
                    "analysis_content": output_content
                });
                println!("{}", serde_json::to_string_pretty(&result).unwrap());
            }
            OutputFormat::Table => {
                println!("{:<25} {:<15} {:<12} {:<10} {:<12}",
                    "Analysis Type", "Traits", "Impls", "Missing", "Suggestions");
                println!("{}", "─".repeat(80));
                println!("{:<25} {:<15} {:<12} {:<10} {:<12}",
                    if implementations_only { "Implementations" }
                    else if missing_only { "Missing" }
                    else if visualize { "Visualization" }
                    else if suggest { "Suggestions" }
                    else { "Full Analysis" },
                    filtered_definitions.len(),
                    filtered_implementations.len(),
