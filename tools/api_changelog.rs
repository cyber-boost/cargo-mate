...
use syn::{parse_file, File, Item, ItemFn, ItemStruct, ItemEnum, Fields, Field, Type, PathSegment, Ident, visit::Visit, spanned::Spanned};
use quote::quote;
...
enum NonBreakingChange {
    ...
    FunctionDocumentationAdded {
        function: String,
        file_path: String,
    },
    StructDocumentationAdded {
        struct_name: String,
        file_path: String,
    },
}
...
impl ApiChangelogTool {
    ...
    fn get_git_commit(&self, version: &str) -> Result<String> {
        let output = ProcessCommand::new("git")
            ...
            .output()
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to get git commit: {}", e)))?;

        if output.status.success() {
            let commit = String::from_utf8_lossy(&output.stdout).trim().to_string();
            Ok(commit)
        } else {
            Err(ToolError::ExecutionFailed(format!("Git command failed: {}", String::from_utf8_lossy(&output.stderr))))
        }
    }
    ...
    fn checkout_version(&self, version: &str) -> Result<()> {
        let output = ProcessCommand::new("git")
            ...
            .output()
            .map_err(|e| ToolError::ExecutionFailed(format!("Failed to checkout version: {}", e)))?;

        if output.status.success() {
            Ok(())
        } else {
            Err(ToolError::ExecutionFailed(format!("Git checkout failed: {}", String::from_utf8_lossy(&output.stderr))))
        }
    }
    ...
    fn analyze_api_surface(&self, version: &str, source_path: &str) -> Result<ApiSurface> {
    ...
    fn analyze_directory(&self, dir_path: &str, functions: &mut Vec<FunctionInfo>, structs: &mut Vec<StructInfo>, enums: &mut Vec<EnumInfo>) -> Result<()> {
        ...
        for entry in entries {
            ...
            if path.is_dir() {
                self.analyze_directory(&path.to_string_lossy(), functions, structs, enums)?;
    ...
    fn analyze_file(&self, file_path: &Path, functions: &mut Vec<FunctionInfo>, structs: &mut Vec<StructInfo>, enums: &mut Vec<EnumInfo>) -> Result<()> {
            ...
            fn visit_item_fn(&mut self, node: &ItemFn) {
                if matches!(node.vis, syn::Visibility::Public(_)) {
                    let func_info = FunctionInfo {
                        ...
                        line_number: 0, // Line info not available in syn 2.0
                    };
        ...
        impl ApiVisitor<'_> {
                ...
                if let Fields::Named(named_fields) = fields {
                    for field in &named_fields.named {
                        if let Some(field_ident) = &field.ident {
                            let visibility = if matches!(field.vis, syn::Visibility::Public(_)) {
                                "pub".to_string()
                            } else {
                                "private".to_string()
                            };
            ...
            fn extract_variants(variants: &syn::punctuated::Punctuated<syn::Variant, syn::token::Comma>) -> Vec<VariantInfo> {
    ...
    fn compare_api_surfaces(&self, old_api: &ApiSurface, new_api: &ApiSurface) -> Result<ApiComparison> {
        let mut breaking_changes = Vec::new();
        let mut new_features = Vec::new();
        let mut non_breaking_changes = Vec::new();

        // Compare functions
        self.compare_functions(&old_api.functions, &new_api.functions, &mut breaking_changes, &mut new_features, &mut non_breaking_changes);

        // Compare structs
        self.compare_structs(&old_api.structs, &new_api.structs, &mut breaking_changes, &mut new_features, &mut non_breaking_changes);

        // Compare enums
        self.compare_enums(&old_api.enums, &new_api.enums, &mut breaking_changes, &mut new_features);
    ...
    fn compare_functions(&self, old_functions: &[FunctionInfo], new_functions: &[FunctionInfo],
                        breaking_changes: &mut Vec<BreakingChange>, new_features: &mut Vec<NewFeature>,
                        non_breaking_changes: &mut Vec<NonBreakingChange>) {
        let old_function_map: std::collections::HashMap<&str, &FunctionInfo> = old_functions.iter()
            .map(|f| (f.name.as_str(), f))
            .collect();

        let new_function_map: std::collections::HashMap<&str, &FunctionInfo> = new_functions.iter()
            .map(|f| (f.name.as_str(), f))
            .collect();
        ...
        for old_func in old_functions {
            if !new_function_map.contains_key(old_func.name.as_str()) {
                breaking_changes.push(BreakingChange::FunctionRemoved {
        ...
        for new_func in new_functions {
            if !old_function_map.contains_key(new_func.name.as_str()) {
                ...
                new_features.push(NewFeature::FunctionAdded {
        ...
        for new_func in new_functions {
            if let Some(old_func) = old_function_map.get(new_func.name.as_str()) {
                ...
                if !changes.is_empty() {
                    ...
                    breaking_changes.push(BreakingChange::FunctionSignatureChanged {
                        function: new_func.name.clone(),
                        old_signature,
                        new_signature,
                        changes,
    ...
    fn compare_function_signatures(&self, old_func: &FunctionInfo, new_func: &FunctionInfo) -> Vec<FunctionChange> {
        let mut changes = Vec::new();

        // Compare parameters
        let old_params: std::collections::HashMap<&str, &ParameterInfo> = old_func.params.iter()
            .map(|p| (p.name.as_str(), p))
            .collect();

        let new_params: std::collections::HashMap<&str, &ParameterInfo> = new_func.params.iter()
            .map(|p| (p.name.as_str(), p))
            .collect();
        ...
        for old_param in &old_func.params {
            if !new_params.contains_key(old_param.name.as_str()) {
                changes.push(FunctionChange::ParameterRemoved {
        ...
        for new_param in &new_func.params {
            if !old_params.contains_key(new_param.name.as_str()) {
                changes.push(FunctionChange::ParameterAdded {
        ...
        if old_func.return_type != new_func.return_type {
            changes.push(FunctionChange::ReturnTypeChanged {
                old_ty: old_func.return_type.clone().unwrap_or_else(|| "None".to_string()),
                new_ty: new_func.return_type.clone().unwrap_or_else(|| "None".to_string()),
            });
        }

        // Compare visibility
        if old_func.visibility != new_func.visibility {
            changes.push(FunctionChange::VisibilityChanged {
    ...
    fn compare_structs(&self, old_structs: &[StructInfo], new_structs: &[StructInfo],
                      breaking_changes: &mut Vec<BreakingChange>, new_features: &mut Vec<NewFeature>,
                      non_breaking_changes: &mut Vec<NonBreakingChange>) {
        let old_struct_map: std::collections::HashMap<&str, &StructInfo> = old_structs.iter()
            .map(|s| (s.name.as_str(), s))
            .collect();

        let new_struct_map: std::collections::HashMap<&str, &StructInfo> = new_structs.iter()
            .map(|s| (s.name.as_str(), s))
            .collect();

        // Check for new structs
        for new_struct in new_structs {
            if !old_struct_map.contains_key(new_struct.name.as_str()) {
                new_features.push(NewFeature::StructAdded {
        ...
        for old_struct in old_structs {
            if !new_struct_map.contains_key(old_struct.name.as_str()) {
                breaking_changes.push(BreakingChange::TypeRemoved {
        ...
        for new_struct in new_structs {
            if let Some(old_struct) = old_struct_map.get(new_struct.name.as_str()) {
                self.compare_struct_fields(old_struct, new_struct, breaking_changes, new_features);
            }
        }
    }

    fn compare_struct_fields(&self, old_struct: &StructInfo, new_struct: &StructInfo,
                            breaking_changes: &mut Vec<BreakingChange>, new_features: &mut Vec<NewFeature>) {
        let old_fields: std::collections::HashMap<&str, &FieldInfo> = old_struct.fields.iter()
            .map(|f| (f.name.as_str(), f))
            .collect();

        let new_fields: std::collections::HashMap<&str, &FieldInfo> = new_struct.fields.iter()
            .map(|f| (f.name.as_str(), f))
            .collect();
        ...
        for old_field in &old_struct.fields {
            if !new_fields.contains_key(old_field.name.as_str()) {
                breaking_changes.push(BreakingChange::StructFieldRemoved {
                    struct_name: new_struct.name.clone(),
        ...
        for new_field in &new_struct.fields {
            if !old_fields.contains_key(new_field.name.as_str()) {
                new_features.push(NewFeature::StructFieldAdded {
                    struct_name: new_struct.name.clone(),
        ...
        for new_field in &new_struct.fields {
            if let Some(old_field) = old_fields.get(new_field.name.as_str()) {
                if old_field.type_info != new_field.type_info {
                    breaking_changes.push(BreakingChange::StructFieldTypeChanged {
                        struct_name: new_struct.name.clone(),
                        field: new_field.name.clone(),
    ...
    fn compare_enums(&self, old_enums: &[EnumInfo], new_enums: &[EnumInfo],
                    breaking_changes: &mut Vec<BreakingChange>, new_features: &mut Vec<NewFeature>) {
        let old_enum_map: std::collections::HashMap<&str, &EnumInfo> = old_enums.iter()
            .map(|e| (e.name.as_str(), e))
            .collect();

        let new_enum_map: std::collections::HashMap<&str, &EnumInfo> = new_enums.iter()
            .map(|e| (e.name.as_str(), e))
            .collect();
        ...
        for new_enum in new_enums {
            if !old_enum_map.contains_key(new_enum.name.as_str()) {
                new_features.push(NewFeature::EnumAdded {
    ...
    fn format_function_signature(&self, func: &FunctionInfo) -> String {
        ...
        match &func.return_type {
            Some(return_type) => format!("fn {}({}) -> {}", func.name, params, return_type),
            None => format!("fn {}({})", func.name, params),
        }
    }

    fn generate_changelog(&self, comparison: &ApiComparison, format: &str, old_version: &str, new_version: &str) -> Result<String> {
        match format {
            "markdown" => self.generate_markdown_changelog(comparison, old_version, new_version),
            "json" => self.generate_json_changelog(comparison),
            "html" => self.generate_html_changelog(comparison, old_version, new_version),
            _ => Err(ToolError::InvalidArguments(format!("Unsupported format: {}", format))),
        }
    }

    fn generate_markdown_changelog(&self, comparison: &ApiComparison, old_version: &str, new_version: &str) -> Result<String> {
        let mut markdown = format!("# API Changelog: {} → {}\n\n", old_version, new_version);
        ...
        if !comparison.breaking_changes.is_empty() {
            ...
            for change in &comparison.breaking_changes {
                match change {
                    BreakingChange::FunctionSignatureChanged { function, old_signature, new_signature, changes } => {
                        markdown.push_str(&format!("### Function Signature Changed: `{}`\n", function));
                        ...
                        for change in changes {
                            match change {
                                FunctionChange::ParameterAdded { name, ty } => {
                                    markdown.push_str(&format!("- Added parameter: `{}`: `{}`\n", name, ty));
                                }
                                FunctionChange::ParameterRemoved { name, ty } => {
                                    markdown.push_str(&format!("- Removed parameter: `{}`: `{}`\n", name, ty));
                                }
                                FunctionChange::ParameterTypeChanged { name, old_ty, new_ty } => {
                                    markdown.push_str(&format!("- Parameter `{}` type changed: `{}` → `{}`\n", name, old_ty, new_ty));
                                }
                                FunctionChange::ReturnTypeChanged { old_ty, new_ty } => {
                                    markdown.push_str(&format!("- Return type changed: `{}` → `{}`\n", old_ty, new_ty));
                                }
                                _ => {}
                            }
                        }
                    ...
                    BreakingChange::FunctionRemoved { function, file_path } => {
                        markdown.push_str(&format!("### Function Removed: `{}`\n", function));
                        markdown.push_str(&format!("**File:** `{}`\n\n", file_path));
                    }
                    BreakingChange::StructFieldRemoved { struct_name, field, file_path } => {
                        markdown.push_str(&format!("### Struct Field Removed: `{}.{}`\n", struct_name, field));
                        markdown.push_str(&format!("**File:** `{}`\n\n", file_path));
                    }
                    BreakingChange::TypeRemoved { type_name, type_kind, file_path } => {
                        markdown.push_str(&format!("### {} Removed: `{}`\n", type_kind, type_name));
                        markdown.push_str(&format!("**File:** `{}`\n\n", file_path));
                    }
                    _ => {}
                }
            ...
            for feature in &comparison.new_features {
                match feature {
                    NewFeature::FunctionAdded { function, signature, file_path } => {
                        markdown.push_str(&format!("### New Function: `{}`\n", function));
                        markdown.push_str(&format!("**Signature:** `{}`\n", signature));
                        markdown.push_str(&format!("**File:** `{}`\n\n", file_path));
                    }
                    NewFeature::StructAdded { struct_name, file_path } => {
                        markdown.push_str(&format!("### New Struct: `{}`\n", struct_name));
                        markdown.push_str(&format!("**File:** `{}`\n\n", file_path));
                    }
                    NewFeature::StructFieldAdded { struct_name, field, field_type, file_path } => {
                        markdown.push_str(&format!("### New Struct Field: `{}.{}`\n", struct_name, field));
        ...
        markdown.push_str(&format!("**Overall Compatibility:** {}%\n\n", compatibility_score));
        markdown.push_str(&format!("**Breaking Changes:** {}\n", comparison.breaking_changes.len()));
        markdown.push_str(&format!("**New Features:** {}\n", comparison.new_features.len()));
        markdown.push_str(&format!("**Non-Breaking Changes:** {}\n\n", comparison.non_breaking_changes.len()));
    ...
    fn generate_json_changelog(&self, comparison: &ApiComparison) -> Result<String> {
        let changelog = serde_json::json!({
            ...
            "compatibility_score": self.calculate_compatibility_score(comparison)
        });

        Ok(serde_json::to_string_pretty(&changelog)?)
    }

    fn generate_html_changelog(&self, comparison: &ApiComparison, old_version: &str, new_version: &str) -> Result<String> {
        let mut html = format!("<!DOCTYPE html>\n<html>\n<head>\n<title>API Changelog: {} → {}</title>\n</head>\n<body>\n", old_version, new_version);
        html.push_str(&format!("<h1>API Changelog: {} → {}</h1>\n", old_version, new_version));
            ...
            for change in &comparison.breaking_changes {
                match change {
                    BreakingChange::FunctionSignatureChanged { function, .. } => {
                        html.push_str(&format!("<h3>Function Signature Changed: <code>{}</code></h3>\n", function));
                    }
                    BreakingChange::FunctionRemoved { function, .. } => {
                        html.push_str(&format!("<h3>Function Removed: <code>{}</code></h3>\n", function));
                    }
                    _ => {}
                }
            }
            ...
            for feature in &comparison.new_features {
                match feature {
                    NewFeature::FunctionAdded { function, .. } => {
                        html.push_str(&format!("<h3>New Function: <code>{}</code></h3>\n", function));
                    }
                    _ => {}
                }
            }
    ...
    fn calculate_compatibility_score(&self, comparison: &ApiComparison) -> u32 {
        ...
        let non_breaking_score = comparison.non_breaking_changes.len() * non_breaking_weight;
        ...
        if max_possible_score == 0 {
            ...
            ((max_possible_score - total_score) as f64 / max_possible_score as f64 * 100.0) as u32
        }
    }
}
...
impl Tool for ApiChangelogTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about("Compare API surfaces between different versions and generate comprehensive changelogs. Automatically detects breaking changes, new features, and analyzes API compatibility.")
            .args(&[
                Arg::new("old-version")
                    .long("old-version")
                    .short('o')
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!("📊 {} - {}", "CargoMate ApiChangelog".bold().blue(), self.description().cyan());

        if !Path::new(source_path).exists() {
            return Err(ToolError::InvalidArguments(format!("Source path not found: {}", source_path)));
        }

        // Check if we're in a git repository
        if !Path::new(".git").exists() {
            return Err(ToolError::ExecutionFailed("Not in a git repository. API changelog requires git history.".to_string()));
        }

        if verbose {
            println!("   📋 Analyzing API changes from {} to {}", old_version, new_version);
        }

        // Analyze old version API surface
        ...
        // Analyze new version API surface
        ...
        if verbose {
            println!("   📊 Found {} breaking changes, {} new features, {} non-breaking changes",
                comparison.breaking_changes.len(),
                comparison.new_features.len(),
                comparison.non_breaking_changes.len());
        }
        ...
        // Generate changelog
        let changelog = self.generate_changelog(&filtered_comparison, format, old_version, new_version)?;

        match output_format {
            OutputFormat::Human => {
                println!("  ✅ Generated API changelog: {} → {}", old_version, new_version);
                println!("     → {}", output.cyan());
            ...
            OutputFormat::Json => {
                let result = serde_json::json!({
                    ...
                    "breaking_changes_count": filtered_comparison.breaking_changes.len(),
                    "new_features_count": filtered_comparison.new_features.len(),
                    "compatibility_score": self.calculate_compatibility_score(&filtered_comparison),
                    "changelog": changelog
                });
            ...
            OutputFormat::Table => {
                println!("{:<20} {:<15} {:<15} {:<12}",
                    "Version Comparison", "Breaking", "New Features", "Compatibility");
                println!("{}", "─".repeat(70));
                let score = self.calculate_compatibility_score(&filtered_comparison);
                println!("{:<20} {:<15} {:<15} {:<12}%",
                    format!("{} → {}", old_version, new_version),
                    filtered_comparison.breaking_changes.len(),
                    filtered_comparison.new_features.len(),
                    score);
            }
        }
