...
impl CoverageGuardTool {
        ...
        if self.is_tool_available("grcov") {
            ...
            Err(
                ToolError::ExecutionFailed(
                    "No coverage tool found. Install grcov, tarpaulin, or cargo-llvm-cov"
                        .to_string(),
                ),
            )
        }
    ...
    fn run_tarpaulin(&self) -> Result<CoverageReport> {
        let output = ProcessCommand::new("cargo")
            ...
            .output()
            .map_err(|e| ToolError::ExecutionFailed(
                format!("Failed to run tarpaulin: {}", e),
            ))?;
        if !output.status.success() {
            return Err(
                ToolError::ExecutionFailed("tarpaulin command failed".to_string()),
            );
        }
    ...
    fn run_llvm_cov(&self) -> Result<CoverageReport> {
        let output = ProcessCommand::new("cargo")
            .args(
                &["llvm-cov", "report", "--json", "--output-path", "/tmp/coverage.json"],
            )
            .output()
            .map_err(|e| ToolError::ExecutionFailed(
                format!("Failed to run llvm-cov: {}", e),
            ))?;
        if !output.status.success() {
            return Err(
                ToolError::ExecutionFailed("llvm-cov command failed".to_string()),
            );
        }
    ...
    fn parse_tarpaulin_json(&self, file_path: &str) -> Result<CoverageReport> {
        ...
        let coverage_percentage = json["coverage_percentage"].as_f64().unwrap_or(0.0);
        let lines_covered = json["covered_lines"].as_u64().unwrap_or(0) as usize;
        let lines_total = json["total_lines"].as_u64().unwrap_or(0) as usize;
        let details = CoverageDetails {
            lines_covered,
            lines_total,
            functions_covered: 0,
    ...
    fn parse_llvm_cov_json(&self, file_path: &str) -> Result<CoverageReport> {
        ...
        let lines_covered = data["lines"]["covered"].as_u64().unwrap_or(0) as usize;
        let lines_total = data["lines"]["count"].as_u64().unwrap_or(0) as usize;
        let functions_covered = data["functions"]["covered"].as_u64().unwrap_or(0)
            as usize;
        let functions_total = data["functions"]["count"].as_u64().unwrap_or(0) as usize;
        let branches_covered = data["branches"]["covered"].as_u64().unwrap_or(0)
            as usize;
        let branches_total = data["branches"]["count"].as_u64().unwrap_or(0) as usize;
        let coverage_percentage = if lines_total > 0 {
            (lines_covered as f64 / lines_total as f64) * 100.0
        } else {
            0.0
    ...
    fn get_current_commit(&self) -> Option<String> {
        ProcessCommand::new("git")
            ...
            .filter(|output| output.status.success())
            .and_then(|output| {
                String::from_utf8_lossy(&output.stdout).trim()[..8].to_string().into()
            })
    }
    fn get_baseline_coverage(&self, baseline: &str) -> Result<f64> {
        let output = ProcessCommand::new("git")
            ...
            .output()
            .map_err(|e| ToolError::ExecutionFailed(
                format!("Failed to get baseline: {}", e),
            ))?;
        if !output.status.success() {
            return Err(
                ToolError::ExecutionFailed(
                    format!(
                        "Baseline commit {} not found or missing coverage data", baseline
                    ),
                ),
            );
        }
        ...
        match output_format {
            OutputFormat::Human => {
                ...
                if verbose {
                    println!("\n📊 Detailed Metrics:");
                    println!(
                        "  • Lines: {}/{} ({:.2}%)", report.details.lines_covered,
                        report.details.lines_total, if report.details.lines_total > 0 {
                        (report.details.lines_covered as f64 / report.details.lines_total
                        as f64) * 100.0 } else { 0.0 }
                    );
                    if report.details.functions_total > 0 {
                        println!(
                            "  • Functions: {}/{} ({:.2}%)", report.details
                            .functions_covered, report.details.functions_total, (report
                            .details.functions_covered as f64 / report.details
                            .functions_total as f64) * 100.0
                        );
                    }
                    if report.details.branches_total > 0 {
                        println!(
                            "  • Branches: {}/{} ({:.2}%)", report.details
                            .branches_covered, report.details.branches_total, (report
                            .details.branches_covered as f64 / report.details
                            .branches_total as f64) * 100.0
                        );
                    }
            ...
            OutputFormat::Table => {
                println!(
                    "{:<20} {:<15} {:<15} {:<10}", "Metric", "Covered", "Total",
                    "Percentage"
                );
                println!("{}", "─".repeat(70));
                println!(
                    "{:<20} {:<15} {:<15} {:.2}%", "Lines", report.details.lines_covered
                    .to_string(), report.details.lines_total.to_string(), if report
                    .details.lines_total > 0 { (report.details.lines_covered as f64 /
                    report.details.lines_total as f64) * 100.0 } else { 0.0 }
                );
                if report.details.functions_total > 0 {
                    println!(
                        "{:<20} {:<15} {:<15} {:.2}%", "Functions", report.details
                        .functions_covered.to_string(), report.details.functions_total
                        .to_string(), (report.details.functions_covered as f64 / report
                        .details.functions_total as f64) * 100.0
                    );
                }
                if report.details.branches_total > 0 {
                    println!(
                        "{:<20} {:<15} {:<15} {:.2}%", "Branches", report.details
                        .branches_covered.to_string(), report.details.branches_total
                        .to_string(), (report.details.branches_covered as f64 / report
                        .details.branches_total as f64) * 100.0
                    );
                }
            }
        }
...
impl Tool for CoverageGuardTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Monitor code coverage and prevent commits when coverage drops below \
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        if ci_mode {
            if report.threshold_met {
                ...
                println!(
                    "::set-output name=coverage-percentage::{:.2}", report
                    .current_coverage
                );
            } else {
                ...
                println!(
                    "::error title=Coverage Check Failed::Coverage {:.2}% is below threshold {:.2}%",
