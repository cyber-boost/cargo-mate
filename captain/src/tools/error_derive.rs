...
impl ErrorDeriveTool {
    ...
    fn analyze_directory(&self, dir_path: &str) -> Result<ErrorAnalysis> {
        ...
        fn visit_dir(
            ...
            handling_patterns: &mut Vec<String>,
        ) -> Result<()> {
            ...
            for entry in entries {
                ...
                if path.is_dir() {
                    ...
                    if ext == "rs" && path.file_name().unwrap_or_default() != "mod.rs" {
                        if let Ok(analysis) = ErrorDeriveTool::new()
                            .analyze_error_usage(&path.to_string_lossy())
                        {
        ...
        Ok(ErrorAnalysis {
            ...
            error_types: all_error_types
                ...
                .into_iter()
                .collect(),
            error_handling_patterns: all_handling_patterns
                .into_iter()
                .collect::<std::collections::HashSet<_>>()
                .into_iter()
                .collect(),
    ...
    fn generate_error_enum(
        ...
        let mut code = format!(
            "// Generated error types based on codebase analysis\n\n"
        );
    ...
    fn generate_error_impls(&self, error_name: &str) -> Result<String> {
        ...
        code.push_str(
            "                Self::Unknown(format!(\"{}: {}\", context.into(), msg))\n",
        );
        code.push_str("            }\n");
        ...
        code.push_str("    }\n\n");
        code.push_str(
            "    pub fn with_field<S: Into<String>>(self, field: S) -> Self {\n",
        );
        code.push_str("        Self::Validation {\n");
    ...
    fn generate_context_methods(&self, error_name: &str) -> Result<String> {
        ...
        code.push_str("use std::fmt;\n\n");
        code.push_str(
            &format!("pub type {}Result<T> = Result<T, {}>;\n\n", error_name, error_name),
        );
        code.push_str("/// Add context to a Result\n");
        code.push_str(
            &format!(
                "pub fn with_context<T, E, S>(result: Result<T, E>, context: S) -> Result<T, {}>\n",
                error_name
            ),
        );
        code.push_str("where\n");
        ...
        code.push_str(
            &format!(
                "        {}::unknown(format!(\"{{}}: {{}}\", context.into(), err))\n",
                error_name
            ),
        );
        code.push_str("    })\n");
        ...
        code.push_str(
            &format!(
                "pub fn with_field<T, S>(field: S) -> impl FnOnce(T) -> {}\n", error_name
            ),
        );
        code.push_str("where\n");
        ...
        code.push_str(
            &format!("        {}::validation(field, err.to_string())\n", error_name),
        );
        code.push_str("    }\n");
        code.push_str("}\n\n");
        ...
        code.push_str(
            &format!("macro_rules! {}validation_error {{\n", error_name.to_lowercase()),
        );
        ...
        code.push_str("#[macro_export]\n");
        code.push_str(
            &format!("macro_rules! {}with_context {{\n", error_name.to_lowercase()),
        );
        code.push_str("    ($result:expr, $context:expr) => {\n");
        code.push_str("        $result.map_err(|err| {\n");
        code.push_str(
            &format!(
                "            {}::unknown(format!(\"{{}}: {{}}\", $context, err))\n",
                error_name
            ),
        );
        code.push_str("        })\n");
    ...
    fn generate_comprehensive_error_type(
        &self,
        analysis: &ErrorAnalysis,
        name: &str,
    ) -> Result<String> {
    ...
    fn generate_error_handling_patterns(
        &self,
        analysis: &ErrorAnalysis,
    ) -> Result<String> {
        ...
        for error_type in &analysis.error_types {
            let pattern_name = self
                .error_type_to_variant_name(error_type)
                .to_lowercase();
        ...
        code.push_str(
            "    pub fn log_and_convert<E: fmt::Display>(err: E, context: &str) -> AppError {\n",
        );
        ...
        code.push_str(
            "    pub async fn async_error_handler<F, Fut, T, E>(future: F, context: &str) -> Result<T>\n",
        );
        code.push_str("    where\n");
...
impl Tool for ErrorDeriveTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Analyze existing error handling patterns in your codebase and generate comprehensive error types with proper Display, Error, From traits, context methods, and error handling patterns. Supports thiserror derive macros and backtrace support.",
            )
            .args(
                &[
                    Arg::new("input")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "🔧 {} - {}", "CargoMate ErrorDerive".bold().blue(), self.description()
            .cyan()
        );
        ...
        if verbose {
            ...
            println!(
                "     • Found {} handling patterns", analysis.error_handling_patterns
                .len()
            );
        ...
        match output_format {
            ...
            OutputFormat::Json => {
                let result = serde_json::json!(
                    { "error_name" : name, "input" : input, "output" : output,
                    "error_types_found" : analysis.error_types.len(), "patterns_found" :
                    analysis.patterns.len(), "thiserror_enabled" : thiserror,
                    "backtrace_enabled" : backtrace, "context_enabled" : context,
                    "patterns_enabled" : patterns, "error_types" : analysis.error_types,
                    "code_preview" : error_code.lines().take(10).collect::< Vec < _ >> ()
                    .join("\n") }
                );
            ...
            OutputFormat::Table => {
                println!(
                    "{:<15} {:<10} {:<8} {:<8} {:<8} {:<8}", "Error Type", "Input",
                    "ThisErr", "Backtrace", "Context", "Patterns"
                );
                println!("{}", "─".repeat(70));
                println!(
                    "{:<15} {:<10} {:<8} {:<8} {:<8} {:<8}", name, Path::new(input)
                    .file_name().unwrap_or_default().to_string_lossy(), if thiserror {
                    "Yes" } else { "No" }, if backtrace { "Yes" } else { "No" }, if
                    context { "Yes" } else { "No" }, if patterns { "Yes" } else { "No" }
                );
            }
        }
