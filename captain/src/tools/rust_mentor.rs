...
use syn::{
    parse_file, File, Item, ItemFn, ItemStruct, ItemTrait, Fields, Field, Type,
    PathSegment, Ident, visit::Visit,
};
use quote::ToTokens;
...
impl RustMentorTool {
    ...
    fn analyze_codebase(&self, input_path: &str) -> Result<CodeAnalysis> {
        ...
        let recommendations = self
            .generate_recommendations(&functions, &structs, &traits, &issues);
        let learning_opportunities = self
            .identify_learning_opportunities(&functions, &structs, &traits);
        Ok(CodeAnalysis {
            functions,
            structs,
            traits,
    ...
    fn analyze_directory(
        ...
        structs: &mut Vec<StructAnalysis>,
        traits: &mut Vec<TraitAnalysis>,
    ) -> Result<()> {
        ...
        impl CodeVisitor<'_> {
            ...
            fn infer_parameter_purpose(name: &str, ty: &str) -> String {
                match (name, ty) {
                    ...
                    (_, "i32" | "i64" | "u32" | "u64") => {
                        "Numeric value or count".to_string()
                    }
            ...
            fn field_considerations(name: &str, ty: &str) -> Vec<String> {
                ...
                if ty.contains("&str") && !ty.contains("String") {
                    considerations
                        .push("Consider using owned String for ownership".to_string());
                }
                if name.ends_with("_at") && ty.contains("u64") {
                    considerations
                        .push(
                            "Consider using DateTime for better time handling"
                                .to_string(),
                        );
                }
                if name == "id" && ty == "String" {
                    considerations
                        .push(
                            "Consider using UUID type for better type safety".to_string(),
                        );
                }
                considerations
            }
            fn suggest_function_improvements(node: &ItemFn) -> Vec<String> {
                ...
                if node.sig.inputs.len() > 4 {
                    improvements
                        .push(
                            "Consider grouping parameters into a configuration struct"
                                .to_string(),
                        );
                }
                if code.len() > 1000 {
                    improvements
                        .push(
                            "Function is quite long - consider breaking into smaller functions"
                                .to_string(),
                        );
                }
            ...
            fn struct_design_considerations(node: &ItemStruct) -> Vec<String> {
                ...
                match &node.fields {
                    Fields::Named(named_fields) if named_fields.named.len() > 10 => {
                        considerations
                            .push(
                                "Large struct - consider splitting into smaller structs"
                                    .to_string(),
                            );
                    }
                    Fields::Unnamed(
                        unnamed_fields,
                    ) if unnamed_fields.unnamed.len() > 5 => {
                        considerations
                            .push(
                                "Many tuple fields - consider using named fields for clarity"
                                    .to_string(),
                            );
                    }
                    _ => {}
            ...
            fn identify_trait_purpose(node: &ItemTrait) -> String {
                ...
                match (name.as_str(), method_count) {
                    ...
                    ("Display", _) => {
                        "Enable user-friendly string representation".to_string()
                    }
                    ("From" | "Into", _) => "Enable type conversions".to_string(),
                    ("Iterator", _) => "Enable iteration over collections".to_string(),
                    (_, 1) => {
                        "Single method interface - likely a callback or conversion trait"
                            .to_string()
                    }
                    (_, 2..=3) => {
                        "Small interface - focused on specific functionality".to_string()
                    }
                    _ => {
                        "Larger interface - represents a capability or behavior"
                            .to_string()
                    }
                }
            }
            fn trait_use_cases(node: &ItemTrait) -> Vec<String> {
                ...
                match name.as_str() {
                    "Debug" => {
                        vec![
                            "Debugging and logging".to_string(), "Error messages"
                            .to_string()
                        ]
                    }
                    "Clone" => {
                        vec![
                            "Creating copies of data".to_string(),
                            "Working with collections".to_string()
                        ]
                    }
                    "Display" => {
                        vec![
                            "User-facing output".to_string(), "Logging and reporting"
                            .to_string()
                        ]
                    }
                    "From" | "Into" => {
                        vec![
                            "Type conversions".to_string(), "Builder patterns"
                            .to_string()
                        ]
                    }
                    "Iterator" => {
                        vec![
                            "Working with collections".to_string(), "Streaming data"
                            .to_string()
                        ]
                    }
            ...
            fn explain_function(node: &ItemFn) -> String {
                ...
                match param_count {
                    ...
                    2..=3 => {
                        explanation.push_str(&format!("{} parameters and ", param_count))
                    }
                    _ => {
                        explanation.push_str(&format!("{} parameters and ", param_count))
                    }
                ...
                {
                    explanation
                        .push_str("It operates on an instance of its type (method). ");
                } else {
            ...
            fn explain_struct(node: &ItemStruct) -> String {
                ...
                match &node.fields {
                    Fields::Named(named_fields) => {
                        ...
                        format!(
                            "Struct `{}` has {} named fields, representing a data structure with clear field names for better code readability and maintainability.",
                            name, field_count
                        )
                    }
                    Fields::Unnamed(unnamed_fields) => {
                        ...
                        format!(
                            "Struct `{}` is a tuple struct with {} unnamed fields, useful for simple data aggregation where field names aren't needed.",
                            name, field_count
                        )
                    }
                    Fields::Unit => {
                        format!(
                            "Struct `{}` is a unit struct with no fields, often used as a marker type or for implementing traits.",
                            name
                        )
                    }
                }
            ...
            fn explain_trait(node: &ItemTrait) -> String {
                ...
                let method_count = node.items.len();
                format!(
                    "Trait `{}` defines an interface with {} methods that types can implement to provide specific functionality. It represents a capability that implementing types must provide.",
                    name, method_count
                )
            }
        }
            ...
            if func.complexity > 10 {
                issues
                    .push(CodeIssue {
                        ...
                        message: format!(
                            "High complexity function (score: {})", func.complexity
                        ),
            ...
            if struct_info.fields.len() > 15 {
                issues
                    .push(CodeIssue {
                        ...
                        message: format!(
                            "Large struct with {} fields", struct_info.fields.len()
                        ),
                        explanation: "Large structs can be difficult to work with and may indicate a need for better organization"
                            .to_string(),
                        suggestion: "Consider splitting into smaller, more focused structs"
                            .to_string(),
                    });
            }
        }
    ...
    fn generate_recommendations(
        ...
        if functions
            .iter()
            .any(|f| f.patterns_used.contains(&"Error Propagation".to_string()))
        {
            recommendations
                .push(Recommendation {
                    category: "error_handling".to_string(),
                    priority: "medium".to_string(),
                    title: "Consider using thiserror for better error handling"
                        .to_string(),
                    description: "Using thiserror provides better error messages and easier error handling"
                        .to_string(),
                    code_example: Some(
                    ...
                    benefits: vec![
                        "Better error messages".to_string(), "Easier error handling"
                        .to_string(), "Consistent error types".to_string(),
                    ],
                });
        }
        if structs
            .iter()
            .any(|s| s.patterns_used.contains(&"Serialization Support".to_string()))
        {
            recommendations
                .push(Recommendation {
                    ...
                    description: "Adding validation ensures data integrity when serializing/deserializing"
                        .to_string(),
                    code_example: Some(
                    ...
                    benefits: vec![
                        "Data integrity".to_string(), "Better error messages"
                        .to_string(), "Security improvements".to_string(),
                    ],
                });
        }
        recommendations
    ...
    fn identify_learning_opportunities(
        ...
        let async_functions = functions
            .iter()
            .filter(|f| f.patterns_used.contains(&"Async Function".to_string()))
            .count();
        if async_functions == 0 {
            opportunities
                .push(LearningOpportunity {
                    ...
                    explanation: "Async programming is becoming increasingly important in Rust for building scalable applications"
                        .to_string(),
                });
        }
        let error_handling = functions
            .iter()
            .filter(|f| f.patterns_used.contains(&"Error Propagation".to_string()))
            .count();
        if error_handling == 0 {
            opportunities
                .push(LearningOpportunity {
                    ...
                    resources: vec![
                        ...
                        .to_string(), "https://docs.rs/thiserror/latest/thiserror/"
                        .to_string(),
                    ],
                    explanation: "Proper error handling is crucial for robust Rust applications"
                        .to_string(),
                });
        }
        let iterator_usage = functions
            .iter()
            .filter(|f| f.patterns_used.contains(&"Iterator Methods".to_string()))
            .count();
        if iterator_usage == 0 {
            opportunities
                .push(LearningOpportunity {
                    ...
                    explanation: "Iterators provide powerful and efficient ways to work with collections"
                        .to_string(),
                });
        }
        opportunities
    ...
    fn explain_pattern(&self, pattern: &str) -> String {
        match pattern {
            "Pattern Matching" => {
                "Pattern matching allows you to destructure and match values against patterns, providing a powerful way to handle different cases in your code."
                    .to_string()
            }
            "If-Let Pattern" => {
                "If-let is a concise way to handle optional values and single-case matches, reducing boilerplate compared to full match statements."
                    .to_string()
            }
            "Iterator Methods" => {
                "Iterator methods like map, filter, and fold provide functional programming patterns that make data transformation more concise and readable."
                    .to_string()
            }
            "Error Propagation" => {
                "The ? operator provides concise error propagation, automatically converting errors to the expected return type."
                    .to_string()
            }
            "Async Function" => {
                "Async functions allow you to write asynchronous code that looks like synchronous code, making concurrent programming more accessible."
                    .to_string()
            }
    ...
    fn pattern_benefits(&self, pattern: &str) -> Vec<String> {
        match pattern {
            ...
            "Iterator Methods" => {
                vec![
                    "More concise and readable code".to_string(), "Composable operations"
                    .to_string(), "Lazy evaluation for performance".to_string(),
                ]
            }
            "Error Propagation" => {
                vec![
    ...
    fn pattern_alternatives(&self, pattern: &str) -> Vec<String> {
        match pattern {
            "Pattern Matching" => {
                vec!["If-else chains".to_string(), "Method dispatch".to_string(),]
            }
            "Iterator Methods" => {
                vec![
                    "Traditional loops".to_string(), "Manual collection building"
                    .to_string(),
                ]
            }
            "Error Propagation" => {
                vec![
                    "Manual match statements".to_string(), "Panic on error".to_string(),
                ]
            }
...
impl Tool for RustMentorTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "An interactive learning and guidance system that analyzes your Rust code and provides personalized explanations, best practice suggestions, and learning recommendations. Perfect for developers at all levels looking to improve their Rust skills.",
            )
            .args(
                &[
                    Arg::new("input")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "🎓 {} - {}", "CargoMate RustMentor".bold().blue(), self.description()
            .cyan()
        );
        ...
        if verbose {
            ...
            println!(
                "   🔍 Identified {} patterns, {} issues, {} recommendations", analysis
                .patterns.len(), analysis.issues.len(), analysis.recommendations.len()
            );
        }
        match output_format {
            OutputFormat::Human => {
...
impl RustMentorTool {
        ...
        for func in functions {
            ...
            if detailed {
                if func.complexity > 1 {
                    println!(
                        "      Complexity: {} (higher = more complex)", func.complexity
                    );
                }
        ...
        for trait_info in traits {
            ...
            if detailed {
                ...
                if !trait_info.common_use_cases.is_empty() {
                    println!(
                        "      Use Cases: {}", trait_info.common_use_cases.join(", ")
                    );
                }
            }
            println!();
    ...
    fn explain_specific_target(&self, analysis: &CodeAnalysis, target: &str) {
        ...
        for struct_info in &analysis.structs {
            if struct_info.name == target {
                ...
                if !struct_info.patterns_used.is_empty() {
                    println!(
                        "   Patterns Used: {}", struct_info.patterns_used.join(", ")
                    );
                }
                return;
            }
    ...
    fn display_learning_opportunities(
        &self,
    ...
    fn display_table_analysis(&self, analysis: &CodeAnalysis) {
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Category", "Count", "Patterns",
            "Issues", "Learning"
        );
        println!("{}", "─".repeat(70));
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Functions", analysis.functions.len(),
            "-", "-", "-"
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Structs", analysis.structs.len(), "-",
            "-", "-"
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Traits", analysis.traits.len(), "-",
            "-", "-"
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Code Patterns", "-", analysis.patterns
            .len(), "-", "-"
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Issues", "-", "-", analysis.issues
            .len(), "-"
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Recommendations", "-", "-", "-",
            analysis.recommendations.len()
        );
        println!(
            "{:<20} {:<10} {:<10} {:<10} {:<10}", "Learning Ops", "-", "-", "-", analysis
