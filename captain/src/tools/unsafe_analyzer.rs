...
impl UnsafeVisitor {
    ...
    fn add_unsafe_usage(&mut self, unsafe_type: String, context: String, line: usize) {
        ...
        let (risk_level, justification_required) = self
            .assess_risk(&unsafe_type, &context);
    ...
    fn generate_suggestion(&self, unsafe_type: &str, context: &str) -> String {
        match unsafe_type {
            "raw_pointer_dereference" => {
                "Use safe alternatives like Box, Rc, Arc, or check for null pointers before dereferencing"
                    .to_string()
            }
            "static_mut_access" => {
                "Use thread-safe alternatives like Mutex, RwLock, or atomic operations"
                    .to_string()
            }
            "ffi_call" => {
                "Ensure proper error handling and memory management when interfacing with C code"
                    .to_string()
            }
            "transmute" => {
                "Use safe casting methods or ensure type compatibility before transmuting"
                    .to_string()
            }
            "union_access" => {
                "Use enums with proper discriminant checking instead of unions"
                    .to_string()
            }
            "unsafe_trait_impl" => {
                "Document safety requirements and provide safe wrapper methods"
                    .to_string()
            }
            "inline_assembly" => {
                "Minimize assembly usage and thoroughly document safety invariants"
                    .to_string()
            }
            ...
            _ => {
                "Review unsafe usage and ensure all safety invariants are maintained"
                    .to_string()
            }
        }
    }
...
impl<'ast> Visit<'ast> for UnsafeVisitor {
    fn visit_item_fn(&mut self, node: &'ast syn::ItemFn) {
        ...
        if self.in_unsafe_fn {
            let context = format!(
                "fn {}() -> {}", node.sig.ident, node.sig.output.to_token_stream()
            );
            self.add_unsafe_usage("unsafe_function".to_string(), context, 0);
        }
    ...
    fn visit_expr_unsafe(&mut self, node: &'ast syn::ExprUnsafe) {
        ...
        self.add_unsafe_usage("unsafe_block".to_string(), context, 0);
    ...
    fn visit_expr_call(&mut self, node: &'ast syn::ExprCall) {
        if self.in_unsafe_block || self.in_unsafe_fn {
            ...
            if call_str.contains("extern") || call_str.contains("::c_") {
                self.add_unsafe_usage("ffi_call".to_string(), call_str, 0);
            }
        }
    ...
    fn visit_expr_unary(&mut self, node: &'ast syn::ExprUnary) {
        if self.in_unsafe_block || self.in_unsafe_fn {
            ...
            if unary_str.contains('*')
                && (unary_str.contains("const") || unary_str.contains("mut"))
            {
    ...
    fn visit_expr_path(&mut self, node: &'ast syn::ExprPath) {
        if self.in_unsafe_block || self.in_unsafe_fn {
            ...
            if path_str.contains("static") && path_str.contains("mut") {
                self.add_unsafe_usage("static_mut_access".to_string(), path_str, 0);
            }
        }
...
impl UnsafeAnalyzerTool {
            ...
            if let Some(fn_name) = captures.get(1) {
                usages
                    .push(UnsafeUsage {
                        ...
                        suggestion: "Document safety requirements and provide safe wrapper functions"
                            .to_string(),
                    });
            }
        }
                ...
                if block_text.contains("transmute") {
                    usages
                        .push(UnsafeUsage {
                            ...
                            justification_required: true,
                            suggestion: "Use safe casting methods or ensure type compatibility"
                                .to_string(),
                        });
                }
                if block_text.contains("*const") || block_text.contains("*mut") {
                    usages
                        .push(UnsafeUsage {
                            ...
                            risk_level: "high".to_string(),
                            justification_required: true,
                            suggestion: "Use safe alternatives like Box, Rc, Arc"
                                .to_string(),
                        });
                }
                if block_text.contains("static mut") {
                    usages
                        .push(UnsafeUsage {
                            ...
                            risk_level: "high".to_string(),
                            justification_required: true,
                            suggestion: "Use thread-safe alternatives like Mutex, RwLock"
                                .to_string(),
                        });
                }
                if block_text.contains("extern") && block_text.contains("fn") {
                    usages
                        .push(UnsafeUsage {
                            ...
                            risk_level: "medium".to_string(),
                            justification_required: true,
                            suggestion: "Ensure proper error handling and memory management"
                                .to_string(),
                        });
                }
            }
    ...
    fn assess_risk(&self, usages: &[UnsafeUsage]) -> RiskAssessment {
        let critical_count = usages
            .iter()
            .filter(|u| u.risk_level == "critical")
            .count();
    ...
    fn generate_recommendations(
        ...
        if risk.overall_risk == "critical" {
            recommendations
                .push(
                    "🚨 CRITICAL: Review all unsafe code immediately - high risk of memory safety issues"
                        .to_string(),
                );
        }
        if usages.iter().any(|u| u.unsafe_type == "raw_pointer_dereference") {
            recommendations
                .push(
                    "Replace raw pointer operations with safe alternatives (Box, Rc, Arc, RefCell)"
                        .to_string(),
                );
        }
        if usages.iter().any(|u| u.unsafe_type == "static_mut_access") {
            recommendations
                .push(
                    "Replace static mut access with thread-safe alternatives (Mutex, RwLock, atomic types)"
                        .to_string(),
                );
        }
        if usages.iter().any(|u| u.unsafe_type == "ffi_call") {
            recommendations
                .push(
                    "Add comprehensive error handling and memory management for FFI calls"
                        .to_string(),
                );
        }
        if usages.iter().any(|u| u.unsafe_type == "transmute") {
            recommendations
                .push(
                    "Avoid transmute where possible, use safe casting methods"
                        .to_string(),
                );
        }
        recommendations
        ...
        recommendations
            .push("Create safe wrapper functions around unsafe operations".to_string());
        recommendations
            .push(
                "Use tools like Miri to test unsafe code under different scenarios"
                    .to_string(),
            );
        recommendations
            .push(
                "Consider gradual migration to safe alternatives where possible"
                    .to_string(),
            );
        recommendations
            .push("Run regular code reviews focusing on unsafe code blocks".to_string());
        recommendations
    }
    fn display_report(
        &self,
        report: &UnsafeAnalysisReport,
        ...
        match output_format {
            OutputFormat::Human => {
                println!(
                    "\n🔍 {} - Unsafe Code Analysis Report", "CargoMate UnsafeAnalyzer"
                    .bold().blue()
                );
                ...
                println!(
                    "  • Functions with Unsafe: {}", report.functions_with_unsafe
                );
                println!("  • Total Unsafe Blocks: {}", report.total_unsafe_blocks);
                println!(
                    "  • Overall Risk Level: {}", report.risk_assessment.overall_risk
                    .red()
                );
                println!(
                    "  • Risk Score: {:.1}/10", report.risk_assessment.risk_score
                );
                println!("\n📈 Risk Assessment:");
                println!(
                    "  • Critical Risk Functions: {}", report.risk_assessment
                    .critical_functions
                );
                println!(
                    "  • High Risk Functions: {}", report.risk_assessment
                    .high_risk_functions
                );
                println!(
                    "  • Medium Risk Functions: {}", report.risk_assessment
                    .medium_risk_functions
                );
                println!(
                    "  • Low Risk Functions: {}", report.risk_assessment
                    .low_risk_functions
                );
                println!("\n📋 Unsafe Statistics:");
                println!(
                    "  • Functions with Unsafe: {}", report.statistics
                    .functions_with_unsafe
                );
                println!("  • Unsafe Blocks: {}", report.statistics.unsafe_blocks);
                println!(
                    "  • Static Mut Access: {}", report.statistics.unsafe_static_access
                );
                println!(
                    "  • Pointer Operations: {}", report.statistics.unsafe_pointer_ops
                );
                ...
                if !report.unsafe_usage.is_empty() {
                    ...
                    for usage in &report.unsafe_usage {
                        ...
                        println!(
                            "  {} {}::{} - {} (Line {})", risk_icon, usage.file_path
                            .split('/').last().unwrap_or(& usage.file_path), usage
                            .function_name, usage.unsafe_type.red(), usage.line_number
                        );
                        if verbose {
                            println!("    📝 Context: {}", usage.context.dimmed());
                            if usage.justification_required {
                                println!(
                                    "    ⚠️  Justification Required: {}", "Yes".red()
                                );
                            }
                ...
                if report.unsafe_usage.is_empty() {
                    ...
                    println!(
                        "   Found {} unsafe usage(s) to review", report.unsafe_usage
                        .len()
                    );
                }
            }
            ...
            OutputFormat::Table => {
                println!(
                    "{:<30} {:<20} {:<15} {:<12} {:<15}", "File", "Function", "Type",
                    "Risk", "Justification"
                );
                println!("{}", "─".repeat(100));
                for usage in &report.unsafe_usage {
                    ...
                    let justification = if usage.justification_required {
                        "Required"
                    } else {
                        "Optional"
                    };
                    println!(
                        "{:<30} {:<20} {:<15} {:<12} {:<15}", file_name.chars().take(29)
                        .collect::< String > (), usage.function_name.chars().take(19)
                        .collect::< String > (), usage.unsafe_type.chars().take(14)
                        .collect::< String > (), usage.risk_level, justification
                    );
                }
            }
        }
...
impl Tool for UnsafeAnalyzerTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Analyze unsafe code usage in Rust projects, identifying potential \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "🔍 {} - Analyzing Unsafe Code", "CargoMate UnsafeAnalyzer".bold().blue()
        );
                ...
                Ok(usages) => {
                    for usage in usages {
                        let include_usage = match risk_threshold.as_str() {
                            ...
                            "high" => {
                                usage.risk_level == "critical" || usage.risk_level == "high"
                            }
                            "medium" => {
                                usage.risk_level == "critical" || usage.risk_level == "high"
        ...
        let recommendations = self
            .generate_recommendations(&all_usages, &risk_assessment);
...
impl UnsafeAnalyzerTool {
    fn generate_sarif_report(&self, report: &UnsafeAnalysisReport) -> Result<String> {
        let sarif = serde_json::json!(
            ...
            "https://schemastore.azurewebsites.net/schemas/json/sarif-2.1.0.json", "runs"
            : [{ "tool" : { "driver" : { "name" : "CargoMate UnsafeAnalyzer", "version" :
            "1.0.0", "informationUri" : "https://example.com", "rules" : [{ "id" :
            "unsafe-usage", "name" : "UnsafeCodeUsage", "shortDescription" : { "text" :
            "Unsafe code usage detected" }, "fullDescription" : { "text" :
            "Usage of unsafe Rust code that may compromise memory safety" }, "help" : {
            "text" :
            "Review unsafe code usage and ensure all safety invariants are maintained" },
            "properties" : { "category" : "security", "impact" : "high" } }] } },
            "results" : report.unsafe_usage.iter().map(| usage | { serde_json::json!({
            "ruleId" : "unsafe-usage", "level" : match usage.risk_level.as_str() {
            "critical" => "error", "high" => "warning", "medium" => "note", _ => "note"
            }, "message" : { "text" : format!("{}: {}", usage.unsafe_type, usage.context)
            }, "locations" : [{ "physicalLocation" : { "artifactLocation" : { "uri" :
            usage.file_path }, "region" : { "startLine" : usage.line_number } } }],
            "properties" : { "riskLevel" : usage.risk_level, "suggestion" : usage
            .suggestion } }) }).collect::< Vec < _ >> () }] }
        );
