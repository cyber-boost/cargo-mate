...
impl BloatCheckTool {
    ...
    fn analyze_binary_size(&self, binary_path: &str) -> Result<BinarySizeInfo> {
        if !Path::new(binary_path).exists() {
            return Err(
                ToolError::InvalidArguments(format!("Binary not found: {}", binary_path)),
            );
        }
            ...
            .output()
        {
            ...
            _ => {
                match ProcessCommand::new("size").arg("-B").arg(binary_path).output() {
                    Ok(output) if output.status.success() => {
                        String::from_utf8_lossy(&output.stdout).to_string()
                    }
                    _ => format!("{} {} {} {}", total_size, 0, 0, 0),
                }
            }
        };
        ...
        if analysis.total_size > 50 * 1024 * 1024 {
            suggestions
                .push(OptimizationSuggestion {
                    ...
                    suggestion: "Consider enabling link-time optimization (LTO) in release builds"
                        .to_string(),
                });
        }
        if analysis.text_size > 20 * 1024 * 1024 {
            suggestions
                .push(OptimizationSuggestion {
                    category: "Code Size".to_string(),
                    description: "Large text section".to_string(),
                    impact: "Medium".to_string(),
                    suggestion: "Review inlining decisions and consider #[inline(never)] for large functions"
                        .to_string(),
                });
        }
        if analysis.data_size > 10 * 1024 * 1024 {
            suggestions
                .push(OptimizationSuggestion {
                    category: "Data Size".to_string(),
                    description: "Large data section".to_string(),
                    impact: "Medium".to_string(),
                    suggestion: "Review static data usage and consider lazy initialization"
                        .to_string(),
                });
        }
        if analysis.bss_size > 5 * 1024 * 1024 {
            suggestions
                .push(OptimizationSuggestion {
                    category: "Memory Usage".to_string(),
                    description: "Large uninitialized data section".to_string(),
                    impact: "Low".to_string(),
                    suggestion: "Review large static arrays and consider dynamic allocation"
                        .to_string(),
                });
        }
        suggestions
            .push(OptimizationSuggestion {
                ...
                impact: "Low".to_string(),
                suggestion: "Use cargo build --release with strip = true in Cargo.toml"
                    .to_string(),
            });
        suggestions
            .push(OptimizationSuggestion {
    ...
    fn analyze_debug_vs_release(
        ...
        let ratio = if release_size > 0 {
            debug_size as f64 / release_size as f64
        } else {
            1.0
        };
...
impl Tool for BloatCheckTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        match output_format {
            OutputFormat::Human => {
                println!(
                    "📊 {} - {}", "Binary Size Analysis".bold(), self.description()
                    .cyan()
                );
                match self.analyze_binary_size(binary_path) {
                    Ok(analysis) => {
                        ...
                        println!(
                            "📏 Size: {}", self.format_size(analysis.total_size)
                            .green().bold()
                        );
                        println!(
                            "🔢 Symbols: {}", analysis.symbol_count.to_string().cyan()
                        );
                        if analysis.text_size > 0 || analysis.data_size > 0
                            || analysis.bss_size > 0
                        {
                            println!("\n📈 Section Sizes:");
                            if analysis.text_size > 0 {
                                println!(
                                    "  Text (code): {}", self.format_size(analysis.text_size)
                                );
                            }
                            if analysis.data_size > 0 {
                                println!(
                                    "  Data (initialized): {}", self.format_size(analysis
                                    .data_size)
                                );
                            }
                            if analysis.bss_size > 0 {
                                println!(
                                    "  BSS (uninitialized): {}", self.format_size(analysis
                                    .bss_size)
                                );
                            }
                        }
                        if let Some(baseline) = baseline_path {
                            match self.analyze_size_changes(binary_path, baseline) {
                                Ok(comparison) => {
                                    ...
                                    println!(
                                        "  Total size: {} ({:.1}%)", self.colorize_diff(comparison
                                        .size_diff, threshold * analysis.total_size as f64 / 100.0),
                                        (comparison.size_diff as f64 / comparison.baseline
                                        .total_size as f64 * 100.0)
                                    );
                                    if comparison.text_diff != 0 {
                                        println!(
                                            "  Text section: {} ({:.1}%)", self.colorize_diff(comparison
                                            .text_diff, threshold * analysis.text_size as f64 / 100.0),
                                            (comparison.text_diff as f64 / comparison.baseline.text_size
                                            as f64 * 100.0)
                                        );
                                    }
                                    if comparison.data_diff != 0 {
                                        println!(
                                            "  Data section: {} ({:.1}%)", self.colorize_diff(comparison
                                            .data_diff, threshold * analysis.data_size as f64 / 100.0),
                                            (comparison.data_diff as f64 / comparison.baseline.data_size
                                            as f64 * 100.0)
                                        );
                                    }
                                    if comparison.bss_diff != 0 {
                                        println!(
                                            "  BSS section: {} ({:.1}%)", self.colorize_diff(comparison
                                            .bss_diff, threshold * analysis.bss_size as f64 / 100.0),
                                            (comparison.bss_diff as f64 / comparison.baseline.bss_size
                                            as f64 * 100.0)
                                        );
                                    }
                                }
                            ...
                            match self.find_largest_symbols(binary_path) {
                                Ok(symbols) if !symbols.is_empty() => {
                                    ...
                                    for (i, symbol) in symbols.iter().enumerate() {
                                        println!(
                                            "  {}. {} ({} bytes) - {}", i + 1, symbol.name.cyan(),
                                            symbol.size.to_string().yellow(), symbol.symbol_type
                                        );
                                    }
                                }
                                Ok(_) => {
                        ...
                        if debug_compare {
                            ...
                            if Path::new(&debug_path).exists() {
                                match self
                                    .analyze_debug_vs_release(&debug_path, binary_path)
                                {
                                    Ok(comparison) => {
                                        println!("\n🔧 Debug vs Release Comparison:");
                                        println!(
                                            "  Debug build: {}", self.format_size(comparison.debug_size)
                                        );
                                        println!(
                                            "  Release build: {}", self.format_size(comparison
                                            .release_size)
                                        );
                                        println!("  Size ratio: {:.1}x", comparison.ratio);
                                        println!(
                                            "  Space savings: {}", self.format_size(comparison.savings)
                                            .green()
                                        );
                                    }
                                    Err(e) => {
                        ...
                        if report {
                            ...
                            println!(
                                "Total Size: {}", self.format_size(analysis.total_size)
                            );
                            println!("Symbol Count: {}", analysis.symbol_count);
                            println!(
                                "Text Section: {}", self.format_size(analysis.text_size)
                            );
                            println!(
                                "Data Section: {}", self.format_size(analysis.data_size)
                            );
                            println!(
                                "BSS Section: {}", self.format_size(analysis.bss_size)
                            );
                            if let Some(baseline) = baseline_path {
                                if let Ok(comparison) = self
                                    .analyze_size_changes(binary_path, baseline)
                                {
                                    println!("\nSize Changes:");
                                    println!(
                                        "Total: {}", self.format_diff(comparison.size_diff)
                                    );
                                    println!(
                                        "Text: {}", self.format_diff(comparison.text_diff)
                                    );
                                    println!(
                                        "Data: {}", self.format_diff(comparison.data_diff)
                                    );
                    ...
                    Err(e) => {
                        return Err(
                            ToolError::ExecutionFailed(
                                format!("Failed to analyze binary: {}", e),
                            ),
                        );
                    }
                }
            ...
            OutputFormat::Json => {
                ...
                let mut json_output = serde_json::json!(
                    { "binary" : analysis.path, "total_size" : analysis.total_size,
                    "text_size" : analysis.text_size, "data_size" : analysis.data_size,
                    "bss_size" : analysis.bss_size, "symbol_count" : analysis
                    .symbol_count, }
                );
                if let Some(baseline) = baseline_path {
                    if let Ok(comparison) = self
                        .analyze_size_changes(binary_path, baseline)
                    {
                        json_output["size_changes"] = serde_json::json!(
                            { "total_diff" : comparison.size_diff, "text_diff" :
                            comparison.text_diff, "data_diff" : comparison.data_diff,
                ...
                if show_symbols {
                    if let Ok(symbols) = self.find_largest_symbols(binary_path) {
                        json_output["largest_symbols"] = serde_json::to_value(&symbols)
                            .unwrap();
                    }
                }
                if optimize {
                    let suggestions = self.generate_optimization_suggestions(&analysis);
                    json_output["optimization_suggestions"] = serde_json::to_value(
                            &suggestions,
                        )
                        .unwrap();
                }
