...
impl EnvCheckTool {
    ...
    fn generate_recommendations(&self, report: &EnvironmentReport) -> Vec<String> {
        ...
        recommendations
            .push("🎯 Keep Rust updated to the latest stable version".to_string());
        recommendations
            .push(
            ...
            .map_or(false, |t| t.installed)
        {
            recommendations
                .push(
                    "🐳 Consider installing Docker for integration testing".to_string(),
                );
        }
...
impl Tool for EnvCheckTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        if report.checks_failed > 0 {
            ...
            println!(
                "\n⚠️  Environment check completed with {} warnings", report
                .checks_warning
            );
        } else {
...
impl EnvCheckTool {
    fn display_human_report(
        ...
        println!(
            "Checks: ✅ {} passed, ❌ {} failed, ⚠️  {} warnings", report
