...
impl CodeAnalyzer {
    ...
    fn parse_rust_functions(&self, file_path: &str) -> Result<Vec<FunctionMetrics>> {
        ...
        let function_regex = Regex::new(
                r"(?s)(?:pub\s+)?(?:async\s+)?(?:unsafe\s+)?fn\s+(\w+)\s*\(([^)]*)\)\s*(?:->\s*([^;{]*?))?\s*\{([^}]*)\}",
            )
            .unwrap();
        for captures in function_regex.captures_iter(&content) {
            ...
            let metrics = FunctionMetrics {
                ...
                is_public: function_name.starts_with("pub")
                    || content.contains(&format!("pub fn {}", function_name)),
    ...
    fn calculate_complexity(&self, code: &str) -> f64 {
        ...
        let branches = code.matches("if").count() as f64
            + code.matches("match").count() as f64;
    ...
    fn analyze_codebase(&self, path: &str) -> Result<CodeAnalysis> {
        ...
        if Path::new(path).is_file() {
            ...
            {
                all_functions
                    .extend(self.parse_rust_functions(&entry.path().to_string_lossy())?);
            }
        }
        ...
        let average_complexity = if total_functions > 0 {
            all_functions.iter().map(|f| f.complexity).sum::<f64>()
                / total_functions as f64
        } else {
            0.0
        };
        let largest_function = all_functions.iter().max_by_key(|f| f.lines).cloned();
        Ok(CodeAnalysis {
            total_functions,
            public_functions,
            async_functions,
    ...
    fn display_analysis(&self, analysis: &CodeAnalysis, verbose: bool) {
        ...
        if let Some(ref largest) = analysis.largest_function {
            println!(
                "  • Largest Function: {} ({} lines)", largest.name, largest.lines
            );
        }
        if verbose && !analysis.functions.is_empty() {
            ...
            for function in &analysis.functions {
                ...
                println!(
                    ...
                    .split('/').last().unwrap_or(& function.file), function.name.cyan(),
                    if function.is_public { "public".green() } else { "private".dimmed()
                    }, function.lines, complexity_color
                );
            }
        }
        println!("\n✅ Analysis complete!");
...
impl Tool for CodeAnalyzer {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Analyzes Rust code to provide metrics on function complexity, API surface, and code quality indicators.",
            )
            .args(
                &[
                    Arg::new("path")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "📊 {} - Analyzing Code Metrics", "CargoMate CodeAnalyzer".bold().blue()
        );
        ...
        match output_format.as_str() {
            ...
            "table" => {
                println!(
                    "{:<30} {:<10} {:<12} {:<8} {:<8}", "Function", "Lines",
                    "Complexity", "Params", "Public"
                );
                println!("{}", "─".repeat(70));
                for function in &analysis.functions {
                    println!(
                        "{:<30} {:<10} {:<12.2} {:<8} {:<8}", format!("{}::{}", function
                        .file.split('/').last().unwrap_or(""), function.name), function
                        .lines, function.complexity, function.parameters, if function
                        .is_public { "Yes" } else { "No" }
                    );
                }
            }
            _ => {
