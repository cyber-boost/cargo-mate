...
use colored::*;
use syn::{
    parse_file, visit::Visit, ItemFn, Lifetime, TypeReference, FnArg, ReturnType,
    GenericParam,
};
use quote::ToTokens;
...
impl LifetimeVisualizerTool {
    ...
    fn parse_lifetimes_in_file(
        &self,
        file_path: &str,
    ) -> Result<Vec<FunctionLifetimeInfo>> {
    ...
    fn build_lifetime_graph(
        &self,
        functions: &[FunctionLifetimeInfo],
    ) -> Result<LifetimeGraph> {
        ...
        for function in functions {
            for constraint in &function.constraints {
                if let Some((from_lifetime, to_lifetime)) = self
                    .parse_constraint(constraint)
                {
                    let from_id = nodes
                        .iter()
                        .find(|n| n.name == from_lifetime)
                        .map(|n| n.id);
                    let to_id = nodes
                        .iter()
                        .find(|n| n.name == to_lifetime)
                        .map(|n| n.id);
        ...
        for function in functions {
            if function.lifetimes.len() > 1 && function.constraints.is_empty() {
                issues
                    .push(LifetimeIssue {
                        ...
                        line: 0,
                        suggestion: format!(
                            "Add lifetime constraints like {}: {}", function
                            .lifetimes[0], function.lifetimes[1]
                        ),
                    });
            }
            if function.output_lifetimes.len() > 0 {
                let unconstrained_outputs: Vec<_> = function
                    .output_lifetimes
                    .iter()
                ...
                if !unconstrained_outputs.is_empty() {
                    issues
                        .push(LifetimeIssue {
                            function: function.name.clone(),
                            issue: "Return lifetime not tied to input lifetime"
                                .to_string(),
                            severity: "High".to_string(),
                            line: 0,
                            suggestion: "Use HRTB (for<'a>) or tie return lifetime to input"
                                .to_string(),
                        });
                }
            }
            if function.lifetimes.len() == 1 && function.input_lifetimes.len() == 1
                ...
                && function.input_lifetimes[0] == function.output_lifetimes[0]
            {
                issues
                    .push(LifetimeIssue {
                        function: function.name.clone(),
                        issue: "Unnecessary explicit lifetime - elision would work"
                            .to_string(),
                        severity: "Low".to_string(),
                        line: 0,
                        suggestion: "Remove explicit lifetimes and let the compiler elide them"
                            .to_string(),
                    });
            }
        }
    ...
    fn generate_graphviz_diagram(&self, graph: &LifetimeGraph) -> Result<String> {
        self.generate_dot_diagram(graph)
    }
    ...
    fn suggest_lifetime_improvements(
        &self,
        issues: &[LifetimeIssue],
    ) -> Vec<LifetimeSuggestion> {
            ...
            match issue.severity.as_str() {
                "High" => {
                    if issue.issue.contains("Return lifetime not tied") {
                        suggestions
                            .push(LifetimeSuggestion {
                                category: "HRTB".to_string(),
                                description: "Use Higher-Ranked Trait Bounds".to_string(),
                                before: "fn cache_get<'k>(key: &'k Key) -> Option<&'k Value>"
                                    .to_string(),
                                after: "fn cache_get(key: &Key) -> Option<&Value>"
                                    .to_string(),
                    ...
                    if issue.issue.contains("Multiple lifetimes") {
                        suggestions
                            .push(LifetimeSuggestion {
                                ...
                                description: "Add explicit lifetime constraints"
                                    .to_string(),
                                before: "fn process<'a, 'b>(data: &'a mut Vec<T>, config: &'b Config)"
                                    .to_string(),
                                after: "fn process<'a, 'b: 'a>(data: &'a mut Vec<T>, config: &'b Config)"
                                    .to_string(),
...
impl Tool for LifetimeVisualizerTool {
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        let filtered_functions: Vec<_> = if let Some(func_name) = specific_function {
            functions.into_iter().filter(|f| f.name == *func_name).collect()
        } else {
            functions
        };
        ...
        match output_format {
            OutputFormat::Human => {
                println!(
                    "🔗 {} - {}", "Lifetime Analysis Report".bold(), self.description()
                    .cyan()
                );
                println!("\n📁 Files Analyzed: {}", input.bold());
                println!(
                    "🔍 Functions Found: {}", filtered_functions.len().to_string()
                    .cyan()
                );
                let total_lifetimes: usize = filtered_functions
                    .iter()
                    .map(|f| f.lifetimes.len())
                    .sum();
                let total_constraints: usize = filtered_functions
                    .iter()
                    .map(|f| f.constraints.len())
                    .sum();
                println!("\n📊 Lifetime Summary:");
                ...
                if borrow_check {
                    ...
                    for function in &filtered_functions {
                        if function.name.contains("process")
                            || function.name.contains("cache")
                        {
                            match self.analyze_borrow_patterns(input) {
                                Ok(analysis) => {
                                    println!(
                                        "• Immutable borrows: {}", analysis.immutable_borrows
                                    );
                                    println!(
                                        "• Mutable borrows: {}", analysis.mutable_borrows
                                    );
                                    println!(
                                        "• Borrow checker errors: {}", analysis.borrow_errors
                                    );
                            ...
                            match self.generate_visualization(&graph, format) {
                                Ok(visualization) => {
                                    ...
                                    if format == "mermaid" {
                                        ...
                                        println!(
                                            "Generated {} diagram with {} nodes and {} edges", format,
            ...
            OutputFormat::Json => {
                ...
                let mut json_output = serde_json::json!(
                    { "files_analyzed" : input, "functions_analyzed" : filtered_functions
                    .len(), "total_lifetimes" : filtered_functions.iter().map(| f | f
                    .lifetimes.len()).sum::< usize > (), "functions" :
                    filtered_functions, }
                );
                if !issues.is_empty() {
                ...
                if visualize {
                    if let Ok(graph) = self.build_lifetime_graph(&filtered_functions) {
                        json_output["lifetime_graph"] = serde_json::to_value(&graph)
                            .unwrap();
                    }
                }
            ...
            OutputFormat::Table => {
                ...
                if visualize {
                    println!(
                        "│ Visualization: {:<35} │", format!("✓ ({})", format)
                    );
                }
                println!(
                    "└────────────────────────────────────────────────────┘"
