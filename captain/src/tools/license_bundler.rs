...
impl LicenseBundlerTool {
    ...
    fn extract_licenses_from_cargo_toml(
        &self,
        toml_path: &str,
    ...
    fn fetch_license_from_registry(
        &self,
        package_name: &str,
        version: &str,
    ) -> Result<Option<String>> {
        let common_licenses = vec![
            ("serde", "MIT OR Apache-2.0"), ("tokio", "MIT"), ("anyhow",
            "MIT OR Apache-2.0"), ("thiserror", "MIT OR Apache-2.0"), ("clap",
            "MIT OR Apache-2.0"), ("regex", "MIT OR Apache-2.0"), ("rand",
            "MIT OR Apache-2.0"), ("chrono", "MIT OR Apache-2.0"), ("reqwest",
            "MIT OR Apache-2.0"), ("serde_json", "MIT OR Apache-2.0"),
        ];
    ...
    fn assess_license_compatibility(
        &self,
        ...
        for license_info in licenses {
            ...
            if license_upper.contains("GPL") && license_upper.contains("PROPRIETARY") {
                incompatible
                    .push(
                        format!(
                            "{}: GPL + Proprietary incompatible", license_info
                            .package_name
                        ),
                    );
            }
            if license_upper.contains("AGPL") {
                warnings
                    .push(
                        format!(
                            "{}: AGPL requires distribution of source code", license_info
                            .package_name
                        ),
                    );
            }
    ...
    fn categorize_license_risk(&self, license: &str) -> String {
        ...
        if license_upper.contains("GPL") || license_upper.contains("AGPL") {
            "high".to_string()
        } else if license_upper.contains("PROPRIETARY")
            || license_upper.contains("COMMERCIAL")
        {
            "medium".to_string()
        ...
        for license_info in licenses {
            if let Some(license_text) = &license_info.license_text {
                let file_name = format!(
                    "{}-{}.license", license_info.package_name, license_info.version
                    .replace(".", "_")
                );
...
",
                        license_info.package_name, license_info.version, license_info
                        .license, license_info.source, license_info.compatibility,
...
",
                        license_info.package_name, license_info.version, license_info
                        .package_name, license_info.version, license_info.license,
                        license_info.source
                    ),
                );
            if let Some(license_text) = &license_info.license_text {
...
", license_text),
    ...
    fn calculate_statistics(&self, licenses: &[LicenseInfo]) -> LicenseStatistics {
        ...
        let most_common_license = by_license_type
            .iter()
        ...
        let compatibility_score = if compatibility.overall_compatible {
            if compatibility.incompatible_licenses.is_empty() { 100.0 } else { 75.0 }
        } else {
            0.0
        };
        LicenseStatistics {
    ...
    fn generate_recommendations(
        ...
        if !compatibility.overall_compatible {
            recommendations
                .push(
                    "🚨 CRITICAL: Resolve incompatible license combinations before distribution"
                        .to_string(),
                );
        }
        if compatibility.copyleft_count > 0 {
            recommendations
                .push(
                    "⚠️ Review copyleft licenses - may require source code distribution"
                        .to_string(),
                );
        }
        let unlicensed_count = licenses
            .iter()
            .filter(|l| l.license.is_empty() || l.license == "Unknown")
            .count();
        if unlicensed_count > 0 {
            recommendations
                .push(
                    format!(
                        "📋 {} packages have unknown licenses - investigate and document",
                        unlicensed_count
                    ),
                );
        }
        recommendations
            .push("📄 Include license bundle in project distribution".to_string());
        recommendations
            .push(
                "🔄 Regularly update license information when dependencies change"
                    .to_string(),
            );
        recommendations
            .push(
                "⚖️ Consult legal counsel for commercial distribution".to_string(),
            );
        recommendations
    }
    fn display_report(
        &self,
        report: &LicenseBundleReport,
        ...
        match output_format {
            OutputFormat::Human => {
                println!(
                    "\n📄 {} - License Bundler Report", "CargoMate LicenseBundler"
                    .bold().blue()
                );
                println!("{}", "═".repeat(60).blue());
                println!("\n📊 Summary:");
                println!(
                    "  • Dependencies Analyzed: {}", report.dependencies_analyzed
                );
                println!("  • Licenses Found: {}", report.licenses_found);
                println!(
                    "  • Licensed Packages: {}", report.statistics.licensed_packages
                );
                println!(
                    "  • Unlicensed Packages: {}", report.statistics
                    .unlicensed_packages
                );
                println!(
                    "  • Compatibility Score: {:.1}%", report.statistics
                    .compatibility_score
                );
                println!("\n📈 License Compatibility:");
                let compat = &report.compatibility_report;
                println!(
                    "  • Overall Compatible: {}", if compat.overall_compatible {
                    "✅ Yes".green() } else { "❌ No".red() }
                );
                ...
                if verbose {
                    ...
                    for license_info in &report.licenses {
                        ...
                        println!(
                            "  {} {} v{} - {}", risk_icon, license_info.package_name,
                            license_info.version, license_info.license
                        );
                    }
                }
            ...
            OutputFormat::Table => {
                println!(
                    "{:<30} {:<15} {:<20} {:<12} {:<10}", "Package", "Version",
                    "License", "Risk", "Compatible"
                );
                println!("{}", "─".repeat(95));
                for license_info in &report.licenses {
                    ...
                    println!(
                        "{:<30} {:<15} {:<20} {:<12} {:<10}", license_info.package_name
                        .chars().take(29).collect::< String > (), license_info.version,
                        license_info.license.chars().take(19).collect::< String > (),
                        license_info.risk_level, compatible
                    );
                }
            }
        }
...
impl Tool for LicenseBundlerTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Generate comprehensive license bundle for all project dependencies, \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "📄 {} - Generating License Bundle", "CargoMate LicenseBundler".bold()
            .blue()
        );
        ...
        if !Path::new(lock_path).exists() {
            return Err(
                ToolError::InvalidArguments(
                    "Cargo.lock not found - run 'cargo update' first".to_string(),
                ),
            );
        }
        ...
        for dep in &mut dependencies {
            if let Some(license) = toml_licenses.get(&dep.name) {
                ...
                if let Ok(Some(license)) = self
                    .fetch_license_from_registry(&dep.name, &dep.version)
                {
        ...
        for dep in &dependencies {
            ...
            let license_name = dep
                .license
                .clone()
                .unwrap_or_else(|| "Unknown".to_string());
            let compatibility = if license_name.contains("GPL")
                || license_name.contains("AGPL")
            {
                "copyleft"
            ...
            licenses
                .push(LicenseInfo {
                    ...
                    compatibility,
                    risk_level: self
                        .categorize_license_risk(
                            &dep.license.clone().unwrap_or_default(),
                        ),
                });
        }
        ...
        let recommendations = self
            .generate_recommendations(&licenses, &compatibility_report);
        ...
        if ci_mode {
            if report.compatibility_report.overall_compatible {
                ...
                println!(
                    "::error title=License Compatibility Issues::Found incompatible license combinations"
                );
            }
            if report.statistics.unlicensed_packages > 0 {
                println!(
                    "::warning title=Unlicensed Packages::Found {} packages without license information",
                    report.statistics.unlicensed_packages
                );
            }
        }
