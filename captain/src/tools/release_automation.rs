...
impl ReleaseAutomationTool {
            ...
            if line.trim().starts_with("version = ") {
                let version = line
                    ...
                    .ok_or_else(|| ToolError::ExecutionFailed(
                        "Cannot parse version from Cargo.toml".to_string(),
                    ))?;
    ...
    fn bump_version(&self, current: &str, bump_type: &str) -> Result<String> {
        ...
        if parts.len() != 3 {
            return Err(
                ToolError::ExecutionFailed(
                    format!("Invalid version format: {}", current),
                ),
            );
        }
        ...
        let patch: u32 = parts[2]
            .parse()
            .map_err(|_| ToolError::ExecutionFailed(
                "Invalid patch version".to_string(),
            ))?;
    ...
    fn update_version_in_file(
        ...
        if dry_run {
            println!(
                "📝 Would update {}: {} -> {}", file_path.cyan(), old_version.red(),
                new_version.green()
            );
        } else {
            fs::write(file_path, new_content)
                .map_err(|e| ToolError::ExecutionFailed(
                    format!("Cannot write {}: {}", file_path, e),
                ))?;
            println!(
                "✅ Updated {}: {} -> {}", file_path.cyan(), old_version.red(),
                new_version.green()
            );
        }
        Ok(())
    ...
    fn generate_changelog(
        ...
        let changelog = match git_log {
            ...
            _ => "Changelog generation failed - git log not available".to_string(),
        };
        let full_changelog = format!(
            "# Release {}\n\n{}\n\n## Previous Version: {}\n", to_version, changelog,
            from_version
        );
        if dry_run {
            ...
            fs::write(changelog_path, new_content)
                .map_err(|e| ToolError::ExecutionFailed(
                    format!("Cannot write changelog: {}", e),
                ))?;
    ...
    fn run_pre_release_checks(&self, dry_run: bool) -> Result<()> {
        ...
        let checks = [
            ...
            (
                "Audit",
                "cargo audit --quiet 2>/dev/null || echo 'cargo-audit not installed'",
            ),
        ];
        for (name, command) in &checks {
            ...
            if result.status.success() {
                ...
                println!(
                    "   ❌ {} failed: {}", name.red(), error.lines().next()
    ...
    fn create_git_tag(&self, version: &str, dry_run: bool) -> Result<()> {
        ...
        if dry_run {
            ...
            let result = ProcessCommand::new("git")
                ...
                .output()
                .map_err(|e| ToolError::ExecutionFailed(
                    format!("Failed to create git tag: {}", e),
                ))?;
            if result.status.success() {
                ...
                let error = String::from_utf8_lossy(&result.stderr);
                return Err(
                    ToolError::ExecutionFailed(format!("Git tag failed: {}", error)),
                );
            }
        }
        Ok(())
    ...
    fn publish_to_crates_io(&self, dry_run: bool) -> Result<()> {
        if dry_run {
            ...
            let result = ProcessCommand::new("cargo")
                ...
                .output()
                .map_err(|e| ToolError::ExecutionFailed(
                    format!("Failed to run cargo publish dry-run: {}", e),
                ))?;
            if !result.status.success() {
                let error = String::from_utf8_lossy(&result.stderr);
                return Err(
                    ToolError::ExecutionFailed(
                        format!("Cargo publish dry-run failed: {}", error),
                    ),
                );
            }
            println!(
                "✅ Dry-run successful! Run without --dry-run to actually publish"
            );
        }
        Ok(())
    }
    fn create_release_plan(
        ...
        steps
            .push(
                format!(
                    "Update version from {} to {} in {} files", current_version,
                    new_version, files_to_update.len()
                ),
            );
        ...
        let changes = vec![
            ...
            "Generate changelog".to_string(), "Create git tag".to_string(),
        ];
        Ok(ReleasePlan {
            current_version,
            new_version,
    ...
    fn execute_release_plan(
        ...
        println!(
            "📊 Release: {} -> {}", plan.current_version.red(), plan.new_version
            .green()
        );
...
impl Tool for ReleaseAutomationTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Handle the complete release workflow: version bumping, changelog generation, git tagging, and publishing to crates.io",
            )
            .args(
                &[
                    Arg::new("patch")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        if verbose {
            println!(
                "🚀 {} - Automating release process", "CargoMate ReleaseAutomation"
                .bold().blue()
            );
        }
        if !Path::new(".git").exists() {
            return Err(
                ToolError::ExecutionFailed("Not in a git repository".to_string()),
            );
        }
