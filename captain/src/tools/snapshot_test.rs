...
impl SnapshotTestTool {
    ...
    fn parse_snapshot_calls(&self, content: &str, file_path: &str) -> Vec<SnapshotData> {
        ...
        let patterns = vec![
            ...
            r#"snapshot!\s*\(\s*"([^"]+)"\s*,"#, r#"snapshot_\w+!\s*\(\s*"([^"]+)"\s*,"#,
        ];
        ...
        for entry in fs::read_dir(snapshots_dir)? {
            ...
            if path.is_file() {
                if let Some(file_name) = path.file_name() {
                    ...
                    if file_name_str.ends_with(".snap")
                        || file_name_str.contains("snapshot")
                    {
    ...
    fn display_report(
        ...
        match output_format {
            OutputFormat::Human => {
                ...
                if report.failed > 0 {
                    ...
                    if report.summary.update_mode {
                        ...
                        println!(
                            "  • Review failed snapshots and update test expectations"
                        );
                        println!(
                            "  • Use --update to automatically update all snapshots"
                        );
                    }
                }
                if report.new_snapshots > 0 {
                    ...
                    println!(
                        "  • Review new snapshot files in {}", report.summary
                        .snapshots_directory
                    );
                }
            }
            ...
            OutputFormat::Table => {
                println!(
                    "{:<40} {:<15} {:<10} {:<10}", "Snapshot", "Status", "Old", "New"
                );
                println!("{}", "─".repeat(80));
                for snapshot in &report.snapshots {
                    ...
                    let new_size = snapshot
                        ...
                        .unwrap_or("-".to_string());
                    println!(
                        "{:<40} {:<15} {:<10} {:<10}", snapshot.name, status, old_size,
                        new_size
                    );
                }
            }
...
impl Tool for SnapshotTestTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Enhanced snapshot testing tool with visual diffs, update modes, \
...
EXAMPLES:
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        for snapshot in &all_snapshots {
            let snapshot_file_name = self
                .generate_snapshot_name(&snapshot.name, &snapshot.name);
            let snapshot_key = format!(
                "{}_{}", snapshot.path.replace("/", "_").replace("\\", "_"),
                snapshot_file_name
            );
            if let Some(existing_content) = existing_snapshots.get(&snapshot_key) {
                let mock_new_content = format!(
                    "Mock content for {}\nGenerated: {}\n", snapshot.name,
                    chrono::Utc::now().to_rfc3339()
                );
                if let Some(diff) = self
                    .compare_snapshots(existing_content, &mock_new_content)
                {
                    if update && !new_only {
        ...
        if ci_mode {
            ...
            if report.failed > 0 {
                println!(
                    "::error title=Snapshot Tests Failed::{} snapshots failed", report
                    .failed
                );
            }
        }
