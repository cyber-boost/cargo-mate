...
use std::path::Path;
use syn::{
    parse_file, File, Item, ItemStruct, Fields, Field, Type, PathSegment, Ident,
    visit::Visit,
};
use quote::quote;
...
impl BuilderGenTool {
        ...
        field
            ...
            .any(|attr| {
                attr.path()
                    .segments
                    .iter()
                    .any(|seg| { seg.ident == "builder" || seg.ident == "default" })
            })
    }
    ...
    fn extract_validation_from_nested_meta(
        &self,
        ...
        match meta {
            ...
            syn::Meta::NameValue(name_value) => {
                if let Some(ident) = name_value.path.get_ident() {
                    let rule = format!(
                        "{} = {}", ident, self.expr_to_string(& name_value.value)
                    );
                    rules.push(rule);
                }
            }
    ...
    fn generate_builder_impl(
        ...
        for field in &struct_info.fields {
            if !field.is_optional && !field.has_default {
                code.push_str(
                    &format!(
                        "        let {} = self.{}.ok_or(BuilderError::MissingField(\"{}\"))?;\n",
                ...
                code.push_str(
                    &format!(
                        "        let {} = self.{}.unwrap_or_else(|| {});\n", field.name,
                        field.name, self.get_default_value_for_type(& field.ty)
                    ),
                );
            }
        }
        ...
        code.push_str(
            "        // if !is_valid(&result) { return Err(BuilderError::ValidationError(\"...\".to_string())); }\n\n",
        );
    ...
    fn generate_validation_code(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        for field in &struct_info.fields {
            if !field.validation_rules.is_empty() {
                ...
                code.push_str(
                    &format!(
                        "fn validate_{}(value: &{}) -> Result<(), BuilderError> {{\n",
                        field.name, field.ty
                    ),
                );
                for rule in &field.validation_rules {
                    if rule.contains("length") {
                        ...
                        code.push_str(
                            "        return Err(BuilderError::ValidationError(\"Field cannot be empty\".to_string()));\n",
                        );
                        code.push_str("    }\n");
    ...
    fn generate_conversion_impl(&self, struct_info: &StructInfo) -> Result<String> {
        ...
        code.push_str(
            "        builder.build().unwrap_or_else(|_| panic!(\"Failed to build from valid builder\"))\n",
        );
        code.push_str("    }\n");
        code.push_str("}\n\n");
        code.push_str(
            &format!("impl TryFrom<{}> for {} {{\n", builder_name, struct_name),
        );
        code.push_str("    type Error = BuilderError;\n\n");
        code.push_str(
            &format!(
                "    fn try_from(builder: {}) -> Result<Self, Self::Error> {{\n",
                builder_name
            ),
        );
        ...
        code.push_str("    }\n");
        code.push_str("}\n\n");
        let all_have_defaults = struct_info
            .fields
            .iter()
            .all(|f| f.is_optional || f.has_default);
    ...
    fn generate_comprehensive_builder(
        &self,
        struct_info: &StructInfo,
        derive_attrs: &str,
        method_prefix: &str,
        ...
        let mut code = format!(
            "// Comprehensive Builder Pattern for {}\n\n", struct_info.name
        );
    ...
    fn is_complex_type(&self, ty: &str) -> bool {
        !matches!(
            ty, "String" | "i32" | "i64" | "u32" | "u64" | "bool" | "f32" | "f64" |
            "Option<T>"
        )
    }
}
impl Tool for BuilderGenTool {
    ...
    fn command(&self) -> Command {
        Command::new(self.name())
            ...
            .long_about(
                "Analyze struct definitions and generate comprehensive builder patterns with validation, optional fields, fluent API methods, and support for nested builders. Perfect for complex structs with many optional fields.",
            )
            .args(
                &[
                    Arg::new("input")
    ...
    fn execute(&self, matches: &ArgMatches) -> Result<()> {
        ...
        println!(
            "🔧 {} - {}", "CargoMate BuilderGen".bold().blue(), self.description()
            .cyan()
        );
        ...
        for struct_info in &structs {
            ...
            let builder_code = self
                .generate_comprehensive_builder(
                    struct_info,
                    derive_attrs,
                    method_prefix,
                )?;
        ...
        match output_format {
            OutputFormat::Human => {
                println!(
                    "  ✅ Generated builder patterns for {} structs", structs.len()
                );
                println!("     → {}", output.cyan());
                if validation {
            ...
            OutputFormat::Json => {
                let result = serde_json::json!(
                    { "input" : input, "output" : output, "structs_processed" : structs
                    .len(), "validation_enabled" : validation, "nested_enabled" : nested,
                    "method_prefix" : method_prefix, "derive_attrs" : derive_attrs,
                    "struct_names" : structs.iter().map(| s | s.name.clone()).collect::<
                    Vec < _ >> (), "code_preview" : all_code.lines().take(10).collect::<
                    Vec < _ >> ().join("\n") }
                );
            ...
            OutputFormat::Table => {
                println!(
                    "{:<20} {:<15} {:<8} {:<8} {:<10}", "Struct", "Input", "Valid",
                    "Nested", "Prefix"
                );
                println!("{}", "─".repeat(65));
                for struct_info in &structs {
                    println!(
                        "{:<20} {:<15} {:<8} {:<8} {:<10}", struct_info.name,
                        Path::new(input).file_name().unwrap_or_default()
                        .to_string_lossy(), if validation { "Yes" } else { "No" }, if
                        nested { "Yes" } else { "No" }, method_prefix
                    );
                }
            }
        }
