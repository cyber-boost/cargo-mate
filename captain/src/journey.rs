...
impl JourneyMarketplace {
    pub fn publish(name: &str, tags: Vec<String>) -> Result<String> {
        if std::env::var("CARGO_MATE_MARKETPLACE").is_ok() {
            let email = std::env::var("CARGO_MATE_EMAIL")
                .unwrap_or_else(|_| "user@cargo.do".to_string());
            let category = std::env::var("CARGO_MATE_CATEGORY")
                .unwrap_or_else(|_| "getting-started".to_string());
            return Self::publish_to_marketplace(name, &email, tags, &category);
        }
        Self::publish_to_gist(name, tags)
    }
    pub fn publish_to_gist(name: &str, tags: Vec<String>) -> Result<String> {
        let journey_file = dirs::home_dir()
            .context("Could not find home directory")?
            .join(".shipwreck")
            .join("journeys")
    ...
    pub fn publish_to_marketplace(
        ...
        let journey_file = dirs::home_dir()
            ...
            .join(format!("{}.json", name));
        if !journey_file.exists() {
            return Err(anyhow::anyhow!("Journey '{}' not found locally", name));
        }
        let content = fs::read_to_string(&journey_file)?;
        let mut journey: Journey = serde_json::from_str(&content)?;
        journey.tags = tags;
        journey.tags.push(category.to_string());
        let json = serde_json::to_string_pretty(&journey)?;
        println!(
            "📤 Publishing journey '{}' to Cargo Mate Marketplace...", name.cyan()
        );
        println!("   📧 Using email: {}", email.cyan());
        println!("   🏷️  Category: {}", category.cyan());
        println!("   📋 Tags: {}", journey.tags.join(", ").cyan());
        let journey_data = serde_json::json!(
            { "email" : email, "journey_data" : { "journey_id" : journey.name
            .to_lowercase().replace(" ", "_"), "title" : journey.name, "description" :
            journey.description, "category" : category, "tags" : journey.tags,
            "difficulty" : "intermediate", "estimated_duration" : 30, "target_audience" :
            "Rust developers", "prerequisites" : "Basic Rust knowledge" }, "files" : [{
            "filename" : format!("{}.json", journey.name), "type" : "json", "content" :
            json }] }
        );
        let temp_file = std::env::temp_dir()
            .join(format!("cargo-mate-marketplace-{}.json", name));
        fs::write(&temp_file, journey_data.to_string())?;
        let output = Command::new("curl")
            .args(
                &[
                    ...
                    "-d",
                    &journey_data.to_string(),
                    "https://cargo.do/api/marketplace/publish-journey",
                ],
            )
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
        ...
        if !output.status.success() {
            let error = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Marketplace API error: {}", error));
        }
        let response = String::from_utf8_lossy(&output.stdout);
        if response.trim().is_empty() {
            return Err(anyhow::anyhow!("Marketplace API returned empty response"));
        }
        let api_response: serde_json::Value = serde_json::from_str(&response)
            .map_err(|e| {
                anyhow::anyhow!("Failed to parse marketplace response: {}", e)
            })?;
        if api_response["success"].as_bool().unwrap_or(false) {
            let journey_id = api_response["journey_id"]
                .as_u64()
                .ok_or_else(|| {
                    anyhow::anyhow!("Failed to get journey ID from response")
                })?;
            println!("✅ Journey published successfully!");
            println!("🔗 Marketplace ID: {}", journey_id.to_string().green());
            println!("🌐 View at: https://cargo.do/marketplace");
            Ok(journey_id.to_string())
        } else {
            let error = api_response["error"].as_str().unwrap_or("Unknown error");
            Err(anyhow::anyhow!("Marketplace error: {}", error))
        }
    }
    pub fn download_from_marketplace(
        journey_id: &str,
        email: Option<&str>,
    ) -> Result<()> {
        println!(
            "📥 Downloading journey {} from Cargo Mate Marketplace...", journey_id
            .cyan()
        );
        let download_data = serde_json::json!(
            { "journey_id" : journey_id.parse::< u64 > ().unwrap_or(0), "email" : email
            .unwrap_or("") }
        );
        let output = Command::new("curl")
            .args(
                &[
                    ...
                    "https://cargo.do/api/marketplace/download-journey",
                ],
            )
            .stdout(Stdio::piped())
            .stderr(Stdio::piped())
        ...
        if !output.status.success() {
            let error = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Marketplace download error: {}", error));
        }
        let response = String::from_utf8_lossy(&output.stdout);
        if response.trim().is_empty() {
            return Err(anyhow::anyhow!("Marketplace download returned empty response"));
        }
        let api_response: serde_json::Value = serde_json::from_str(&response)
            .map_err(|e| anyhow::anyhow!("Failed to parse download response: {}", e))?;
        if api_response["success"].as_bool().unwrap_or(false) {
            if let Some(files) = api_response["files"].as_array() {
                if let Some(file) = files.get(0) {
                    if let (Some(filename), Some(content)) = (
                        ...
                        let journey_dir = dirs::home_dir()
                            ...
                            .join("journeys");
                        fs::create_dir_all(&journey_dir)?;
                        let journey_file = journey_dir.join(filename);
                        fs::write(&journey_file, content)?;
                        println!("✅ Journey downloaded successfully!");
                        println!(
                            "📁 Saved to: {}", journey_file.display().to_string()
                            .dimmed()
                        );
                        if let Ok(journey) = serde_json::from_str::<Journey>(content) {
                            println!("📝 Title: {}", journey.name.cyan());
                            println!("📖 Description: {}", journey.description);
                            if let Some(author) = &journey.author {
            ...
            return Err(anyhow::anyhow!("No valid files found in download response"));
        } else {
            let error = api_response["error"].as_str().unwrap_or("Unknown error");
            Err(anyhow::anyhow!("Marketplace download error: {}", error))
        }
    }
    pub fn search_marketplace(
        ...
        category: Option<&str>,
        limit: usize,
    ) -> Result<Vec<MarketplaceJourney>> {
        println!("🔍 Searching Cargo Mate Marketplace for '{}'...", query.cyan());
        let params = if let Some(cat) = category {
            format!(
                "q={}&category={}&limit={}", urlencoding::encode(query),
                urlencoding::encode(cat), limit
            )
        } else {
            format!("q={}&limit={}", urlencoding::encode(query), limit)
        };
        let output = Command::new("curl")
            .args(
                &[
                    ...
                    "GET",
                    &format!(
                        "https://cargo.do/api/marketplace/search-journeys?{}", params
                    ),
                ],
            )
            .stdout(Stdio::piped())
        ...
        if !output.status.success() {
            let error = String::from_utf8_lossy(&output.stderr);
            return Err(anyhow::anyhow!("Marketplace search error: {}", error));
        }
        let response = String::from_utf8_lossy(&output.stdout);
        if response.trim().is_empty() {
            return Err(anyhow::anyhow!("Marketplace search returned empty response"));
        }
        let api_response: serde_json::Value = serde_json::from_str(&response)
            .map_err(|e| anyhow::anyhow!("Failed to parse search response: {}", e))?;
        if api_response["success"].as_bool().unwrap_or(false) {
            let mut journeys = Vec::new();
            if let Some(journeys_array) = api_response["journeys"].as_array() {
                for journey in journeys_array {
                    journeys
                        .push(MarketplaceJourney {
                            gist_id: journey["journey_id"]
                                .as_str()
                                .unwrap_or("unknown")
                                .to_string(),
                            name: journey["title"]
                            ...
                            tags: journey["tags"]
                                ...
                                .unwrap_or_default(),
                            downloads: journey["download_count"].as_u64().unwrap_or(0)
                                as u32,
                            rating: journey["average_rating"].as_f64().unwrap_or(0.0)
                                as f32,
                            created: DateTime::parse_from_rfc3339(
                                    journey["created_at"].as_str().unwrap_or(""),
                                )
                                .ok()
                                .map(|dt| dt.with_timezone(&Utc))
                                .unwrap_or_else(Utc::now),
                            updated: DateTime::parse_from_rfc3339(
                                    journey["updated_at"].as_str().unwrap_or(""),
                                )
                                .ok()
                                .map(|dt| dt.with_timezone(&Utc))
                                .unwrap_or_else(Utc::now),
            ...
            if journeys.is_empty() {
                ...
                println!("Found {} journey(s) in marketplace:", journeys.len());
                for (i, journey) in journeys.iter().enumerate() {
                    println!(
                    ...
                    println!("   {}", journey.description);
                    println!(
                        "   ⭐ Rating: {:.1} • 📥 Downloads: {} • 🏷️  Tags: {}",
                        journey.rating, journey.downloads, journey.tags.join(", ")
                    );
                    println!("   📋 ID: {}", journey.gist_id.dimmed());
                }
            }
            ...
            let error = api_response["error"].as_str().unwrap_or("Unknown error");
            Err(anyhow::anyhow!("Marketplace search error: {}", error))
        }
    }
}
