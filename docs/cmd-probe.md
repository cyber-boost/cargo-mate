...
**Source**: `cargo-mate/captain/src/probe.rs` and `cargo-mate/captain/src/cmd/smune.rs:Commands::Probe`
...
**Main Handler**: `probe.rs:handle_probe()` (verified in `probe.rs:274-307`)

**Implementation**: All subcommands are handled by matching on `ProbeAction` enum:
- `Flake` → `handle_flake()` (verified in `probe.rs:276-277`)
- `Impact` → `handle_impact()` (verified in `probe.rs:279-280`)
- `Coverage` → `handle_coverage()` (verified in `probe.rs:282-283`)
- `Profile` → `handle_profile()` (verified in `probe.rs:285-286`)
- `Tag` → `handle_tag()` (verified in `probe.rs:288-289`)
- `CiGen` → `handle_ci_gen()` (verified in `probe.rs:291-292`)
- `Env` → `handle_env()` (verified in `probe.rs:294-295`)
- `Replay` → `handle_replay()` (verified in `probe.rs:297-298`)
- `Order` → `handle_order()` (verified in `probe.rs:300-301`)
- `Doc` → `handle_doc()` (verified in `probe.rs:303-304`)
